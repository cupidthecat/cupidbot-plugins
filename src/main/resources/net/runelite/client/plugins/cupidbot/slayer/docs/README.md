...
- **Inventory Setups Integration**: Uses CupidBot Inventory Setups to load correct gear per task (via task profiles)
...
- **Break Handler**: Integrates with CupidBot break handler; only allows breaks during safe states (Idle, At Location)
...
|-------|------|---------|-------------|
| `setup` | string | - | Inventory setup name to load from CupidBot Inventory Setups |
...
7. **Traveling** - Uses the CupidBot walker to navigate to the task location (from profile or auto-detected).
...
- CupidBot client version 2.1.11 or higher
- Slayer skill unlocked
- Sufficient combat level for your chosen slayer master
- Food and prayer potions in your bank
- CupidBot Inventory Setups plugin configured with gear setups matching your profile names
...
- **Inventory Setups**: Create named gear setups (e.g., "melee", "ranged", "burst") in the CupidBot Inventory Setups plugin. Your JSON profile `setup` field should match these names.
...
- **Break Handler**: The plugin integrates with the CupidBot break handler and only pauses during safe states (Idle, At Location) to avoid breaking mid-bank or mid-travel.
