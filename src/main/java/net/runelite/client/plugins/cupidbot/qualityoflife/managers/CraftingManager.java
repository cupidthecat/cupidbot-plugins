...
import net.runelite.client.plugins.cupidbot.qualityoflife.enums.DragonhideCrafting;
...
public class CraftingManager {
    ...
    private void craftHideOnClick(MenuEntry event) {
        ...
        if (craftableHides.isEmpty()) {
            CupidBot.log("<col=5F1515>No craftable hides found in inventory</col>");
            return;
        }

        Rs2ItemModel hideItem = craftableHides.get(0);
        if (hideItem == null) {
            CupidBot.log("<col=5F1515>Could not find the selected hide in inventory</col>");
            return;
        }

        CupidBot.log("<col=245C2D>Crafting: " + hideItem.getName() + "</col>");
