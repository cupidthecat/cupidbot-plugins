package net.runelite.client.plugins.cupidbot.wintertodt;
...
import net.runelite.client.plugins.cupidbot.wintertodt.enums.State;
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if (!init) {
                    ...
                    state = State.BANKING;
                    sleep(2000);
                    CupidBot.log("Validating inventory...");
                    if (config.axeInInventory()) {
                        if (!Rs2Inventory.hasItem("axe")) {
                            CupidBot.showMessage("It seems that you selected axeInInventory option but no axe was found in your inventory.");
                            sleep(5000);
                            return;
                        }
                        ...
                        CupidBot.showMessage("Please wear an axe OR If you'd like to have an axe in your inventory, you can enable the setting 'Axe In Inventory'.");
                        sleep(5000);
                        return;
                    }
                    init = true;
                ...
                brazierModel = CupidBot.getRs2TileObjectCache().query().withId(BRAZIER_29312)
                        .where(o -> o.getWorldLocation().equals(config.brazierLocation().getOBJECT_BRAZIER_LOCATION())).nearest();
                Rs2TileObjectModel brokenBrazierModel = CupidBot.getRs2TileObjectCache().query().withId(ObjectID.BRAZIER_29313)
                        .where(o -> o.getWorldLocation().equals(config.brazierLocation().getOBJECT_BRAZIER_LOCATION())).nearest();
                Rs2TileObjectModel fireBrazierModel = CupidBot.getRs2TileObjectCache().query().withId(ObjectID.BURNING_BRAZIER_29314)
                        .where(o -> o.getWorldLocation().equals(config.brazierLocation().getOBJECT_BRAZIER_LOCATION())).nearest();
                ...
                switch (state) {
                    ...
                    case CHOP_ROOTS:
                        ...
                        if (!Rs2Player.isAnimating()) {
                            CupidBot.getRs2TileObjectCache().query().interact(ObjectID.BRUMA_ROOTS, "Chop");
                    ...
                    case FLETCH_LOGS:
                        ...
                        break;
                    case BURN_LOGS:
                        if (!CupidBot.isGainingExp || resetActions) {
                            Rs2TileObjectModel burningBrazier = CupidBot.getRs2TileObjectCache().query().withId(BURNING_BRAZIER_29314).nearest();
                            if (brokenBrazierModel != null && config.fixBrazier()) {
                                brokenBrazierModel.click("fix");
                                CupidBot.log("Fixing brazier");
...
//            }
            ...
            if (brazierModel != null && brazierModel.getClickbox() != null) {
                CupidBot.getMouse().move(brazierModel.getClickbox().getBounds());
            }
        }
    }
    ...
    private void dodgeOrbDamage() {
        for (GraphicsObject graphicsObject : CupidBot.getClient().getGraphicsObjects()) {
            if (!resetActions && graphicsObject.getId() == 502
                    && WorldPoint.fromLocalInstance(CupidBot.getClient(),
                    graphicsObject.getLocation()).distanceTo(Rs2Player.getWorldLocation()) == 1) {
                //walk south
                List<Rs2TileObjectModel> gameObjects = CupidBot.getRs2TileObjectCache().query().within(5).toList();
    ...
    private boolean handleBankLogic(MWintertodtConfig config) {
        if (config.rejuvenationPotions()) {
            ...
            if (Rs2Inventory.hasItemAmount("Rejuvenation potion ", config.foodAmount())) {
                ...
                Rs2TileObjectModel crate = CupidBot.getRs2TileObjectCache().query()
                ...
                Rs2TileObjectModel roots = CupidBot.getRs2TileObjectCache().query()
        ...
        if (!Rs2Bank.hasBankItem(config.food().getName(), config.foodAmount(), true)) {
            CupidBot.showMessage("Insufficient food supply");
    ...
    public int getWarmthLevel() {
        ...
        if (warmthWidgetText == null || warmthWidgetText.isEmpty()) {
            CupidBot.log("No Warmth Level Found");
