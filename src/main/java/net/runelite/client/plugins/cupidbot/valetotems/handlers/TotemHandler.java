...
import net.runelite.client.plugins.cupidbot.valetotems.enums.SpiritAnimal;
import net.runelite.client.plugins.cupidbot.valetotems.enums.TotemLocation;
import net.runelite.client.plugins.cupidbot.valetotems.models.TotemProgress;
...
public class TotemHandler {
    ...
    public static boolean identifySpiritAnimals(TotemLocation totemLocation, TotemProgress progress) {
        try {
            ...
            while (attemptCount < maxAttempts && !allIdentified) {
                attemptCount++;
                CupidBot.log("Attempting to identify spirit animals (attempt " + attemptCount + "/" + maxAttempts + ")");

                // Clear previous identifications for this attempt
                progress.clearIdentifiedAnimals();

                // Search for spirit animals in the area
                List<Rs2NpcModel> nearbyNpcs = CupidBot.getRs2NpcCache().query().toList().stream()
                ...
                CupidBot.log("Found " + nearbyNpcs.size() + " spirit animals in the area");
                ...
                if (!allIdentified && attemptCount < maxAttempts) {
                    CupidBot.log("Not enough spirit animals found or identified. Retrying in 2 seconds...");
                    sleep(2000);
                }
            }

            if (!allIdentified) {
                CupidBot.log("Failed to identify all spirit animals after " + maxAttempts + " attempts");
            }
    ...
    public static boolean carveAnimalsIntoTotem(TotemLocation totemLocation, TotemProgress progress) {
        try {
            ...
            CupidBot.log("Carving animals into totem");
        ...
        try {
            if (!sleepUntil(() -> Rs2Widget.hasWidgetText("What animal would you like to carve?",270,5, false), 5000)) {
                CupidBot.getClientThread().invoke(() ->
                        CupidBot.getRs2TileObjectCache().query().withNameContains(GameObjectId.EMPTY_TOTEM.getSearchTerm()).interact("Carve"));
                if (!sleepUntil(() -> Rs2Widget.hasWidgetText("What animal would you like to carve?",270,5, false), 5000)) {
    ...
    public static boolean decorateTotem(TotemLocation totemLocation, TotemProgress progress) {
        try {
            ...
            CupidBot.log("Decorating totem with fletched bows");
            ...
            while (attemptCount < maxAttempts && !decorated) {
                ...
                if (!decorated && attemptCount < maxAttempts) {
                    CupidBot.log("Decoration attempt failed, retrying in 1 second...");
                    sleep(1000);
                }
            }

            if (!decorated) {
                CupidBot.log("Failed to decorate totem after " + maxAttempts + " attempts");
                return false;
            }
            ...
            if (decorated) {
                ...
                if (endingBows - startingBows == -4) {
                    ...
                    CupidBot.log("Totem decoration completed!");
                    return true;
                }
            }
    ...
    private static void hoverOverSpiritAnimal(SpiritAnimal animal) {
        ...
        WorldPoint animalLocation = CupidBot.getRs2NpcCache().query().toList().stream()
        ...
        Rs2NpcModel animalNpc = CupidBot.getRs2NpcCache().query().toList().stream()
                .filter(npc -> npc.getId() == animal.getNpcId())
                .findFirst()
                .orElse(null);
