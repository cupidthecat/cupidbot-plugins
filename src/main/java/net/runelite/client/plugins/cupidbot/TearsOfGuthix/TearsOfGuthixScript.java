package net.runelite.client.plugins.cupidbot.TearsOfGuthix;
...
public class TearsOfGuthixScript extends Script {
    ...
    public boolean run(TearsOfGuthixConfig config) {
        CupidBot.log("Starting Tears of Guthix script");
                ...
                switch (BOT_STATUS) {
                    case TRAVELLING:
                        if (Rs2Player.getWorldLocation().getRegionID() == TOG_REGION) {
                            CupidBot.log("Already in Tears of Guthix region, collecting tears...");
                            BOT_STATUS = State.GETTING_WORLD;
                            break;
                        }
                        var junaLocation = new WorldPoint(3251, 9516, 2);
                        if (!Rs2Inventory.hasItem("Games necklace")) {
                            CupidBot.log("No games necklace in inventory, going to bank...");
                            Rs2Bank.walkToBank();
                            Rs2Bank.openBank();
                            ...
                            if (Rs2Inventory.hasItem("Games necklace")) {
                                ...
                                CupidBot.log("No games necklace in bank, stopping script.");
                                shutdown();
                            }
                        } else {
                            CupidBot.log("Travelling to Tears of Guthix...");
                    ...
                    case GETTING_WORLD:
                        if (config.queryForOptimalWorld()) {
                            CupidBot.log("Querying for optimal world...");
                            getBestWorld();
                        } else {
                            CupidBot.log("Skipping world query, using current world.");
    ...
    private void getTears() {
        ...
        if (Rs2Player.getWorldLocation().getRegionID() != TOG_REGION) {
            if (hasBeenRunning) {
                CupidBot.log("Done collecting tears - shutting down.");
                shutdown();
            }
            CupidBot.log("Not in Tears of Guthix region, travelling...");
    ...
    private void collectTears() {
        ...
        if (currentStream != null && currentStreamSpawnTime != null) {
            ...
            if (timeRemaining > 0) {
                ...
                CupidBot.log(status + " stream: ~" + timeRemaining + "s remaining" + timeSinceClick);
            }
        }
    ...
    private void clickStream(DecorativeObject stream, long timeRemaining) {
        ...
        CupidBot.getClientThread().invoke(() -> {
            CupidBot.getMouse().click(stream.getCanvasLocation());
            CupidBot.log("Switching to blue tear stream (Time left: ~" + timeRemaining + "s, Distance: " + distance + " tiles)");
            return true;
        });
    }
    ...
    private void getBestWorld() {
        if (bestWorld != 0) {
            if (CupidBot.getClient().getWorld() == bestWorld) {
                CupidBot.log("Already in the best world: " + bestWorld);
                BOT_STATUS = State.GETTING_TEARS;
                return;
            }
            boolean isHopped = CupidBot.hopToWorld(bestWorld);
            if (!isHopped) return;
        ...
        CupidBot.log("Fetching optimal world for tears collection...");
        ...
        try {
            ...
            CupidBot.log("Fetched data for " + worldData.size() + " worlds");
            ...
            if (optimalWorld != null) {
                ...
                CupidBot.log("Found optimal world: " + bestWorld +
                        " (Hits: " + optimalWorld.getHits() +
                        ", Pattern: " + optimalWorld.getStream_order() + ")");

            } else {
                CupidBot.log("No optimal world found, continuing with current world");
                BOT_STATUS = State.GETTING_TEARS;
            }
        } catch (Exception e) {
            CupidBot.log("Error fetching TOG world data: " + e.getMessage());
            BOT_STATUS = State.GETTING_TEARS;
        }
    }
    ...
    public void shutdown() {
        ...
        CupidBot.log("Shutting down Tears of Guthix script");
    }
