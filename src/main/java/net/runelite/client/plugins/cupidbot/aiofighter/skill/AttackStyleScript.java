package net.runelite.client.plugins.cupidbot.aiofighter.skill;
...
public class AttackStyleScript extends Script {
    ...
    private void scheduledTask(AIOFighterConfig config) {
        try {
            ...
            if (!CupidBot.isLoggedIn() || !super.run() || !config.toggleAllowStyleSwitch() || disableIfMaxed(config.toggleDisableOnMaxCombat()))
                return;
            ...
            if (attackStyle == AttackStyle.LONGRANGE || attackStyle == AttackStyle.RANGING) {
                ...
                if (weaponAttackType != null && weaponAttackType.getAttackOptions().stream().anyMatch(attackOption -> attackOption.getStyle().equals("Rapid"))) {
                    ...
                    int attackStyleVarbit = CupidBot.getVarbitPlayerValue(VarPlayer.ATTACK_STYLE);
            ...
            CupidBot.log("Current Attack Style: " + attackStyle.getName());
            CupidBot.log("Attack Style to Train: " + attackStyleToTrain.getName());
            // Change attack style if needed
            if (attackStyle != attackStyleToTrain) {
                changeAttackStyle(config, componentToDisplay);
            }
        } catch (Exception ex) {
            CupidBot.logStackTrace("AttackStyleScript", ex);
        }
    ...
    private void updateAttackStyleInfo() {
        int attackStyleVarbit = CupidBot.getVarbitPlayerValue(VarPlayer.ATTACK_STYLE);
        equippedWeaponTypeVarbit = CupidBot.getVarbitValue(Varbits.EQUIPPED_WEAPON_TYPE);
        int castingModeVarbit = CupidBot.getVarbitValue(Varbits.DEFENSIVE_CASTING_MODE);
    ...
    private AttackStyle[] getWeaponTypeStyles(int weaponType) {
        int weaponStyleEnum = CupidBot.getEnum(EnumID.WEAPON_STYLES).getIntValue(weaponType);
        ...
        for (int style : weaponStyleStructs) {
            String attackStyleName = CupidBot.getStructComposition(style).getStringValue(ParamID.ATTACK_STYLE_NAME);
    ...
    private int getSkillLevel(Skill skill) {
        return CupidBot.getClient().getRealSkillLevel(skill);
    }
