        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if (npc != null) {
                    ...
                    var configuredNpc = CupidBot.getRs2NpcCache().query()
                            .withName(targetNpcName)
                            .nearestOnClientThread();
                    if (configuredNpc == null) {
                        CupidBot.log("Unable to find NPC: " + targetNpcName);
                        return;
                    }
                    if (!Rs2Magic.cast(plugin.getStunSpell().getRs2Spell().getMagicAction())) {
                        CupidBot.log("Unable to select stun spell: " + plugin.getStunSpell().name());
                        return;
                    }
                    ...
                    if (!configuredNpc.click()) {
                        CupidBot.log("Unable to cast stun on NPC: " + targetNpcName);
                        return;
                    }
                }
                ...
                CupidBot.log("Stun-Alch loop failed: " + ex.getMessage());
            }
