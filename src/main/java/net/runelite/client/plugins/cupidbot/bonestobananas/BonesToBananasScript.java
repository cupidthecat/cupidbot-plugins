...
import net.runelite.client.plugins.cupidbot.bonestobananas.util.ElementalStaff;
...
public class BonesToBananasScript extends Script {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if (!BONES_TO_BANANAS_SPELL.hasRequiredLevel()) {
                    CupidBot.showMessage("Magic level is too low.");
                    shutdown();
                    return;
                }
    ...
    private void handleBoneWithdrawal() {
        if (!Rs2Bank.hasItem(BONES_ID)) {
            CupidBot.showMessage("You have no bones left in your bank.");
            shutdown();
            return;
        }
