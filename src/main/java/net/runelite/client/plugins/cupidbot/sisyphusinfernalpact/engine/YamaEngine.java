package net.runelite.client.plugins.cupidbot.sisyphusinfernalpact.engine;
...
import net.runelite.client.plugins.cupidbot.sisyphusinfernalpact.SisyphusInfernalPactConfig;
...
public class YamaEngine {
    ...
    public boolean tick(SisyphusInfernalPactConfig config) {
        ...
        CupidBot.log("[Yama] fire=" + fireLocs.size()
     ...
     * Queries both the CupidBot NPC cache (manual ID filter) and the raw
    ...
    private List<WorldPoint> getAllFireLocations() {
        ...
        // Source 1: CupidBot NPC cache
        try {
            List<Rs2NpcModel> cached = CupidBot.getRs2NpcCache().query().toList();
            if (cached != null) {
        ...
        try {
            List<NPC> clientNpcs = CupidBot.getClient().getNpcs();
            if (clientNpcs != null) {
