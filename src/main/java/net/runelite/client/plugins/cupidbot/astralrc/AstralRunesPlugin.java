...
@PluginDescriptor(
        ...
        tags = {"astral", "rune", "cupidbot"},
        authors = { "Maxxin" },
        version = AstralRunesPlugin.version,
        minClientVersion = "2.0.7",
        iconUrl = "AstralRunesPlugin/assets/icon.png",
        cardUrl = "AstralRunesPlugin/assets/card.png",
...
public class AstralRunesPlugin extends Plugin {
    ...
    public void onChatMessage(ChatMessage event) {
        if (event.getType() == ChatMessageType.GAMEMESSAGE && !CupidBot.pauseAllScripts.get()) {
            if (event.getMessage().toLowerCase().contains("you don't have enough coins.")) {
                CupidBot.status = "[Shutting down] - Reason: Not enough coins.";
