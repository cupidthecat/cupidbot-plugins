package net.runelite.client.plugins.cupidbot.delveprayerhelper;

import lombok.extern.slf4j.Slf4j;
import net.runelite.api.Projectile;
import net.runelite.client.plugins.cupidbot.CupidBot;
import net.runelite.client.plugins.cupidbot.Script;
import net.runelite.client.plugins.cupidbot.delveprayerhelper.enums.DelvePrayerHelperState;
import net.runelite.client.plugins.cupidbot.delveprayerhelper.enums.DelvePrayerHelperProjectile;
...
public class DelvePrayerHelperScript extends Script {
    ...
    void handleProjectiles() {
        int currentCycle = CupidBot.getClient().getGameCycle();
        Rs2NpcModel boss = CupidBot.getRs2NpcCache().query().where(n -> n.getName() != null && n.getName().contains("Doom of Mokhaiotl")).nearest();
        ...
        if (!bossAlive) {
            ...
            CupidBot.status = state.toString();
    ...
    private void toggleOffensivePrayer() {
        if(config.offensivePrayer()) {
            var doom = CupidBot.getRs2NpcCache().query().where(n -> n.getName() != null && n.getName().contains("Doom of Mokhaiotl")).nearest();
