package net.runelite.client.plugins.cupidbot.DemonicGorillaKiller;
...
public class DemonicGorillaScript extends Script {
    ...
    public boolean run(DemonicGorillaConfig config) {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                if (!CupidBot.isLoggedIn() || !super.run()) return;
    ...
    private void preflightChecks(DemonicGorillaConfig config) {
        if (config.useMagicStyle() && config.magicGear() == null) {
            CupidBot.showMessage("You've selected magic combatstyle, but your magic inventory setup is null. Please set it in the config. If you already have one selected. Select another inventory setup and then select your magic setup again.");
            shutdown();
            return;
        }
        if (config.useRangeStyle() && config.rangeGear() == null) {
            CupidBot.showMessage("You've selected ranged combatstyle, but your range inventory setup is null. Please set it in the config. If you already have one selected. Select another inventory setup and then select your range setup again.");
            shutdown();
            return;
        }
        if (config.useMeleeStyle() && config.meleeGear() == null) {
            CupidBot.showMessage("You've selected melee combatstyle, but your melee inventory setup is null. Please set it in the config. If you already have one selected. Select another inventory setup and then select your melee setup again.");
            shutdown();
            return;
        }
        if (config.gearSetup() == null) {
            CupidBot.showMessage("Your banking gear setup is null. Please set it in the config. If you already have one selected. Select another inventory setup and then select your gear setup again.");
            shutdown();
            return;
        }
        ...
        Plugin qolPlugin = CupidBot.getActiveCupidBotPlugins().stream().filter(p -> p.getClass().getSimpleName().contains("QoLPlugin"))
                .findFirst().orElse(null);

        if (qolPlugin != null) {
            CupidBot.stopPlugin(qolPlugin.getClass());
        }
    }
    ...
    private void handleBanking(DemonicGorillaConfig config) {
        ...
        switch (bankingStep) {
            ...
            case LOAD_INVENTORY:
                CupidBot.status = "Loading inventory and equipment setup...";
                ...
                if (equipmentLoaded && inventoryLoaded) {
                    ...
                    while (Rs2Player.getHealthPercentage() < 70 || ateFood || drankPrayerPot) {
                        ...
                        CupidBot.log("Eating food or drinking prayer potion before going to gorillas.");
        ...
        if (!Rs2Player.isInCombat()) {
            if (outOfCombatTime == null) {
                ...
                if (currentTarget != null) {
                    ...
                    currentTarget = getTarget(true);
                    if (currentTarget == null) {
                        CupidBot.getClientThread().invoke(() -> CupidBot.getRs2NpcCache().query().withName("Demonic gorilla").interact("Attack"));
                    }
                }
    ...
    private void retreatToSafety(DemonicGorillaConfig config) {
        ...
        escapeTosafety();
        sleepUntil(() -> SAFE_LOCATION.equals(CupidBot.getClientThread().invoke(() -> CupidBot.getClient().getLocalPlayer().getWorldLocation())), 5000);
        disableAllPrayers();
        CupidBot.pauseAllScripts.compareAndSet(true, false);
        BOT_STATUS = State.BANKING;
        CupidBot.log("Changing to default gear");
    ...
    private void handleDemonicGorillaAttacks(DemonicGorillaConfig config) {
        ...
        if (currentTarget != null && !currentTarget.getNpc().isDead()) {
            ...
            if (currentAnimation == DEMONIC_GORILLA_AOE_ATTACK && demonicGorillaRockPosition != null) {
                ...
                dangerousWorldPoints.add(CupidBot.getClientThread().invoke(() -> CupidBot.getClient().getLocalPlayer().getWorldLocation()));
            ...
            if (!dodgedRock) {
                if ((currentGear == ArmorEquiped.RANGED || currentGear == ArmorEquiped.MAGIC) && (currentAnimation != DEMONIC_GORILLA_MELEE_ATTACK && currentAnimation != -1 && currentDefensivePrayer != Rs2PrayerEnum.PROTECT_MELEE)) {
                    var isMeleeDist = currentTarget.getNpc().getWorldArea().isInMeleeDistance(CupidBot.getClient().getLocalPlayer().getWorldArea());
                    if (isMeleeDist) {
                        moveAwayFromTarget();
                    }
                }
    ...
    public Rs2NpcModel getTarget(boolean force) {
        ...
        if (interacting != null) {
            if (Objects.equals(interacting.getName(), "Demonic gorilla")) {
                var match = CupidBot.getRs2NpcCache().query().withName("Demonic gorilla")
                        .where(n -> n.isInteractingWithPlayer()).nearest();
                if (match != null) return match;
            }
        }
        var playerLocation = CupidBot.getClientThread().invoke(() -> CupidBot.getClient().getLocalPlayer().getWorldLocation());

        var alreadyInteractingNpcs = CupidBot.getRs2NpcCache().query().withName("Demonic gorilla")
        ...
        List<Rs2NpcModel> demonicGorillas = CupidBot.getRs2NpcCache().query().withName("Demonic gorilla").toListOnClientThread();
    ...
    private WorldPoint findSafeTile(WorldPoint playerLocation, List<WorldPoint> dangerousWorldPoints) {
        ...
        for (WorldPoint tile : nearbyTiles) {
            final LocalPoint location = LocalPoint.fromWorld(CupidBot.getClient(), tile);
