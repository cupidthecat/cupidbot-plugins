...
import static net.runelite.client.plugins.cupidbot.AmmoniteCrabs.enums.AmmoniteCrabState.FIGHT;
...
public class AmmoniteCrabScript extends Script {
    ...
    private void preStateChecks(AmmoniteCrabConfig config) {
        ...
        if (CupidBot.getClientThread().invoke(() -> CupidBot.getClient().getLocalPlayer().getWorldLocation()).distanceTo(config.crabLocation().getWorldhopLocation()) > 10 && (ammoniteCrabState != AmmoniteCrabState.RESET_AGGRO && ammoniteCrabState != AmmoniteCrabState.WALK_BACK && ammoniteCrabState != AmmoniteCrabState.BANK)) {
    ...
    private void scanAmmoniteCrabLocation(AmmoniteCrabConfig config) {
        ...
        if (config.crabLocation().getWorldhopLocation().distanceTo(CupidBot.getClientThread().invoke(() -> CupidBot.getClient().getLocalPlayer().getWorldLocation())) > 2) {
            Rs2Walker.walkTo(config.crabLocation().getWorldhopLocation());
            return;
        }
    ...
    private void walkBack(AmmoniteCrabConfig config) {
        ...
        if (CupidBot.getClientThread().invoke(() -> CupidBot.getClient().getLocalPlayer().getWorldLocation()).distanceTo(config.crabLocation().getFightLocation()) <= 3) {
    ...
    private void attackScatteredCrabs(AmmoniteCrabConfig config) {
        var ammoniteCrabs = CupidBot.getRs2NpcCache().query().withName("Ammonite Crab")
    ...
    private boolean isNpcAggressive() {
        List<Rs2NpcModel> npcs = CupidBot.getRs2NpcCache().query().withName("Fossil Rock").toListOnClientThread();
        if (npcs.isEmpty()) {
            return true;
        }
        for (Rs2NpcModel ammoniteRock : npcs) {
            if (!ammoniteRock.getNpc().getWorldArea().isInMeleeDistance(CupidBot.getClient().getLocalPlayer().getWorldArea()))
                continue;
    ...
    private void resetAggro(AmmoniteCrabConfig config) {
        ...
        if (CupidBot.getClientThread().invoke(() -> CupidBot.getClient().getLocalPlayer().getWorldLocation()).distanceTo(config.crabLocation().getResetLocation()) <= 3) {
            ammoniteCrabState = AmmoniteCrabState.WALK_BACK;
        }
    }
