...
public class QoLScript extends Script {
    ...
    private void handleInventorySetup() {
        if (!openBank()) {
            CupidBot.log("Bank did not open");
    ...
    /**
     * Client-only reads via {@code CupidBot.getClientThread().invoke} (see AutoPrayer).
    ...
    private void handleWorkbenchActions() {
        ...
        CupidBot.doInvoke(QoLPlugin.workbenchMenuEntry, new Rectangle(1, 1));
    ...
    private void processBankMenuEntry(NewMenuEntry menuEntry) {
        ...
        CupidBot.doInvoke(menuEntry, Objects.requireNonNull(menuEntry.getWidget()).getBounds());
