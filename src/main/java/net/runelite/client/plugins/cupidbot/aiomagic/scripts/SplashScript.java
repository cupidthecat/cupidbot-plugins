		...
		mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
			try {
				...
				if (!Rs2Magic.canCast(plugin.getCombatSpell().getMagicAction())) {
					CupidBot.showMessage("Out of runes for " + plugin.getCombatSpell().name());
					shutdown();
					return;
				}
				...
				if (Rs2Magic.getCurrentAutoCastSpell() != plugin.getCombatSpell()) {
					...
					if (interacting != null && interacting.getName() != null && interacting.getName().equalsIgnoreCase(plugin.getNpcName())) {
						...
						CupidBot.log("Splashing stalled or timed out. Re-engaging...");
					}
					...
					if (targetNpcName.isEmpty()) {
						CupidBot.showMessage("Set an NPC name in config");
						shutdown();
						return;
					}

					Rs2NpcModel targetNpc = CupidBot.getRs2NpcCache().query()
