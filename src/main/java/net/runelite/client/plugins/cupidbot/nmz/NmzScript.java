...
public class NmzScript extends Script {
    ...
    public boolean run() {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                            ...
                            if (config.inventorySetup() != null) {
                                ...
                                if (!inventorySetup.doesInventoryMatch() || !inventorySetup.doesEquipmentMatch()) {
                                    ...
                                    if (!inventorySetup.loadEquipment() || !inventorySetup.loadInventory()) {
                                        CupidBot.log("Failed to load inventory setup");
                ...
                useOverload = CupidBot.getClient().getBoostedSkillLevel(Skill.RANGED) == CupidBot.getClient().getRealSkillLevel(Skill.RANGED) && config.overloadPotionAmount() > 0;
                if (isOutsideNmz) {
    ...
    public void handleOutsideNmz() {
        boolean hasStartedDream = CupidBot.getVarbitValue(VarbitID.NZONE_PURCHASEDDREAM) > 0;
    ...
    public void manageSelfHarm() {
        int currentHP = CupidBot.getClient().getBoostedSkillLevel(Skill.HITPOINTS);
        int currentRangedLevel = CupidBot.getClient().getBoostedSkillLevel(Skill.RANGED);
        int realRangedLevel = CupidBot.getClient().getRealSkillLevel(Skill.RANGED);
    ...
    public void useOverloadPotion() {
        if (useOverload && Rs2Inventory.hasItem("overload") && CupidBot.getClient().getBoostedSkillLevel(Skill.HITPOINTS) > 50) {
    ...
    public void useAbsorptionPotion() {
        if (CupidBot.getVarbitValue(VarbitID.NZONE_ABSORB_POTION_EFFECTS) < minAbsorption && Rs2Inventory.hasItem("absorption")) {
    ...
    public void consumeEmptyVial() {
        if (CupidBot.getClientThread().runOnClientThreadOptional(() ->
        ...
        if (!CupidBot.getClientThread().runOnClientThreadOptional(widget::isHidden).orElse(false)) {
    ...
    public void handleStore() {
        ...
        int overloadAmt = CupidBot.getVarbitValue(VarbitID.NZONE_POTION_3);
        int absorptionAmt = CupidBot.getVarbitValue(VarbitID.NZONE_POTION_4);
        ...
        int nmzPoints = CupidBot.getVarbitPlayerValue(VarPlayerID.NZONE_REWARDPOINTS);

        if (nmzPoints < totalCost) {
            CupidBot.showMessage("BOT SHUTDOWN: Not enough points to buy potions (have " + nmzPoints + ", need " + totalCost + ")");
            CupidBot.stopPlugin(plugin);
            return;
        }
