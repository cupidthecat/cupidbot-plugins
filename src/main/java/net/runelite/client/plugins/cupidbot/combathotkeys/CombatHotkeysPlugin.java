...
@PluginDescriptor(
        ...
        iconUrl = "CombatHotkeysPlugin/assets/icon.jpg",
        cardUrl = "CombatHotkeysPlugin/assets/card.jpg"
)
@Slf4j
    ...
    public void onMenuEntryAdded(MenuEntryAdded event)
    {
        if (event.getOption().equals("Walk here") && config.yesDance())
        {
            CupidBot.getClient().getMenu().createMenuEntry(-1)
                    ...
                    .onClick(e -> {
                        final var target = CupidBot.getClient().getTopLevelWorldView().getSelectedSceneTile();
                        if (target != null)
                        {
