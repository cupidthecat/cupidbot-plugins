package net.runelite.client.plugins.cupidbot.tormenteddemons;
...
public class TormentedDemonScript extends Script {
    ...
    private void handleTravel(TormentedDemonConfig config) {
        ...
        switch (travelStep) {
            case LOCATION_ONE:
                CupidBot.status = "Teleporting to Guthixian temple...";
            ...
            case CLIMB_FIRST_STAIRS:
                CupidBot.status = "Climbing first stairs...";
                if (CupidBot.getRs2TileObjectCache().query().interact(53623, "Climb-up")) {
                    Rs2Player.waitForAnimation();
                    sleepUntil(() -> !Rs2Player.isAnimating());
            ...
            case CLIMB_SECOND_STAIRS:
                CupidBot.status = "Climbing second stairs...";
                if (CupidBot.getRs2TileObjectCache().query().interact(53624, "Climb-up")) {
                    Rs2Player.waitForAnimation();
                    sleepUntil(() -> !Rs2Player.isAnimating());
            ...
            case CLIMB_THROUGH:
                CupidBot.status = "Climbing through the path...";
                if (CupidBot.getRs2TileObjectCache().query().interact(54082, "Climb-through")) {
                    Rs2Player.waitForAnimation();
                    sleepUntil(() -> !Rs2Player.isAnimating());
            ...
            case LOCATION_THREE:
                CupidBot.status = "Approaching Tormented Demon location...";
                if (Rs2Walker.walkTo(targetFinalLocation, 2)) {
                    sleepUntil(() -> targetFinalLocation.equals(CupidBot.getClientThread().invoke(() -> CupidBot.getClient().getLocalPlayer().getWorldLocation())), 5000);
                    travelStep = TravelStep.LOCATION_ONE;
                    BOT_STATUS = State.FIGHTING;
                }
                break;
    ...
    private void handleBanking(TormentedDemonConfig config) {
        ...
        switch (bankingStep) {
            case DRINK:
                CupidBot.status = "Drinking at Ferox Enclave pool...";
                int currentHealth = CupidBot.getClient().getBoostedSkillLevel(Skill.HITPOINTS);
                int maxHealth = CupidBot.getClient().getRealSkillLevel(Skill.HITPOINTS);
                int currentPrayer = CupidBot.getClient().getBoostedSkillLevel(Skill.PRAYER);
                int maxPrayer = CupidBot.getClient().getRealSkillLevel(Skill.PRAYER);

                if (currentHealth < maxHealth || currentPrayer < maxPrayer) {
                    if (CupidBot.getRs2TileObjectCache().query().interact(FEROX_POOL_ID, "Drink")) {
                        Rs2Player.waitForAnimation();
                        sleepUntil(() ->
                                CupidBot.getClient().getBoostedSkillLevel(Skill.HITPOINTS) == maxHealth &&
                                        CupidBot.getClient().getBoostedSkillLevel(Skill.PRAYER) == maxPrayer
                        );
    ...
    private void handleFighting(TormentedDemonConfig config) {
        if (currentTarget == null || currentTarget.isDead()) {
            ...
            if (currentTarget.getInteracting() != CupidBot.getClient().getLocalPlayer()) {
                currentTarget = findNewTarget(config);
            }
    ...
    private Rs2NpcModel findNewTarget(TormentedDemonConfig config) {
        return CupidBot.getRs2NpcCache().query()
                ...
                .where(npc -> npc.getInteracting() == null || npc.getInteracting() == CupidBot.getClient().getLocalPlayer())
                .where(npc -> {
