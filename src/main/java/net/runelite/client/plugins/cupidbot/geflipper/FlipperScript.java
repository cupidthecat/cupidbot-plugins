...
public class FlipperScript extends Script {
            ...
            try {
                ...
                switch (state) {
                    case GOING_TO_GE:
                        ...
                        if (!grandExchangeArea.contains(CupidBot.getClientThread().invoke(() -> CupidBot.getClient().getLocalPlayer().getWorldLocation()))) {
	...
	private Plugin getFlippingCopilot()
	{
		...
		{
			flippingCopilot = CupidBot.getPluginManager()
				.getPlugins()
				.stream()
