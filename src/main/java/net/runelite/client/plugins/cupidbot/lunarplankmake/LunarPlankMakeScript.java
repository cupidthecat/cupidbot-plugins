package net.runelite.client.plugins.cupidbot.lunarplankmake;

import net.runelite.client.plugins.cupidbot.CupidBot;
import net.runelite.client.plugins.cupidbot.Script;
import net.runelite.client.plugins.cupidbot.lunarplankmake.enums.Logs;
...
public class LunarPlankMakeScript extends Script {
    ...
    public boolean run(LunarPlankMakeConfig config) {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                CupidBot.log("Exception in LunarPlankMakeScript: " + ex.getMessage());
            }
    ...
    private void plankItems(LunarPlankMakeConfig config) {
        ...
        if (lazyMode) {
            ...
            if (waitUntilNoLogsRemaining(config, initialLogQuantity)) {
                ...
                CupidBot.log("Lazy mode: timed out waiting for logs to finish converting.");
        ...
        if (waitForInventoryChange(plankId, initialPlankCount)) {
            ...
            CupidBot.log("Failed to detect plank creation.");
            currentState = State.WAITING;
        }
    }
    ...
    private void bank(LunarPlankMakeConfig config) {
        ...
        if (!Rs2Bank.hasItem(logId)) {
            CupidBot.showMessage("No more " + config.ITEM().getName() + " to plank.");
            shutdown();
            return;
        }
    ...
    private static int gePrice(String itemName) {
        try {
            return CupidBot.getItemManager().search(itemName).get(0).getPrice();
        } catch (Exception e) {
            return 0;
        }
    }
