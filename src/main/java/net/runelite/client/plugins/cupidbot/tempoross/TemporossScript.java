package net.runelite.client.plugins.cupidbot.tempoross;
...
import net.runelite.client.plugins.cupidbot.tempoross.enums.HarpoonType;
...
public class TemporossScript extends Script {
    ...
    private void determineWorkArea() {
        if (workArea == null) {
            ...
            List<Rs2NpcModel> forfeitNpcs = CupidBot.getRs2NpcCache().query()
            ...
            List<Rs2NpcModel> ammoCrates = CupidBot.getRs2NpcCache().query()
                    .where(npc -> npc.getNpc() != null && npc.getNpc().getComposition() != null
    ...
    private void finishGame() {
        ...
        Rs2NpcModel exitNpc = CupidBot.getRs2NpcCache().query()
                .where(npc -> npc.getNpc() != null && npc.getNpc().getComposition() != null
        ...
        if (exitNpc != null) {
            ...
            if (emptyBucketCount > 0) {
                if(CupidBot.getRs2TileObjectCache().query().interact(41004, "Fill-bucket"))
    ...
    private void forfeit() {
        LocalPoint playerLocal = CupidBot.getClient().getLocalPlayer() != null
                ? CupidBot.getClient().getLocalPlayer().getLocalLocation() : null;
        if (playerLocal == null) return;
        var forfeitNpc = CupidBot.getRs2NpcCache().query()
                .where(npc -> npc.getNpc() != null && npc.getNpc().getComposition() != null
                        && Arrays.asList(npc.getNpc().getComposition().getActions()).contains("Forfeit"))
                .toList().stream()
                .filter(npc -> npc.getNpc().getLocalLocation() != null)
    ...
    private void fetchMissingItems()
    {
        ...
        if (!hasHarpoon() && harpoonType != HarpoonType.BAREHAND) {
            LocalPoint lp = LocalPoint.fromWorld(CupidBot.getClient().getTopLevelWorldView(), workArea.harpoonPoint);
        ...
        if (needBuckets) {
            LocalPoint lp = LocalPoint.fromWorld(CupidBot.getClient().getTopLevelWorldView(), workArea.bucketPoint);
        ...
        if (!needBuckets && fullBucketCount <= 0) {
            LocalPoint lp = LocalPoint.fromWorld(CupidBot.getClient().getTopLevelWorldView(), workArea.pumpPoint);
            needed.add(new int[]{2, lp != null ? playerLocal.distanceTo(lp) : Integer.MAX_VALUE});
        }

        if (temporossConfig.rope() && !temporossConfig.spiritAnglers() && !Rs2Inventory.contains(ItemID.ROPE)) {
            LocalPoint lp = LocalPoint.fromWorld(CupidBot.getClient().getTopLevelWorldView(), workArea.ropePoint);
        ...
        if (temporossConfig.hammer() && !Rs2Inventory.contains(ItemID.HAMMER)) {
            LocalPoint lp = LocalPoint.fromWorld(CupidBot.getClient().getTopLevelWorldView(), workArea.hammerPoint);
    ...
    private boolean isOnStartingBoat() {
        Rs2TileObjectModel startingLadder = CupidBot.getRs2TileObjectCache().query().withId(ObjectID.ROPE_LADDER_41305).nearest();
    ...
    private void handleEnterMinigame() {
        ...
        Rs2TileObjectModel waterPump = CupidBot.getRs2TileObjectCache().query().withId(ObjectID.WATER_PUMP_41000).nearest();
    ...
    public static void handleWidgetInfo() {
        try {
            Widget energyWidget = CupidBot.getClient().getWidget(InterfaceID.TEMPOROSS, 35);
            Widget essenceWidget = CupidBot.getClient().getWidget(InterfaceID.TEMPOROSS, 45);
            Widget intensityWidget = CupidBot.getClient().getWidget(InterfaceID.TEMPOROSS, 55);
    ...
    public static void updateFireData(){
        List<Rs2NpcModel> allFires = CupidBot.getRs2NpcCache().query()
                .where(npc -> npc.getNpc() != null && npc.getNpc().getComposition() != null
        ...
        LocalPoint exitLocal = workArea != null
                ? LocalPoint.fromWorld(CupidBot.getClient().getTopLevelWorldView(), workArea.exitNpc) : null;
    ...
    public static void updateAmmoCrateData(){
        LocalPoint mastLocal = LocalPoint.fromWorld(CupidBot.getClient().getTopLevelWorldView(), workArea.mastPoint);
        List<Rs2NpcModel> ammoCrates = CupidBot.getRs2NpcCache().query()
                .where(npc -> npc.getNpc() != null && npc.getNpc().getComposition() != null
                        && Arrays.asList(npc.getNpc().getComposition().getActions()).contains("Fill")
    ...
    public static void updateFishSpotData(){
        fishSpots = CupidBot.getRs2NpcCache().query()
    ...
    private void handleFires() {
        ...
        for (Rs2NpcModel fire : sortedFires) {
            if(isFilling){
                CupidBot.log("Filling, skipping fire");
                return;
            }
    ...
    private void handleStateLoop() {
        temporossPool = CupidBot.getRs2NpcCache().query().withId(NpcID.SPIRIT_POOL)
                .where(npc -> npc.getNpc() != null && npc.getNpc().getComposition() != null
        ...
        switch (state) {
            ...
            case THIRD_CATCH:
                ...
                if (fishSpot != null && fishSpot.getNpc() != null) {
                    ...
                    LocalPoint localPoint = LocalPoint.fromWorld(CupidBot.getClient().getTopLevelWorldView(), workArea.totemPoint);
                    if (localPoint == null) {
            ...
            case THIRD_COOK:
                ...
                if (range != null && rawFishCount > 0) {
                    ...
                    LocalPoint localPoint = LocalPoint.fromWorld(CupidBot.getClient().getTopLevelWorldView(), workArea.rangePoint);
                    if (localPoint == null) {
            ...
            case INITIAL_FILL:
                ...
                LocalPoint fillPlayerLocal = CupidBot.getClient().getLocalPlayer() != null
                        ? CupidBot.getClient().getLocalPlayer().getLocalLocation() : null;
                ...
                if (inCloud(CupidBot.getClientThread().invoke(() -> CupidBot.getClient().getLocalPlayer().getLocalLocation()))) {
    ...
    private static WorldPoint getTrueWorldPoint(WorldPoint point) {
        LocalPoint localPoint = LocalPoint.fromWorld(CupidBot.getClient().getTopLevelWorldView(), point);
    ...
    private void walkToSafePoint() {
        LocalPoint localPoint = LocalPoint.fromWorld(CupidBot.getClient().getTopLevelWorldView(), workArea.safePoint);
        if (localPoint == null) {
            log("Safe point off-screen, using Rs2Walker");
            Rs2Walker.walkTo(workArea.safePoint);
            return;
        }
        if (Objects.equals(CupidBot.getClient().getLocalDestinationLocation(), localPoint))
            return;
        LocalPoint playerLocal = CupidBot.getClient().getLocalPlayer() != null
                ? CupidBot.getClient().getLocalPlayer().getLocalLocation() : null;
    ...
    private void walkToSpiritPool() {
        LocalPoint localPoint = LocalPoint.fromWorld(CupidBot.getClient().getTopLevelWorldView(), workArea.spiritPoolPoint);
        if (localPoint == null) {
    ...
    private boolean handleCloudDodge() {
        ...
        LocalPoint playerLocal = LocalPoint.fromWorld(CupidBot.getClient().getTopLevelWorldView(), playerLoc);
        if (playerLocal == null) {
            return false;
        }
    ...
    public static boolean inCloud(WorldPoint point, int radius) {
        ...
        LocalPoint playerLocal = LocalPoint.fromWorld(CupidBot.getClient().getTopLevelWorldView(), point);
        if (playerLocal == null) {
            return false;
        }
    ...
    public boolean fightFiresInPath(WorldPoint location) {
        ...
        LocalPoint playerLocal = CupidBot.getClient().getLocalPlayer() != null
                ? CupidBot.getClient().getLocalPlayer().getLocalLocation() : null;
        LocalPoint destLocal = LocalPoint.fromWorld(CupidBot.getClient().getTopLevelWorldView(), location);
