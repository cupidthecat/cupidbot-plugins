package net.runelite.client.plugins.cupidbot.woodcutting;
...
import net.runelite.client.plugins.cupidbot.util.inventory.Rs2LogBasket;
...
import net.runelite.client.plugins.cupidbot.util.skills.fletching.Rs2Fletching;
...
import net.runelite.client.plugins.cupidbot.woodcutting.enums.*;
import net.runelite.client.plugins.cupidbot.woodcutting.enums.WoodcuttingTreeLocations;
...
import static net.runelite.client.plugins.cupidbot.util.player.Rs2Player.getRealSkillLevel;
...
public class AutoWoodcuttingScript extends Script {
    ...
    private boolean preFlightChecks(AutoWoodcuttingConfig config) {
        if (!CupidBot.isLoggedIn()) return true;
        ...
        if (!hasAutoHopMessageShown && config.hopWhenPlayerDetected()) {
            CupidBot.showMessage("Make sure autologin plugin is enabled and randomWorld checkbox is checked!");
            hasAutoHopMessageShown = true;
        }

        if (config.hopWhenPlayerDetected() && config.enableForestry()) {
            CupidBot.showMessage("Autohop is not supported with forestry enabled, shutting down.");
            shutdown();
            return true;
        }
        ...
        if (!getActiveTree().hasRequiredLevel()) {
            CupidBot.showMessage("You do not have the required woodcutting level to cut this tree. " + Rs2Player.getRealSkillLevel(Skill.WOODCUTTING));
            shutdown();
            return true;
        }
        ...
        if (!Rs2Inventory.hasItem("axe")) {
            if (!Rs2Equipment.isWearing("axe")) {
                CupidBot.showMessage("Unable to find axe in inventory/equipped");
                shutdown();
                return true;
            }
        }
    ...
    private void burnLog(AutoWoodcuttingConfig config) {
        ...
        if (!isFiremake() && !useCampfire) {
            ...
            CupidBot.log("Sleeping until not animating or no more logs");
