...
import net.runelite.client.plugins.cupidbot.sailing.features.trials.data.*;
import net.runelite.client.plugins.cupidbot.sailing.features.trials.debug.BoatPathHelper;
...
public class TrialsScript {
    ...
    public void run(SailingConfig config) {
        try {
            ...
            CupidBot.getClientThread().invoke(() -> client.setHintArrow(hintTarget));
    ...
    private void navigateToWaypoint(WorldPoint target) {
        var boat = CupidBot.getClientThread().invoke(() -> boatCache.getLocalBoat());
        if (boat == null) {
    ...
    private void resetState() {
        ...
        activeRoute = null;
        lastVisitedIndex = -1;
        CupidBot.getClientThread().invoke(() -> client.clearHintArrow());
    }
