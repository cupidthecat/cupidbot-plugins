package net.runelite.client.plugins.cupidbot.bluedragons;
...
public class BlueDragonsScript extends Script {
    ...
    public boolean run(BlueDragonsConfig config) {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                consecutiveErrors[0]++;
                CupidBot.log("Error in Blue Dragons script: " + ex.getMessage());
                if (consecutiveErrors[0] >= 5) {
                    CupidBot.log("Too many consecutive errors. Stopping script.");
                    shutdown();
                }
            }
    ...
    private void determineStartingState() {
        boolean hasAgilityOrKey = CupidBot.getClient().getRealSkillLevel(Skill.AGILITY) >= 70 || hasDustyKey();
        if (!hasAgilityOrKey) {
            CupidBot.log("Requires Agility level 70 or a Dusty Key. Stopping.");
            shutdown();
            return;
        }

        if (!hasTeleportToFalador()) {
            CupidBot.log("Missing Falador teleport (tab or runes). Stopping.");
            shutdown();
            return;
        }
    ...
    private void handleBanking() {
        ...
        if (foodNeeded > 0) {
            if (!Rs2Bank.hasItem(config.foodType().getName())) {
                CupidBot.log("No " + config.foodType().getName() + " in bank. Stopping.");
                Rs2Bank.closeBank();
                shutdown();
                return;
            }
    ...
    private void handleCombat() {
        ...
        if (config.waitForLoot() && currentTargetId != null) {
            Rs2NpcModel target = CupidBot.getRs2NpcCache().query().withId(currentTargetId).nearestOnClientThread();
    ...
    private void lootItems() {
        ...
        if (config.lootMiscItems() && (!Rs2Inventory.isFull() || config.eatForLoot())) {
            ...
            CupidBot.log("[Loot] Value-based loot pass (threshold=" + config.lootValueThreshold()
    ...
    private void lootItem(String itemName) {
        ...
        if (Rs2GroundItem.lootItemsBasedOnNames(params)) {
            CupidBot.log("[Loot] Picked up: " + itemName);
    ...
    private Rs2NpcModel getAvailableDragon() {
        Rs2NpcModel dragon = CupidBot.getRs2NpcCache().query()
    ...
    private boolean isPlayerAtSafeSpot() {
        return SAFE_SPOT.equals(CupidBot.getClientThread().invoke(() ->
                CupidBot.getClient().getLocalPlayer().getWorldLocation()));
    }
    ...
    private void hopIfPlayerAtSafeSpot() {
        ...
        boolean occupied = players.stream().anyMatch(p ->
                p != null && !p.equals(CupidBot.getClient().getLocalPlayer())
                        && p.getWorldLocation().distanceTo(SAFE_SPOT) <= 1);

        if (occupied) {
            CupidBot.log("Player at safe spot. Hopping worlds.");
            CupidBot.hopToWorld(findRandomWorld());
            sleep(5000);
        }
    }
