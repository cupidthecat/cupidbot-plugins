package net.runelite.client.plugins.cupidbot.CalcifiedRockMiner;
...
import net.runelite.client.plugins.cupidbot.CalcifiedRockMiner.CalcifiedRockMinerConfig;
...
public class CalcifiedRockMinerScript extends Script {
    ...
    public boolean run(net.runelite.client.plugins.cupidbot.CalcifiedRockMiner.CalcifiedRockMinerConfig config) {
    ...
    private void handleMining(net.runelite.client.plugins.cupidbot.CalcifiedRockMiner.CalcifiedRockMinerConfig config) {
        ...
        if (config.focusCrackedWaterDeposits() && !Rs2Player.isMoving()) {
            Rs2TileObjectModel weepingRock = CupidBot.getRs2TileObjectCache().query()
                    .withId(WEEPING_ROCK)
                    .within(Rs2Player.getWorldLocation(), 15)
                    .first();
            if (weepingRock != null) {
                Rs2TileObjectModel tearRock = CupidBot.getClientThread().invoke(() ->
        ...
        Rs2TileObjectModel rock = CupidBot.getClientThread().invoke(() ->
                CupidBot.getRs2TileObjectCache().query()
        ...
        if (rock != null && shouldTryMiningAgain) {
            ...
            CupidBot.log("No calcified rock found. Waiting...");
        }
    }

    private boolean hopIfTooManyPlayersNearby(net.runelite.client.plugins.cupidbot.CalcifiedRockMiner.CalcifiedRockMinerConfig config) {
    ...
    private void handleCrushing(net.runelite.client.plugins.cupidbot.CalcifiedRockMiner.CalcifiedRockMinerConfig config) {
        if (config.crushDeposits() && hasHammer() && Rs2Inventory.hasItem(29088)) {
            if (Rs2Player.getWorldLocation().distanceTo(ANVIL) < 1) {
                ...
                Rs2TileObjectModel anvil = CupidBot.getClientThread().invoke(() ->
                        CupidBot.getRs2TileObjectCache().query()
                                .within(ANVIL, 3)
                                .withName("Anvil")
                                .nearestReachable());
    ...
    private void MoveCameraToRock(WorldPoint rock) {
        ...
        Rs2Camera.resetZoom();
        Rs2Camera.turnTo(LocalPoint.fromWorld(CupidBot.getClient().getTopLevelWorldView(), rock));
    }
