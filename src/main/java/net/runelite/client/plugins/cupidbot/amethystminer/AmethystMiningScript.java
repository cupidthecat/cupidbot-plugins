package net.runelite.client.plugins.cupidbot.amethystminer;
...
import net.runelite.client.plugins.cupidbot.amethystminer.enums.AmethystCraftingOption;
import net.runelite.client.plugins.cupidbot.amethystminer.enums.MiningSpot;
import net.runelite.client.plugins.cupidbot.amethystminer.enums.Status;
...
public class AmethystMiningScript extends Script {
    ...
    private void executeTask() {
        try {
            if (!super.run() || !CupidBot.isLoggedIn()) {
            ...
            if (pickAxeInInventory.isEmpty() && config.pickAxeInInventory()) {
                CupidBot.showMessage("Pickaxe was not found in your inventory");
                sleep(5000);
                return;
            }
            ...
            if (Rs2Player.isAnimating() || CupidBot.getClient().getLocalPlayer().isInteracting()) return;
            ...
            CupidBot.log("Error in AmethystMiningScript: " + e.getMessage());
        }
    }
    ...
    private void chiselAmethysts() {
        ...
        if (CupidBot.getClient().getRealSkillLevel(Skill.CRAFTING) >= requiredLevel ) {
            ...
            CupidBot.showMessage("You do not have the required crafting level to make " + craftingOption.getDisplayName());
            status = Status.BANKING;
        }
    }
    ...
    private int distanceToPlayer(WallObject wallObject) {
        ...
        return CupidBot.getClientThread().invoke(() -> CupidBot.getClient().getLocalPlayer().getWorldLocation().distanceTo2D(closestWalkableNeighbour));
    }
