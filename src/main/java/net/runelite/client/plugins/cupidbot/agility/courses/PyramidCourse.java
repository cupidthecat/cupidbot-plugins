...
import net.runelite.client.plugins.cupidbot.agility.courses.PyramidObstacleData.ObstacleArea;
...
public class PyramidCourse implements AgilityCourseHandler {
    ...
    public TileObject getCurrentObstacle() {
        ...
        if (shouldTurnIn) {
            if (pyramidCount > 0) {
                ...
                CupidBot.showMessage("Inventory is full but no pyramid tops found! Clear inventory to continue.");
    ...
    public boolean waitForCompletion(int agilityExp, int plane) {
        ...
        while (System.currentTimeMillis() - startTime < timeoutMs) {
            int currentXp = CupidBot.getClient().getSkillExperience(Skill.AGILITY);
            int currentPlane = CupidBot.getClient().getTopLevelWorldView() != null
    ...
    private boolean handlePyramidTurnIn() {
        try {
            ...
            // Try to find Simon
            Rs2NpcModel simon = CupidBot.getRs2NpcCache().query().withName(SIMON_NAME).nearestOnClientThread();
