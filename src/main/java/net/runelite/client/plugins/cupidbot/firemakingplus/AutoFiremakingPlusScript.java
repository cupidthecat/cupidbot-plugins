package net.runelite.client.plugins.cupidbot.firemakingplus;
...
public class AutoFiremakingPlusScript extends Script {
    ...
    public boolean run(AutoFiremakingPlusConfig config) {
        ...
        startSkillXp = CupidBot.getClientThread().runOnClientThreadOptional(() ->
                CupidBot.getClient().getSkillExperience(Skill.FIREMAKING)).orElse(0);
        startSkillLevel = CupidBot.getClientThread().runOnClientThreadOptional(() ->
                CupidBot.getClient().getRealSkillLevel(Skill.FIREMAKING)).orElse(1);
        actionsCompleted = 0;
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if (CupidBot.pauseAllScripts.get()) {
                    CupidBot.status = "[PAUSED]";
                    return;
                }

                if (config.stopAfterMinutes() > 0 && !shutdownAfterCleanup
                        && (System.currentTimeMillis() - startTimeMillis) / 60000 >= config.stopAfterMinutes()) {
                    CupidBot.log("AutoFiremakingPlus: reached stopAfterMinutes. Banking then shutting down.");
                    shutdownAfterCleanup = true;
                    state = State.BANKING;
                }
                if (config.stopAfterXp() > 0 && !shutdownAfterCleanup) {
                    int currentXp = CupidBot.getClientThread().runOnClientThreadOptional(() ->
                            CupidBot.getClient().getSkillExperience(Skill.FIREMAKING)).orElse(startSkillXp);
                    if (currentXp - startSkillXp >= config.stopAfterXp()) {
                        CupidBot.log("AutoFiremakingPlus: reached stopAfterXp. Banking then shutting down.");
                        shutdownAfterCleanup = true;
                        state = State.BANKING;
                    }
                }
                if (config.targetLevel() > 0 && !shutdownAfterCleanup) {
                    int currentLevel = CupidBot.getClientThread().runOnClientThreadOptional(() ->
                            CupidBot.getClient().getRealSkillLevel(Skill.FIREMAKING)).orElse(startSkillLevel);
                    if (currentLevel >= config.targetLevel()) {
                        CupidBot.log("AutoFiremakingPlus: reached targetLevel (" + currentLevel
                ...
                if (activeLog == null || !activeLog.hasRequiredLevel()) {
                    CupidBot.status = "Firemaking level too low for "
                ...
                CupidBot.logStackTrace("AutoFiremakingPlusScript", ex);
            }
    ...
    private void runCampfire(AutoFiremakingPlusConfig config) {
        ...
        if (Rs2Player.isMoving()) {
            CupidBot.status = "Returning to campfire";
            return;
        }
        ...
        if (target == null) {
            ...
            if (!Rs2Inventory.hasItem(TINDERBOX_NAME)) {
                CupidBot.status = "No tinderbox - banking for one";
                lastLogCount = -1;
                handleBanking(config);
                return;
            }
            CupidBot.status = "Lighting own fire";
            ...
            if (Rs2Player.waitForXpDrop(Skill.FIREMAKING, 5000)) {
                ...
                CupidBot.status = "Can't light here - repositioning";
        ...
        if (lastLogCount >= 0 && count < lastLogCount) {
            ...
            CupidBot.status = "Burning logs (" + count + " left)";
            return;
        }
        ...
        CupidBot.status = "Adding logs to campfire";
        Rs2Inventory.use(activeLog.getItemId());
        if (!sleepUntil(Rs2Inventory::isItemSelected, 2000)) {
            CupidBot.log("[Firemaking] log did not select; retrying next tick");
            return;
        }
        ...
        if (sleepUntil(() -> Rs2Widget.getWidget(BURN_INTERFACE_WIDGET) != null, 5000)) {
            ...
            CupidBot.log("[Firemaking] burn dialog did NOT open; retrying next tick");
        }
    }
    ...
    private Rs2TileObjectModel findCampfire(WorldPoint anchor, int radius) {
        return CupidBot.getClientThread().runOnClientThreadOptional(() -> {
            Rs2TileObjectModel t = CupidBot.getRs2TileObjectCache().query()
                    .withNameContains("ampfire")
                    .nearest(anchor, radius);
            if (t == null) {
                t = CupidBot.getRs2TileObjectCache().query()
    ...
    private void lineScan(AutoFiremakingPlusConfig config) {
        CupidBot.status = "Scanning for open space";
        ...
        if (line == null) {
            emptyScans++;
            if (emptyScans >= 5) {
                CupidBot.status = "No open line found - move me to open ground";
                emptyScans = 0;
                blockedLineY = Integer.MIN_VALUE; // reset so banking->return can retry fresh
                state = State.BANKING;
            } else {
                CupidBot.status = "No open space found - retrying";
            }
            return;
        }
        emptyScans = 0;
        currentLine = line;
        CupidBot.status = "Found line: " + line.getLength() + " tiles";
    ...
    private void lineWalkTo() {
        ...
        CupidBot.status = "Walking to line";
    ...
    private void lineBurn() {
        ...
        if (Rs2Player.isMoving()) {
            CupidBot.status = "Walking after lighting";
            return;
        }
        if (Rs2Player.isAnimating()) {
            CupidBot.status = "Lighting animation";
            return;
        }
        if (!Rs2Inventory.hasItem(TINDERBOX_NAME)) {
            state = State.BANKING;
        ...
        if (TileScanner.hasFire(playerPos)) {
            ...
            if (!Rs2Tile.isWalkable(westTile)) {
                ...
                CupidBot.status = "Blocked west - rescanning";
                state = State.SCANNING;
                return;
            }
        ...
        if (!Rs2Tile.isWalkable(playerPos)) {
            ...
            CupidBot.status = "Blocked tile - rescanning";
            state = State.SCANNING;
            return;
        }

        CupidBot.status = "Lighting " + activeLog.getLogName();
    ...
    private void handleBanking(AutoFiremakingPlusConfig config) {
        ...
        if (shutdownAfterCleanup) {
            Rs2Bank.closeBank();
            CupidBot.log("AutoFiremakingPlus: target reached, banked, shutting down.");
            super.shutdown();
            return;
        }
        ...
        if (needTinderbox && !Rs2Inventory.hasItem(TINDERBOX_NAME)) {
            if (!Rs2Bank.hasItem(TINDERBOX_ID)) {
                CupidBot.showMessage("No tinderbox in the bank!");
                super.shutdown();
                return;
            }
        ...
        if (!Rs2Bank.hasItem(activeLog.getItemId())) {
            CupidBot.showMessage("No " + activeLog.getLogName() + " in the bank!");
            super.shutdown();
            return;
        }
        CupidBot.status = "Withdrawing logs";
