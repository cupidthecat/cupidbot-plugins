package net.runelite.client.plugins.cupidbot.revkiller;
...
public class revKillerScript extends Script {
    ...
    public boolean run(revKillerConfig config) {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if(firstRun || weDied) {
                    CupidBot.log("It's our first run or we died!");
                    if(firstRun && CupidBot.getRs2NpcCache().query().withName(config.selectedRev().getName()).nearestOnClientThread() != null){
                        // we're all ready geared and there
                        firstRun = false;
                        CupidBot.log("It's our first run and we're all ready here!");
                        return;
                    }
                ...
                if(!areWeEquipped()||isItTimeToGo()){
                    if(WeAreInTheCaves()){
                        if(goodLootOnGround()){
                            if(areWeEquipped()){
                                if(!isPkerAround()){
                                    CupidBot.log("Returning, there's still loot on the ground");
                                    return;
                                }
                            }
                        }
    ...
    public boolean playerCheck(){
        ...
        List<net.runelite.client.plugins.cupidbot.api.player.models.Rs2PlayerModel> nearbyPlayers =
                CupidBot.getRs2PlayerCache()
                        .query()
        ...
        if(!nearbyPlayers.isEmpty()){
            if(!Rs2Player.isInCombat()) {
                CupidBot.log("There's another player here hopping.");
                hopToNewWorld();
            }
            return true;
        }
    ...
    public boolean weAreInCombat(){
        ...
        if(!Rs2Player.isAnimating()){
            CupidBot.log("Checking if we're in combat");
    ...
    public boolean goodLootOnGround(){
        ...
        if(CupidBot.getRs2TileItemCache().query().where(it->it.getWorldLocation().distanceTo(Rs2Player.getWorldLocation()) < 10 && it.getTotalGeValue() > 500).nearest() != null) return true;
        return false;
    }

    public Rs2NpcModel revenentKnight() {
        return CupidBot.getRs2NpcCache().query().withId(7939).nearest();
    }
    ...
    public void kiteTheKnight(){
        ...
        if(Rs2Player.getWorldLocation().equals(fifthTile)){
            if(weAreInCombat()) {
                CupidBot.log("We've all ready jammed the knight");
                return;
            } else {
                CupidBot.log("We need to click the rev.");
                if(revenentKnight()!=null && revenentKnight().getWorldLocation().distanceTo(jammedTile)<=2) {
                    if (revenentKnight().click("Attack")) {
                        CupidBot.log("We attacked the knight");
                        return;
                    }
                } else {
                    CupidBot.log("We need to re-jam the knight");
                }
            }
        } else {
                ...
                if(!revenentKnight().getWorldLocation().equals(thirdTile)){
                    ...
                    while(!revenentKnight().getWorldLocation().equals(thirdTile)){
                        ...
                        if(revenentKnight().getWorldLocation().distanceTo(Rs2Player.getWorldLocation())<=1 && !revenentKnight().getWorldLocation().equals(thirdTile)){
                            CupidBot.log("Rev is on a bad tile breaking loop");
                            return;
                        }
                        if(!Rs2Player.getWorldLocation().equals(secondTile)){
                            moveCameraToTile(secondTile);
        ...
        if(Rs2Player.getWorldLocation().equals(fifthTile)) {
            if (revenentKnight().click("Attack")) {
                CupidBot.log("We attacked the knight");
                CupidBot.log("Rev should be locked");
            }
        }
    ...
    public void moveCameraToTile(WorldPoint wp){
        LocalPoint lp = LocalPoint.fromWorld(CupidBot.getClient().getWorldView(CupidBot.getClient().getTopLevelWorldView().getId()), wp);
        Rectangle chatBox = null;
        Point canvasPoint = null;

        if(lp != null) {
            canvasPoint = Perspective.localToCanvas(CupidBot.getClient(), lp, wp.getPlane());
            if(canvasPoint != null) {
    ...
    public void WalkToRevs(){
        if(!WeAreInTheCaves()){
            ...
            if(Rs2Player.getWorldLocation().distanceTo(cave) > 6){
                ...
                if(selectedRev.contains("knight")){
                    ...
                    if(Rs2Walker.walkTo(selectedWP)){
                        CupidBot.log("Teleporting to level 40 rev cave.");
                    }
                } else {
                    if(Rs2Walker.walkTo(cave)){
                        CupidBot.log("Walking to cave. with new method.");
                    }
                }
            } else {
                if(!Rs2Dialogue.isInDialogue()){
                    CupidBot.log("At the cave, clicking.");
                    if(CupidBot.getRs2TileObjectCache().query().interact(31555, "Enter")){
            ...
            if(WeAreInTheCaves()){
                if(Rs2Walker.walkTo(selectedWP)){
                    CupidBot.log("Walking to Revs. with new method.");
                }
            }
        }
    }
    ...
    public void hopWorldsBasedOnTimer(){
        if(useTimedWorldHopper) {
            ...
            if (currentTime - startTime >= howLongUntilHop * 60 * 1000) {
                CupidBot.log("Hopping to a new world, it's been "+howLongUntilHop+" minutes");
                if(Rs2Player.isInCombat()){
                ...
                CupidBot.log("We'll hop to a new world in "+howLongUntilHop+" minutes");
            }
        }
    }
    public void getAwayFromPkerKnight(){
        CupidBot.log("Attempting to logout");
        Rs2Walker.setTarget(null);
        int io = 0;
        int tries = 10;
        if (CupidBot.isLoggedIn()) {
            while (CupidBot.isLoggedIn()) {
                ...
                Rs2Player.logout();
                sleepUntil(() -> !CupidBot.isLoggedIn() || Rs2Player.isInCombat(), Rs2Random.between(1500, 3000));
                if(Rs2Player.isInCombat() && CupidBot.isLoggedIn() || io>=tries){
                    CupidBot.log("We can't log out, running away instead.");
                    break;
                }
                io++;
            }
        ...
        if (isPkerAround() && CupidBot.isLoggedIn()) {
            getAwayFromPker();
            shouldFlee = false;
            return;
        }
        ...
        if (!CupidBot.isLoggedIn()) {
            while (!CupidBot.isLoggedIn()) {
                ...
                if (!CupidBot.isLoggedIn()) {
                    ...
                    sleepUntil(() -> CupidBot.isLoggedIn(), Rs2Random.between(10000, 20000));
                }
            }
            shouldFlee = false;
        }
    ...
    public void getAwayFromPker(){
        ...
        CupidBot.log("Attemping to get away from the PKer.");
        //kill the walker incase we were walking.
        Rs2Walker.setTarget(null);

        if(!Rs2Player.isTeleBlocked()){
            CupidBot.log("At least we're not teleblocked.");
            enablePrayer();
            ...
            if(Rs2Pvp.getWildernessLevelFrom(Rs2Player.getWorldLocation()) >= 20 && Rs2Pvp.getWildernessLevelFrom(Rs2Player.getWorldLocation()) <= 30) {
                while (Rs2Pvp.getWildernessLevelFrom(Rs2Player.getWorldLocation()) >= 20 && Rs2Pvp.getWildernessLevelFrom(Rs2Player.getWorldLocation()) <= 30) {
                    CupidBot.log("Attempting to teleport via glory");
                    if (!super.isRunning()) break;
                    if(!shouldFlee) break;
                    if(weDied){
                        shouldFlee = false;
            ...
            if(Rs2Pvp.getWildernessLevelFrom(Rs2Player.getWorldLocation()) <= 20) {
                while (Rs2Pvp.getWildernessLevelFrom(Rs2Player.getWorldLocation()) <= 20) {
                    CupidBot.log("Attempting to teleport via dueling");
                    if (!super.isRunning()) break;
                    if(weDied){
                        shouldFlee = false;
                        return;
            ...
            CupidBot.log("We're teleblocked! Attempting to run to the bank");
            enablePrayer();
    ...
    public void fightrev(revKillerConfig config){
        Rs2NpcModel Rev = CupidBot.getRs2NpcCache().query().withName(config.selectedRev().getName()).nearest();
        ...
        if(Rev!=null){
            ...
            if(Rev.isInteracting()) {
                if(hoppedWorld) {
                    CupidBot.log("Rev is attacking us attacking back.");
                    if (Rev.click("Attack")) {
                        hoppedWorld=false;
                        sleepUntil(() -> Rev.isDead() || !Rs2Player.isInCombat() || isItTimeToGo() || Rs2Player.getHealthPercentage() <= generateRandomNumber(70, 80), generateRandomNumber(60000, 120000));
                    }
            ...
            if(!Rs2Player.isInCombat()) {
                CupidBot.log("No revs found, hopping");
    ...
    public void startHealthCheck() {
        CupidBot.log("Health detection started");
    ...
    public void startPkerDetection() {
        CupidBot.log("PKer detection started");
    ...
    public void futurePKCheck(){
        try {
            ...
            CupidBot.log("Error during PK check: " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    public void reJfount(){
        ...
        while(Rs2Player.getBoostedSkillLevel(Skill.PRAYER) < rejat || Rs2Player.getRunEnergy() <= runener){
            ...
            if(Rs2Bank.isOpen()){
                ...
                Rs2TileObjectModel rej = CupidBot.getRs2TileObjectCache().query().withId(39651).nearest();

                if(rej == null){
                    return;
                }
                CupidBot.log("Drinking");
                if(rej.click("Drink")){
                    sleepUntil(()-> Rs2Player.isMoving(), Rs2Random.between(1000,3000));
    ...
    public void hopToNewWorld(){
        ...
        World currentWorld = CupidBot.getWorldService().getWorlds().findWorld(CupidBot.getClient().getWorld());

        if(currentWorld == null){
            CupidBot.log("Couldn't find our world");
            return;
        }
        ...
        if(newRandomWorld == 0){
            CupidBot.log("Couldn't find a new random world");
            return;
        }

        CupidBot.log("We're going to world " + newRandomWorld);
        int attempts = 0;
        ...
        while(Rs2Player.getWorld() != newRandomWorld){
            ...
            if(CupidBot.hopToWorld(newRandomWorld)){
                sleepUntil(() -> !CupidBot.isHopping() || isPkerAround() || Rs2Player.getWorld() == newRandomWorld, generateRandomNumber(5000, 10000));
            }
    ...
    public void drinkPotion(){
        if(!Rs2Player.hasRangingPotionActive(potthresh)){
            CupidBot.log("Drinking ranging potion");
    ...
    public void drinkStamPotion(){
        if(!Rs2Player.hasStaminaActive()){
            if(Rs2Player.getRunEnergy() <= generateRandomNumber(20,40)) {
                CupidBot.log("Drinking stamina potion");
    ...
    public void loot(){
        List<Rs2TileItemModel> itemsOnGround = CupidBot.getRs2TileItemCache().query().where(it -> it.getWorldLocation().distanceTo(Rs2Player.getWorldLocation()) < 12 && !it.getName().contains("arrow") && it.getTotalGeValue() > 500).toList();
        List<Rs2TileItemModel> BlightedItemsOnGround = CupidBot.getRs2TileItemCache().query().where(it -> it.getWorldLocation().distanceTo(Rs2Player.getWorldLocation()) < 12
        ...
        if(!itemsOnGround.isEmpty()){
            for (Rs2TileItemModel theItem : itemsOnGround) {
                ...
                if(!Rs2Inventory.isFull()){
                    if(CupidBot.getRs2TileItemCache().query().withId(theItem.getId()).interact("Take")){
    ...
    public void Bankfortrip(){
        ...
        if(!Rs2Bank.isOpen()){
            ...
            if(!WeAreInTheCaves()) {
                CupidBot.log("Walking and using bank");
                OpenTheInv();
                stopTeleSpam();
            ...
            CupidBot.log("Random number: " + howtobank);
            //equipring
            if(howtobank <= 80){
                if(isItTimeToGo() || weHaveLoot()){
                    CupidBot.log("We have loot, depositing all");
                    Rs2Bank.depositAll();
            ...
            if(howtobank <= 40){
                if(Rs2Equipment.get(EquipmentInventorySlot.AMULET).getName().equals("Amulet of glory")){
                    CupidBot.log("Getting a fresh amulet of glory");
                    if(Rs2Bank.get(it->it!=null&&it.getName().contains("Amulet of glory(")) != null){
                        ...
                        CupidBot.log("Out of amulets of glory 6");
                        super.shutdown();
                    }
                    if(Rs2Inventory.contains(it->it!=null&&it.getName().contains("Amulet of glory("))){
            ...
            if(howtobank <= 40){
                if(Rs2Equipment.get(EquipmentInventorySlot.RING)!=null){
                    ...
                    CupidBot.log("Getting the ring of dueling");
                    if(Rs2Bank.count(ItemID.RING_OF_DUELING8)>0){
                        ...
                        CupidBot.log("Out of rings of dueling");
                        super.shutdown();
                    }
                    if(Rs2Inventory.contains(ItemID.RING_OF_DUELING8)){
                ...
                if(Rs2Equipment.get(EquipmentInventorySlot.GLOVES)!=null){
                    if(Rs2Equipment.get(EquipmentInventorySlot.GLOVES).getName().contains("ethereum")){
                        if(Rs2Equipment.get(EquipmentInventorySlot.GLOVES).getId() == ItemID.BRACELET_OF_ETHEREUM_UNCHARGED){
                            CupidBot.log("We need to charge our bracelet");
                            if(Rs2Bank.hasItem("Revenant ether") && Rs2Bank.count("Revenant ether") > 100){
                                ...
                                CupidBot.log("We're out of ether. Stopping.");
                                super.shutdown();
                                return;
                            }
                            if(Rs2Inventory.contains("Revenant ether")){
                        ...
                        if(!Rs2Inventory.contains(selectedArrow)){
                            ...
                            if(selectedArrow == ItemID.BOLT_RACK){
                                ...
                                if(Rs2Equipment.get(EquipmentInventorySlot.WEAPON).getId() == ItemID.KARILS_CROSSBOW_25){
                                    CupidBot.log("Please get a fresh karil's crossbow. Shutting down.");
                                    super.shutdown();
                                    return;
                                }
                            }
            ...
            if(selectedRev.contains("knight")) {
                if (howtobank <= 80) {
                    CupidBot.log("Grabbing rev cave teles");
                    if (!Rs2Inventory.contains("Revenant cave teleport")) {
                        if (Rs2Bank.count("Revenant cave teleport") > 5) {
                            ...
                            CupidBot.log("Rev cave teleports, need at least 5");
                            super.shutdown();
                            return;
                        }
                    }
            ...
            //get stamina pot
            if(howtobank <= 40){
                CupidBot.log("Withdrawing Stamina potion");
                if(!Rs2Inventory.contains(it->it!=null&&it.getName().contains("Stamina"))){
                    if(Rs2Bank.count("Stamina potion(4)") > 0 || Rs2Bank.count("Stamina potion(3)") > 0 || Rs2Bank.count("Stamina potion(2)") > 0){
                        ...
                        CupidBot.log("Out of stamina potions");
                        super.shutdown();
                        return;
                    }
                }
            ...
            if(howtobank <= 40){
                CupidBot.log("Withdrawing Ranging potion");
                if(!Rs2Inventory.contains(it->it!=null&&it.getName().contains("Ranging"))){
                    if(Rs2Bank.getBankItem("Ranging potion(4)") != null || Rs2Bank.getBankItem("Ranging potion(3)") != null){
                        ...
                        CupidBot.log("Out of ranging potions");
                        super.shutdown();
                        return;
                    }
                }
            ...
            if(howtobank <= 40){
                CupidBot.log("Withdrawing Sharks");
    ...
    public boolean isItTimeToGo(){
        ...
        if(value>=config.leaveAtValue()){
            CupidBot.log("We have enough loot");
            return true;
        }
        if(Rs2Inventory.isFull()){
            CupidBot.log("We have enough loot");
        ...
        if(timeToBreak()){
            CupidBot.log("It's time to break");
            return true;
        }
        CupidBot.log("We have "+value+" worth of loot");
        return false;
    }
    ...
    if (Rs2Equipment.get(EquipmentInventorySlot.AMMO) == null) {
        CupidBot.log("We have no ammo!");
        return false;
    }

    if (Rs2Equipment.get(EquipmentInventorySlot.AMMO).getQuantity() < LowOnArrowsCount) {
        CupidBot.log("We don't have enough ammo!");
        return false;
    }

    if (!Rs2Inventory.contains(it -> it != null && it.getName().contains("Stamina"))) {
        CupidBot.log("We have no stam!");
        return false;
    }

    if (!Rs2Inventory.contains(it -> it != null && it.getName().contains("Ranging"))) {
        CupidBot.log("We have no Ranging potion!");
        return false;
    }

    if (!Rs2Inventory.contains(ItemID.SHARK)) {
        CupidBot.log("We're out of sharks!");
        return false;
    }

    if (Rs2Inventory.count(ItemID.SHARK) < 2) {
        CupidBot.log("We have less than 2 sharks!");
        return false;
    }

    if (Rs2Equipment.get(EquipmentInventorySlot.RING) == null) {
        CupidBot.log("ring is null");
        return false;
    }

    if (!Rs2Equipment.get(EquipmentInventorySlot.RING).getName().contains("dueling")) {
        CupidBot.log("We don't have our ring of dueling");
        return false;
    }
    ...
    if(Rs2Equipment.get(EquipmentInventorySlot.GLOVES)!=null) {
        if (Rs2Equipment.get(EquipmentInventorySlot.GLOVES).getName().contains("ethereum")) {
            if (Rs2Equipment.get(EquipmentInventorySlot.GLOVES).getId() == ItemID.BRACELET_OF_ETHEREUM_UNCHARGED) {
                CupidBot.log("Our bracelet doesn't have enough charges.");
                return false;
            }
        }
    }

    CupidBot.log("We're fully equipped and ready to go.");
    return true;
    ...
    public boolean isPkerAround(){
        ...
        for (Rs2PlayerModel player : playerlist) {
            CupidBot.log("There may be a pker around us "+player.getName());
            ...
            CupidBot.log("They have a "+NameOfPlayersWeapon);
            CupidBot.log("They're wearing a' "+nameOfPlayersChestPiece);

            for (String weapon : weapons) {
                if (lowercaseWeapon.contains(weapon) || lowercaseWeapon.equals(weapon)) {
                    CupidBot.log("This player is wielding a " + NameOfPlayersWeapon + " which is used to pk so we're outy.");
                    return true;
                }
            }
            ...
            for (String chestArmor : chestArmors) {
                if (lowercaseChestPiece.contains(chestArmor) || lowercaseChestPiece.equals(chestArmor)) {
                    CupidBot.log("This player is wielding a " + nameOfPlayersChestPiece + " which is used to pk so we're outy.");
                    return true;
                }
            }
            ...
            if ((player.getInteracting() != null) && (player.getInteracting().equals(Rs2Player.getLocalPlayer()))) {
                CupidBot.log(player.getName() + " is attacking us");
                return true;
            }
