package net.runelite.client.plugins.cupidbot.playermonitor;
...
public class PlayerMonitorScript extends Script {
    ...
    public boolean run(PlayerMonitorConfig config, OverlayManager overlayManager) {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            ...
            try {
                if (!config.liteMode()) {
                    if (CupidBot.getClient().getGameCycle() % 20 >= 10) {
                        if (!playAlarm) {
                            if (plugin.isOverlayOn()) {
                                if (config.playAlarmSound()) {
                                    CupidBot.getClientThread().invokeLater(() -> CupidBot.getClient().playSoundEffect(config.alarmSoundID().getId(), 127));
                                }
                                playAlarm = true;
                                if (config.useFlash()) {
                                    flashOverlay.setFlashColor(config.flashColor());
                                }
                                if ((config.useFlash() || config.playAlarmSound()) && config.useEmergency() && !newPlayer && (config.onlyWilderness() && CupidBot.getVarbitValue(Varbits.IN_WILDERNESS) == 1) || !config.onlyWilderness()) {
                                    newPlayer = true;
                                    ...
                                    CupidBot.getClientThread().runOnSeperateThread(() -> {
                                        switch (config.emergencyAction()) {
                                            ...
                                            case HOP_WORLDS:
                                                ...
                                                if (this.isRunning()) {
                                                    CupidBot.hopToWorld(Login.getRandomWorld(Rs2Player.isMember()));
                                                }
                                                if (this.isRunning()) {
                                                    sleep(61, 93);
                                                }
                            ...
                            if (PlayerMonitorPlugin.mouseAlarm && config.mouseAlarm()) {
                                CupidBot.getClientThread().invokeLater(() -> CupidBot.getClient().playSoundEffect(config.mouseAlarmSound().getId(), 127));
                            }
                        }
                    } else {
                        if (playAlarm) {
                ...
                if (config.liteMode()) {
                    if (plugin.isPlayerDetected() && !logoutInitiated) {
                        ...
                        CupidBot.log("PlayerMonitorLite: Player detected - logging out");
