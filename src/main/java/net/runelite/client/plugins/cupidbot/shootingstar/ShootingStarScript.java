package net.runelite.client.plugins.cupidbot.shootingstar;
...
import net.runelite.client.plugins.cupidbot.shootingstar.ShootingStarConfig;
import net.runelite.client.plugins.cupidbot.shootingstar.ShootingStarPlugin;
import net.runelite.client.plugins.cupidbot.shootingstar.enums.Pickaxe;
import net.runelite.client.plugins.cupidbot.shootingstar.enums.ShootingStarState;
import net.runelite.client.plugins.cupidbot.shootingstar.model.Star;
...
public class ShootingStarScript extends Script
{
			...
			try
			{
				...
				switch (state)
				{
					case WAITING_FOR_STAR:
						if (config.useNearestHighTierStar())
						{
							if (!hasSelectedStar())
							{
								...
								if (currentStar == null)
								{
									CupidBot.showMessage("Unable to find a star within your tier range. Consider disabling useNearestHighTierStar until higher mining level.");
									CupidBot.stopPlugin(plugin);
									return;
								}
							}
					...
					case WALKING:
						...
						{
							CupidBot.hopToWorld(currentStar.getWorld());
					...
					case MINING:
						...
						{
							CupidBot.status = "Waiting for star to degrade";
						...
						var starObject = CupidBot.getRs2TileObjectCache().query().withId(currentStar.getObjectId()).nearest();
					...
					case BANKING:
						...
						else
						{
							if (pickaxe == null)
							{
								...
								{
									CupidBot.showMessage("Unable to find pickaxe, please purchase a pickaxe");
									CupidBot.stopPlugin(plugin);
									return;
								}
							}
	...
	{
		boolean isInventorySetupPluginEnabled = CupidBot.isPluginEnabled(MInventorySetupsPlugin.class);
	...
	private ShootingStarState updateStarState()
	{
		...
		if (state == ShootingStarState.MINING)
		{
			var starModel = CupidBot.getRs2TileObjectCache().query()
					.withIds(
