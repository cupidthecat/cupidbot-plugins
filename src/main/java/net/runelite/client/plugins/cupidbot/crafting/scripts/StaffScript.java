...
import net.runelite.client.plugins.cupidbot.crafting.enums.Staffs;
...
public class StaffScript extends Script implements ICraftingScript {
    ...
    private void verifyItemInBank(String item) {
        if (Rs2Bank.isOpen() && !Rs2Bank.hasItem(item)) {
            Rs2Bank.closeBank();
            CupidBot.status = "[Shutting down] - Reason: " + item + " not found in the bank.";
            CupidBot.getNotifier().notify(CupidBot.status);
            shutdown();
        }
    }
    ...
    public ProgressiveStaffmakingModel calculateItemToCraft() {
        int craftingLvl = CupidBot.getClient().getRealSkillLevel(Skill.CRAFTING);
