package net.runelite.client.plugins.cupidbot.lunartanner;
...
public class TanLeatherScript extends Script {
    ...
    public boolean run(TanLeatherConfig config) {
        ...
        int unprocessedItemPrice = CupidBot.getItemManager().search(config.ITEM().getName()).get(0).getPrice();
        int processedItemPrice = CupidBot.getItemManager().search(config.ITEM().getFinished()).get(0).getPrice();
    ...
    private void bank(TanLeatherConfig config) {
        if (currentState != State.BANKING) {
            ...
            if (Rs2Bank.hasItem(config.ITEM().getName())) {
                ...
                CupidBot.showMessage("No more " + config.ITEM().getName() + " to tan.");
                shutdown();
                return;
            }
