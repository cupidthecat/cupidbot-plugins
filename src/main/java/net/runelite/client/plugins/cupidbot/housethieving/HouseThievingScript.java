package net.runelite.client.plugins.cupidbot.housethieving;
...
import static net.runelite.client.plugins.cupidbot.housethieving.HouseThievingConstants.*;
...
public class HouseThievingScript extends Script {
            ...
            try {
                ...
                switch (state) {
                    ...
                    case THIEVING_HOUSES:
                        var houseNpc = CupidBot.getRs2NpcCache().query().withName(currentThievingHouse.npcName).nearestOnClientThread();
                ...
                CupidBot.log("Error: " + ex.getMessage(), Level.ERROR);
                ex.printStackTrace();
            }
    ...
    private void handlePickPocketing(HouseThievingConfig config) {
        ...
        if (hasMaxHouseKeys(config) && (hasFood || hasDodgyNecklaceInv)) {
            CupidBot.log("Have enough house keys, depositing inventory at bank", Level.INFO);
            state = State.BANKING;
            return;
        } else if (hasMaxHouseKeys(config) && !hasFood && !hasDodgyNecklaceInv) {
            CupidBot.log("Have enough house keys, thieving houses", Level.INFO);
            ...
            CupidBot.log("Need food or dodgy necklace to keep pickpocketing, withdrawing from bank", Level.INFO);
            state = State.BANKING;
            return;
        }
        ...
        if (Rs2Player.getWorldLocation().distanceTo(PICKPOCKET_LOCATION) > 8) {
            CupidBot.log("Walking to pickpocket location", Level.INFO);
        ...
        var aureliaNpc = CupidBot.getRs2NpcCache().query().withName("Aurelia").nearestOnClientThread();
        ...
        if (aureliaNpc != null) {
            ...
            if (aureliaAnim == 866 || aureliaAnim == 860) {
                ...
                if (pickpocketNpc == null) {
                    var nearbyWealthyCitizens = CupidBot.getRs2NpcCache().query().withName(WEALTHY_CITIZEN).toListOnClientThread().stream();
            ...
            if (config.worldHopForDistractedCitizens()) {
                CupidBot.log(String.format("Waiting %s seconds for Aurelia pickpocket", config.worldHopWaitTime()), Level.INFO);
                var waitForPickpocket = sleepUntil(() -> aureliaNpc.getAnimation() == 866 || aureliaNpc.getAnimation() == 860, config.worldHopWaitTime() * 1000);
                CupidBot.log(String.format("Finished waiting %s seconds for Aurelia pickpocket", config.worldHopWaitTime()), Level.INFO);
    ...
    private void doWorldHop() {
        CupidBot.log("World hopping to check for distracted citizen event", Level.INFO);
        boolean hoppedWorlds = CupidBot.hopToWorld(Login.getRandomWorld(Rs2Player.isMember()));
        if (hoppedWorlds) {
    ...
    private void handleFindingHouse(HouseThievingConfig config) {
        ...
        if (lockedDoorTile != null) {
            ...
            if (wallObject != null && wallObject.getId() == LOCKED_DOOR_ID) {
                CupidBot.log(currentThievingHouse.npcName + " house can be thieved", Level.INFO);
                attemptWaitForHouseNpc(houseNpc);

                if (!Rs2Tile.isTileReachable(currentThievingHouse.houseCenter)) {
                    CupidBot.getRs2TileObjectCache().query().withId(wallObject.getId()).interact();
                    ...
                    CupidBot.log("Entered " + currentThievingHouse.npcName + " house", Level.INFO);
                }
                if (Rs2Tile.isTileReachable(currentThievingHouse.houseCenter)) {
                    ...
                    CupidBot.log("Starting thieving in " + currentThievingHouse.npcName + " house.", Level.INFO);
                    return;
                }
                CupidBot.log("Failed to enter " + currentThievingHouse.npcName + " house, trying again..", Level.INFO);
            } else {
                Rs2Random.waitEx(3000.0, 100.0);
                setNextThievingHouse();
                CupidBot.log("Checking " + currentThievingHouse.npcName + " house for thieving..", Level.INFO);
            }
        }
    }
    ...
    private void attemptWaitForHouseNpc(Rs2NpcModel houseNpc) {
        ...
        if (houseNpc != null) {
            ...
            if (overheadText != null && !overheadText.isEmpty()) {
                CupidBot.log("House NPC is talking about leaving, waiting for them to leave..", Level.INFO);
        ...
        CupidBot.log("Distance from house NPC to Door: " + distanceToHouseNpc, Level.INFO);
        if (distanceToHouseNpc < 6) {
            CupidBot.log("House NPC is too close to locked door, waiting for them to leave..", Level.INFO);
    ...
    private void handleThievingHouse(Rs2NpcModel houseNpc) {
        ...
        if (houseNpc != null) {
            ...
            if (laviniaWorldLoc.distanceTo(currentThievingHouse.lockedDoorEntrance) < 5) {
                CupidBot.log("Owner in house or approaching currently", Level.INFO);
                if (exitHouse())
                    return;
            }
        }
        ...
        var hintArrow = CupidBot.getClientThread().runOnClientThreadOptional(() -> CupidBot.getClient().getHintArrowPoint()).orElse(null);
        if (hintArrow != null && (lastSearchedValuables == null || lastSearchedValuables.distanceTo(hintArrow) > 2)) {
            ...
            var bonusPiece = CupidBot.getRs2TileObjectCache().query()
        ...
        var valuables = CupidBot.getRs2TileObjectCache().query()
                .withIds(VALUABLES_OBJECT_IDS)
    ...
    private boolean exitHouse() {
        ...
        if (windowTile != null) {
            ...
            if (windowTileWallObject != null) {
                ...
                CupidBot.getRs2TileObjectCache().query().withId(windowTileWallObject.getId()).interact("Exit-window");
                Rs2Random.waitEx(5000.0, 200.0);
                searchingPiece = false;
                setNextThievingHouse();
                state = State.FINDING_HOUSE;
    ...
    private void handleBanking(HouseThievingConfig config) {
        ...
        if (!Rs2Bank.isOpen() && Rs2Player.distanceTo(BANKING_LOCATION) <= 3) {
            ...
            var bankObject = CupidBot.getRs2TileObjectCache().query()
            ...
            if (!hasMaxHouseKeys(config)) {
                if (!hasFood && !config.worldHopForDistractedCitizens() && !config.onlyPickpocketDistracted()) {
                    if (!Rs2Bank.hasItem(config.foodSelection().getId())) {
                        CupidBot.showMessage("No food found in bank!");
                        shutdown();
                        return;
                    }
                ...
                if (config.useDodgyNecklace() && !config.worldHopForDistractedCitizens() && !config.onlyPickpocketDistracted()) {
                    if (!Rs2Bank.hasItem(DODGY_NECKLACE)) {
                        CupidBot.showMessage("No dodgy necklace found in bank!");
                        shutdown();
                        return;
                    }
