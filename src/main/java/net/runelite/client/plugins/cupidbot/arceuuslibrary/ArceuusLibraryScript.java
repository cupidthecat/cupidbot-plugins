package net.runelite.client.plugins.cupidbot.arceuuslibrary;
...
import net.runelite.client.plugins.cupidbot.arceuuslibrary.KourendLibraryBridge.BookSnapshot;
import net.runelite.client.plugins.cupidbot.arceuuslibrary.KourendLibraryBridge.BookcaseSnapshot;
import net.runelite.client.plugins.cupidbot.arceuuslibrary.KourendLibraryBridge.Solved;
...
public class ArceuusLibraryScript extends Script
{
    ...
    private void tick()
    {
        ...
        {
            CupidBot.log("Arceuus Library tick error: " + e.getMessage());
    ...
    private void handleDeliver(BookSnapshot wanted)
    {
        ...
        Rs2NpcModel customer = activeId > 0
                ? CupidBot.getRs2NpcCache().query().withId(activeId).nearestOnClientThread()
    ...
    private Rs2NpcModel findCustomer(int minDist, boolean excludeLastDelivered, boolean excludeRecentlyAttempted)
    {
        ...
        int bestDist = Integer.MAX_VALUE;
        for (Rs2NpcModel npc : CupidBot.getRs2NpcCache().query()
    ...
    private void ensureInsideLibrary()
    {
        ...
        CupidBot.log("Player not in Arceuus Library (region " + here.getRegionID() + ") — please position manually");
    }
    ...
    private void restrictPathfinderToLocal()
    {
        ...
        CupidBot.getEventBus().post(new PluginMessage("shortestpath", "path", data));
        log.info("Pathfinder restricted to local routes (no teleports/transports)");
    }

    private void clearPathfinderRestrictions()
    {
        CupidBot.getEventBus().post(new PluginMessage("shortestpath", "clear"));
