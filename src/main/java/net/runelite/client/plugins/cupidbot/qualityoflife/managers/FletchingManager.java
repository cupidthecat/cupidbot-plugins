package net.runelite.client.plugins.cupidbot.qualityoflife.managers;
...
import net.runelite.client.plugins.cupidbot.qualityoflife.enums.FletchingArrow;
import net.runelite.client.plugins.cupidbot.qualityoflife.enums.FletchingBolt;
import net.runelite.client.plugins.cupidbot.qualityoflife.enums.FletchingDarts;
import net.runelite.client.plugins.cupidbot.qualityoflife.enums.FletchingLogs;
...
public class FletchingManager
{
    ...
    private void fletchBrumaRootsOnClick(MenuEntry event)
    {
        ...
        if (brumaRootSlot == -1)
        {
            CupidBot.log("<col=5F1515>Bruma root not found in inventory</col>");
            return;
        }
        CupidBot.log("<col=245C2D>Fletching Kindling</col>");
        NewMenuEntry combined = createWidgetOnWidgetEntry("Fletch", "Bruma root", brumaRootSlot, event.getParam1(), ItemID.WINT_BRUMA_ROOT);
        CupidBot.doInvoke(combined, new Rectangle(1, 1));
    }
    ...
    private void quickFletchLogsOnClick(MenuEntry event)
    {
        ...
        {
            CupidBot.log("<col=5F1515>No fletchable logs found in inventory</col>");
            return;
        }
        Rs2ItemModel logToFletch = fletchableLogs.get(0);
        if (logToFletch.getSlot() == -1)
        {
            CupidBot.log("<col=5F1515>Couldn't get item slot</col>");
            return;
        }

        CupidBot.log("<col=245C2D>Fletching: " + logToFletch.getName() + " To: " + config.fletchingItem().getName() + "</col>");
    ...
    private void quickFletchHeadlessArrowOnClick(MenuEntry event)
    {
        ...
        if (featherSlot == -1) {
            CupidBot.log("<col=5F1515>Feather not found in inventory</col>");
            return;
        }

        CupidBot.log("<col=245C2D>Fletching: Headless Arrow</col>");
        NewMenuEntry combined = createWidgetOnWidgetEntry("Fletch", "Feather", featherSlot, event.getParam1(), ItemID.FEATHER);
        CupidBot.getMouse().click(CupidBot.getClient().getMouseCanvasPosition(), combined);
    ...
    private void quickFletchDartsOnClick(MenuEntry event, FletchingDarts dart)
    {
        ...
        CupidBot.log("<col=245C2D>Fletching: " + dart.getDartTip() + " To: " + dart.getDart() + "</col>");
        NewMenuEntry combined = createWidgetOnWidgetEntry("Fletch", "Feather", featherSlot, event.getParam1(), ItemID.FEATHER);
        CupidBot.getMouse().click(CupidBot.getClient().getMouseCanvasPosition(), combined);
    }
    ...
    private void quickFletchBoltOnClick(MenuEntry event, FletchingBolt bolt)
    {
        ...
        CupidBot.log("<col=245C2D>Fletching: " + bolt.getBoltTip() + " To: " + bolt.getBolt() + "</col>");
        NewMenuEntry combined = createWidgetOnWidgetEntry("Fletch", "Feather", featherSlot, event.getParam1(), ItemID.FEATHER);
        CupidBot.getMouse().click(CupidBot.getClient().getMouseCanvasPosition(), combined);
    }
    ...
    private void quickFletchArrowOnClick(MenuEntry event, FletchingArrow arrow)
    {
        ...
        if (headlessSlot == -1) {
            CupidBot.log("<col=5F1515>Headless Arrow not found in inventory</col>");
            return;
        }

        CupidBot.log("<col=245C2D>Fletching: " + arrow.getArrow() + "</col>");
