package net.runelite.client.plugins.cupidbot.plankrunner;
...
import net.runelite.client.plugins.cupidbot.plankrunner.enums.PlankRunnerState;
                ...
                switch (state) {
                    case BANKING:
                        ...
                        if (!Rs2Bank.hasBankItem(plugin.getPlank().getLogItemId(), logsToWithdraw)) {
                            CupidBot.showMessage("Not enough logs for a full run!");
                            shutdown();
                            return;
                        }
                    ...
                    case RUNNING_TO_SAWMILL:
                        ...
                        if (!isNearSawmill) {
                            CupidBot.status = "Running to Sawmill";
                        ...
                        var sawmillOperator = CupidBot.getRs2NpcCache().query()
                                .where(n -> sawmillNpcs.contains(n.getId()))
                                .nearest();

                        if (sawmillOperator == null) {
                            CupidBot.showMessage("Unable to find Sawmill Operator!");
                            shutdown();
                            return;
                        }

                        sawmillOperator.click("Buy-plank");
                        CupidBot.status = "Buying Planks";
