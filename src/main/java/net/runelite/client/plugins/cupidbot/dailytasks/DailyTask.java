package net.runelite.client.plugins.cupidbot.dailytasks;
...
import static net.runelite.client.plugins.cupidbot.CupidBot.doInvoke;
...
public enum DailyTask {
    ...
    HERB_BOXES(
            ...
            () -> CupidBot.getClient().getVarbitValue(VarbitID.IRONMAN) == 0
                    && CupidBot.getClient().getVarpValue(VarPlayerID.NZONE_REWARDPOINTS) >= 9500
                    && CupidBot.getClient().getVarbitValue(VarbitID.NZONE_HERBBOXES_PURCHASED) < 15,
            () -> {
                CupidBot.getRs2TileObjectCache().query().interact(26273, "Search");
    ...
    BATTLESTAVES(
            ...
            () -> CupidBot.getClient().getVarbitValue(Varbits.DIARY_VARROCK_EASY) == 1
                    && CupidBot.getClient().getVarbitValue(Varbits.DAILY_STAVES_COLLECTED) == 0,
            () -> {
                CupidBot.getRs2TileObjectCache().query().interact(30357);
    ...
    PURE_ESSENCE(
            ...
            () -> CupidBot.getClient().getVarbitValue(Varbits.DIARY_ARDOUGNE_MEDIUM) == 1
                    && CupidBot.getClient().getVarbitValue(Varbits.DAILY_ESSENCE_COLLECTED) == 0,
            () -> {
                CupidBot.getRs2NpcCache().query().withId(8481).interact("Claim");
                Rs2Inventory.waitForInventoryChanges(1000);
            },
...
//            () -> CupidBot.getClient().getVarbitValue(Varbits.DIARY_WILDERNESS_EASY) == 1
//                    && CupidBot.getClient().getVarbitValue(Varbits.DAILY_RUNES_COLLECTED) == 0,
//            () -> {
//            },
...
//    ),
    ...
    FLAX(
            ...
            () -> CupidBot.getClient().getVarbitValue(Varbits.DIARY_KANDARIN_EASY) == 1
                    && CupidBot.getClient().getVarbitValue(Varbits.DAILY_FLAX_STATE) == 0,
            () -> {
                CupidBot.getRs2NpcCache().query().withId(5522).interact("Exchange");
...
//            () -> {
//                if (CupidBot.getClient().getVarbitValue(Varbits.DIARY_MORYTANIA_MEDIUM) != 1) {
//                    return false;
//                }
//                int collected = CupidBot.getClient().getVarbitValue(Varbits.DAILY_BONEMEAL_STATE);
//                int max = 13;
//                if (CupidBot.getClient().getVarbitValue(Varbits.DIARY_MORYTANIA_HARD) == 1) {
//                    max += 13;
//                    if (CupidBot.getClient().getVarbitValue(Varbits.DIARY_MORYTANIA_ELITE) == 1) {
//                        max += 13;
//                    }
//                }
...
//            () -> CupidBot.getClient().getVarbitValue(Varbits.DIARY_KOUREND_MEDIUM) == 1
//                    && CupidBot.getClient().getVarbitValue(Varbits.DAILY_DYNAMITE_COLLECTED) == 0,
//            () -> {
//            },
...
//    ),
    ...
    MISCELLANIA(
            ...
            () -> CupidBot.getClient().getVarpValue(VarPlayer.THRONE_OF_MISCELLANIA) > 0,
            () -> {
                sleepUntil(() -> Rs2Player.getWorldLocation().getRegionID() == 10044, 10000);
                CupidBot.getRs2TileObjectCache().query().interact(15079);
                sleepUntilOnClientThread(() -> CupidBot.getClient().getVarbitValue(Varbits.KINGDOM_APPROVAL) == 127, 10000);
                Rs2Walker.walkTo(new WorldPoint(2502, 3858, 1), 5);
                CupidBot.getRs2NpcCache().query().withId(5448).interact("Collect");
    ...
    private static boolean approvalAtMax() {
        return CupidBot.getVarbitValue(Varbits.KINGDOM_APPROVAL) >= MAX_APPROVAL;
    }
    ...
    private static void rakeMiscellania() {
        ...
        boolean fairyRingItemless = CupidBot.getClientThread()
                .runOnClientThreadOptional(() -> CupidBot.getClient().getVarbitValue(Varbits.DIARY_LUMBRIDGE_ELITE) == 1)
                .orElse(false);
        ...
        if (!Rs2Inventory.hasItem(ItemID.RAKE)) {
            CupidBot.log("Rake Miscellania: no rake in inventory or bank — skipping.");
            return;
        }
        ...
        if (!fairyRingItemless && !Rs2Equipment.isWearing(ItemID.DRAMEN_STAFF) && !Rs2Equipment.isWearing(ItemID.LUNAR_STAFF)) {
            if (Rs2Inventory.hasItem(ItemID.DRAMEN_STAFF)) {
                ...
                CupidBot.log("Rake Miscellania: no Dramen/Lunar staff and no Elite Lumbridge diary — cannot reach Miscellania, skipping.");
                return;
            }
        ...
        if (Rs2Player.getWorldLocation().getRegionID() != MISCELLANIA_REGION) {
            CupidBot.log("Rake Miscellania: failed to reach Miscellania — skipping.");
            return;
        }
        ...
        while (CupidBot.isLoggedIn() && CupidBot.isPluginEnabled(DailyTasksPlugin.class)) {
