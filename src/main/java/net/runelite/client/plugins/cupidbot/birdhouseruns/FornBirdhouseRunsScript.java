package net.runelite.client.plugins.cupidbot.birdhouseruns;
...
import net.runelite.client.plugins.cupidbot.birdhouseruns.FornBirdhouseRunsInfo.states;
import net.runelite.client.plugins.cupidbot.birdhouseruns.enums.Log;
...
import net.runelite.client.plugins.cupidbot.shortestpath.Restriction;
...
import static net.runelite.client.plugins.cupidbot.birdhouseruns.FornBirdhouseRunsInfo.*;
...
public class FornBirdhouseRunsScript extends Script {
                    ...
                    switch (botStatus) {
                        ...
                        case FINISHING:
                            ...
                            if (config.startGiantSeaweedAfter()) {
                                ...
                                if (!CupidBot.startPlugin("net.runelite.client.plugins.cupidbot.GiantSeaweedFarmer.GiantSeaweedFarmerPlugin")) {
    ...
    private boolean dismantleBirdhouse(WorldPoint loc, int varpId) {
        ...
        int varp = CupidBot.getVarbitPlayerValue(varpId);
        if (!isSeeded(varp)) {
        ...
        if (!sleepUntil(() -> isEmpty(CupidBot.getVarbitPlayerValue(varpId)), 10000)) {
            log.warn("Dismantle[{}]: timeout waiting for varp→0 after Empty click; varp={} (player at {})",
                    varpId, CupidBot.getVarbitPlayerValue(varpId), Rs2Player.getWorldLocation());
            return false;
        }
    ...
    private boolean buildBirdhouse(WorldPoint loc, int varpId) {
        ...
        if (!sleepUntil(() -> !isEmpty(CupidBot.getVarbitPlayerValue(varpId)), 15000)) {
        ...
        log.info("Build[{}]: success (varp={})", varpId, CupidBot.getVarbitPlayerValue(varpId));
        return true;
    }
