package net.runelite.client.plugins.cupidbot.GemCrabKiller;
...
public class GemCrabKillerScript extends Script {
    ...
    private void handleWaiting(GemCrabKillerConfig config) {
        ...
        if (CupidBot.getRs2NpcCache().query().withId(CRAB_NPC_ID).nearest() != null) {
        ...
        if (Rs2Bank.isOpen()) {
            if (config.useInventorySetup()) {
                if (config.useInventorySetup() && config.inventorySetup() == null) {
                    CupidBot.showMessage("Please select an inventory setup in the plugin settings. If you've already done so, please reselect the inventory setup in the plugin settings.");
                    shutdown();
                    return;
                }
                ...
                if (equipmentMatches && inventoryMatches) {
                    ...
                    CupidBot.showMessage("Unable to load inventory setup. Shutting down.");
                    shutdown();
                }
            } else {
    ...
    private void handleFighting(GemCrabKillerConfig config) {
        Rs2NpcModel npc = CupidBot.getRs2NpcCache().query().withId(CRAB_NPC_ID).nearest();
        Rs2NpcModel deadNpc = CupidBot.getRs2NpcCache().query().withId(CRAB_NPC_DEAD_ID).nearest();
        if (deadNpc != null) {
            totalCrabKills++;
            ...
            CupidBot.getRs2TileObjectCache().query().withId(CAVE_ENTRANCE_ID).interact("Crawl-through");
    ...
    private void handleWalking() {
        ...
        Rs2TileObjectModel caveEntrance = CupidBot.getRs2TileObjectCache().query().withId(CAVE_ENTRANCE_ID).nearest();
        if (caveEntrance != null) {
            ...
            if (composition != null && java.util.Arrays.stream(composition.getActions()).anyMatch("Crawl-through"::equals)) {
                CupidBot.getRs2TileObjectCache().query().withId(CAVE_ENTRANCE_ID).interact("Crawl-through");
                sleepUntil(() -> CupidBot.getRs2NpcCache().query().withId(CRAB_NPC_ID).nearest() != null, 5000);
                if (CupidBot.getRs2NpcCache().query().withId(CRAB_NPC_ID).nearest() != null) {
                    gemCrabKillerState = GemCrabKillerState.FIGHTING;
                }
                return;
