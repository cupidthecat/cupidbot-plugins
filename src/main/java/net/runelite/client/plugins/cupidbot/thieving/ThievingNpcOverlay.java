...
public class ThievingNpcOverlay extends Overlay {
    ...
    public Dimension render(Graphics2D graphics) {
        ...
        final Shape hull = CupidBot.getClientThread().runOnClientThreadOptional(npc::getConvexHull).orElse(null);
        if (hull == null) return null;

        OverlayUtil.renderPolygon(graphics, hull, TARGET_COLOR);

        final String label = plugin.getThievingScript().getThievingNpcName();
        CupidBot.getClientThread().runOnClientThreadOptional(() -> npc.getCanvasTextLocation(graphics, label, npc.getLogicalHeight() + 40))
