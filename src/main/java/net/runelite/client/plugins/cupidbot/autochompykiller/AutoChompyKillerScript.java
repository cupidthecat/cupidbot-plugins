package net.runelite.client.plugins.cupidbot.autochompykiller;
...
public class AutoChompyKillerScript extends Script {
    ...
    private boolean isBloatedToadOnGround() {
        long toadCount = CupidBot.getRs2NpcCache().query().withId(NpcID.BLOATED_TOAD).where(element -> element.getWorldLocation().equals(Rs2Player.getWorldLocation())).count();

        return toadCount > 0;
    }

    private boolean isDeadChompyNearby() {
        long deadChompyCount = CupidBot.getRs2NpcCache().query().withId(NpcID.CHOMPYBIRD_DEAD).where(element -> Rs2Player.getWorldLocation().distanceTo(element.getWorldLocation()) <= 5).count();
    ...
    private Rs2NpcModel getNearestReachableNpc(int npcId) {
        ...
        return CupidBot.getRs2NpcCache().query().withId(npcId).toList().stream()
    ...
    private void manageRunEnergy(AutoChompyKillerConfig config) {
        if (CupidBot.getClient().getEnergy() >= 20) return;
    ...
    private void handleLogoutOnCompletion(String reason) {
        CupidBot.showMessage(reason + " - waiting to logout...");
        CupidBot.status = "Waiting to logout";

        sleepUntil(() -> !Rs2Player.isInCombat(), 30000);

        if (Rs2Player.isInCombat()) {
            CupidBot.showMessage("Still in combat after 30 seconds - logging out anyway...");
        }

        CupidBot.status = "Logging out";
        Rs2Player.logout();
        sleepUntil(() -> !CupidBot.isLoggedIn(), 5000);

        if (CupidBot.isLoggedIn()) {
            CupidBot.showMessage("Logout failed - stopping script...");
        }
    ...
    private boolean checkStopConditions(AutoChompyKillerConfig config) {
        if (config.stopOnKillCount() && chompyKills >= config.killCount()) {
            ...
            if (config.logoutOnCompletion()) {
                ...
                CupidBot.showMessage(reason + " - stopping");
                CupidBot.status = "IDLE";
            }
            return true;
        }
    ...
    private boolean validateConfig() {
        if (config == null) {
            ...
            CupidBot.showMessage("Configuration error");
            return false;
        }
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if (!Rs2Equipment.isWearing(EquipmentInventorySlot.AMMO)) {
                    ...
                    CupidBot.showMessage("No ammo - aborting...");
                    CupidBot.status = "IDLE";
                ...
                if (!Rs2Equipment.isWearing(ItemID.OGRE_BOW) && !Rs2Equipment.isWearing(ItemID.ZOGRE_BOW)) {
                    ...
                    CupidBot.showMessage("No ogre bow equipped - aborting...");
                    CupidBot.status = "IDLE";
                    // wait briefly before shutdown to display message
                    sleepUntil(() -> true, 3000);
                    shutdown();
                ...
                switch (state) {
                    case FILLING_BELLOWS:
                        ...
                        CupidBot.status = "Filling bellows";
                        if (CupidBot.getRs2TileObjectCache().query().interact(ObjectID.SWAMPBUBBLES, "Suck")) {
                    ...
                    case INFLATING:
                        ...
                        CupidBot.status = "Inflating toads";
                    ...
                    case ATTACKING:
                        ...
                        CupidBot.status = "Attacking chompy";
                    ...
                    case PLUCKING:
                        ...
                        CupidBot.status = "Plucking chompy";
    ...
    public void handleNotMyChompy() {
        CupidBot.showMessage("Someone else is hunting chompys in this world - aborting...");
        CupidBot.status = "IDLE";
        shutdown();
    }

    public void handleBowNotPowerfulEnough() {
        CupidBot.showMessage("Your bow isn't powerful enough for those arrows - aborting...");
        CupidBot.status = "IDLE";
        shutdown();
    }
    ...
    public void handleCantReachBubbles() {
        var bubbles = CupidBot.getRs2TileObjectCache().query().withId(ObjectID.SWAMPBUBBLES).toList();
        if (bubbles != null && !bubbles.isEmpty()) {
            ...
            if (bubble != null) {
                ...
                CupidBot.showMessage("No bubbles available - stopping");
                CupidBot.status = "IDLE";
                shutdown();
            }
        } else {
            CupidBot.showMessage("No bubbles found - stopping");
            CupidBot.status = "IDLE";
            shutdown();
        }
    }
