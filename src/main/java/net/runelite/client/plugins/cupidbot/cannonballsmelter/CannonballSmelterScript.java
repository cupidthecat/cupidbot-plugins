...
import net.runelite.client.plugins.cupidbot.cannonballsmelter.enums.CannonballSmelterStates;
import net.runelite.client.plugins.cupidbot.cannonballsmelter.enums.Furnace;
...
public class CannonballSmelterScript extends Script {
    ...
    public void smelt() {
        Rs2TileObjectModel furnace = CupidBot.getRs2TileObjectCache().query().withId(config.getFurnace().furnaceID).nearest();

        if(config.getFurnace() == Furnace.SHILO_VILLAGE) {
            furnace = CupidBot.getRs2TileObjectCache().query().withName("Furnace").nearestOnClientThread();
        }

        if (furnace != null) {
            furnace.click("Smelt");
            CupidBot.status = "Moving to furnace...";
            sleepUntil(() -> Rs2Widget.getWidget(17694733) != null);
            if(Rs2Widget.getWidget(17694733) != null) {
                Rs2Widget.clickWidget(17694734);
                CupidBot.status = "Smelting Cannonballs...";
                sleep(200,600);
                mouseOff();
            ...
            CupidBot.log("Cannot find furnace...");
            sleep(10000);
        }
    }
    ...
    public void bank() {
        ...
        while (!Rs2Bank.isOpen() && attempts++ < 10) {
            ...
            if(config.getFurnace() == Furnace.SHILO_VILLAGE) {
                var banker = CupidBot.getRs2NpcCache().query().withName("Banker").nearestOnClientThread();
        ...
        if (Rs2Bank.isOpen() && !Rs2Bank.hasItem(ItemID.STEEL_BAR)) {
            CupidBot.showMessage("No steel bars in bank. Halting.");
            sleep(3000, 5000);
            shutdown();
            return;
        }
        ...
        if (hasBars()) {
            ...
            CupidBot.showMessage("Failed to withdraw steel bars.");
            shutdown();
        }
    }
    ...
    public void getMould() {
        if(!Rs2Inventory.hasItem("ammo mould") && !Rs2Inventory.hasItem("double ammo mould")) {
            ...
            if(!Rs2Bank.hasItem("ammo mould") && !Rs2Bank.hasItem("double ammo mould")) {
                CupidBot.showMessage("Could not find ammo mould or double ammo mould in bank, exiting...");
                sleep(3000, 5000);
                shutdown();
            }
        ...
        if(!Rs2Bank.hasItem(ItemID.STEEL_BAR)) {
            CupidBot.showMessage("Can't find Steel bars in bank, exiting...");
            sleep(3000,5000);
            shutdown();
        }
        Rs2Bank.withdrawAll(ItemID.STEEL_BAR);
        ...
        if (!Rs2Inventory.hasItem(ItemID.STEEL_BAR)) {
            CupidBot.showMessage("Could not find item in bank.");
            shutdown();
        }
    }
    ...
    public void mouseOff() {
        ...
        if (exitHorizontally) {
            CupidBot.naturalMouse.moveTo(horizontal, random.nextInt(0, client.getCanvasHeight() + 1));
        } else {
            CupidBot.naturalMouse.moveTo(random.nextInt(0, client.getCanvasWidth() + 1), vertical);
        }
    }
