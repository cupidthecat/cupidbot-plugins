...
import net.runelite.client.plugins.cupidbot.util.magic.Rs2Staff;
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if (!plugin.getSuperHeatItem().hasRequiredLevel()) {
                    CupidBot.showMessage("You do not have the required level for this item");
                    shutdown();
                    return;
                }
                ...
                switch (state) {
                    case BANKING:
                        ...
                        if (!hasFireStaffEquipped) {
                            ...
                            if (staffItem == null) {
                                CupidBot.showMessage("Unable to find staff");
                                shutdown();
                                return;
                            }
                        ...
                        if (!Rs2Inventory.hasItem(ItemID.NATURERUNE)) {
                            if (!Rs2Bank.hasItem(ItemID.NATURERUNE)) {
                                CupidBot.showMessage("Nature Runes not found");
                                shutdown();
                                return;
                            }
                        ...
                        if (!Rs2Bank.hasBankItem(plugin.getSuperHeatItem().getItemID(), requiredPrimary)) {
                            CupidBot.showMessage("Missing Ore Requirement!");
                            shutdown();
                            return;
                        }
                        ...
                        if (requiredSecondary > 0) {
                            if (!Rs2Bank.hasBankItem(plugin.getSuperHeatItem().getSecondaryItemID(), requiredSecondary)) {
                                CupidBot.showMessage("Missing Secondary Material Requirement!");
                                shutdown();
                                return;
                            }
