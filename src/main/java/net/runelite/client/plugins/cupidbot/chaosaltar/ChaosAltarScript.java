package net.runelite.client.plugins.cupidbot.chaosaltar;
...
import static net.runelite.client.plugins.cupidbot.util.walker.Rs2Walker.walkTo;
...
public class ChaosAltarScript extends Script {
    ...
    public boolean run(ChaosAltarConfig config, ChaosAltarPlugin plugin) {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                CupidBot.log("Current state: " + currentState);
    ...
    private Rs2TileObjectModel getChaosAltar() {
        return CupidBot.getRs2TileObjectCache().query().withId(CHAOS_ALTAR).nearest();
    }
    ...
    private void dieToNpc() {
        CupidBot.log("Walking to dangerous NPC to die");
        Rs2Walker.walkTo(2979, 3845, 0);
        sleepUntil(() -> CupidBot.getRs2NpcCache().query().withId(CHAOS_FANATIC).nearest() != null, 60000);
        CupidBot.getClientThread().invoke(() -> CupidBot.getRs2NpcCache().query().withName("Chaos Fanatic").interact("Attack"));
        // Wait until player dies
        sleepUntil(() -> CupidBot.getClient().getBoostedSkillLevel(Skill.HITPOINTS) == 0, 60000);
    ...
    private void offerBones() {
        ...
        if (lastBones != null && isRunning()) {
            ...
            sleep(300, 500);
            CupidBot.getRs2TileObjectCache().query().interact(CHAOS_ALTAR);
            sleep(300, 500);
    ...
    private void offerBonesFast() {
        CupidBot.log("Offering bones at altar - IN OFFERBONES");
    ...
    private void handleBanking() {
        ...
        if (!Rs2Bank.hasItem(DRAGON_BONES)) {
            CupidBot.log("NO BONES, SHUTTING DOWN");
            shutdown();
            return;
        }

        if (!Rs2Bank.hasBankItem("Burning Amulet")) {
            CupidBot.log("NO BURNING AMULET, SHUTTING DOWN");
            shutdown();
            return;
        }
        ...
        if (!hasBurningAmulet()) {
            sleep(400);
            CupidBot.log("Withdrawing burning amulet");
    ...
    private State determineState() {
        ...
        if (didWeDie) {
            didWeDie = false;
            CupidBot.log("We died! Going to the bank...");
            return State.BANK;
        }
