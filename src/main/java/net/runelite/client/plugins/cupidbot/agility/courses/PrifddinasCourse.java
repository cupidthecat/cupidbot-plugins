...
import net.runelite.client.plugins.cupidbot.util.misc.Operation;
...
public class PrifddinasCourse implements AgilityCourseHandler
{
	...
	{
		var portalModel = CupidBot.getRs2TileObjectCache().query().withIds(PORTAL_OBSTACLE_IDS.stream().mapToInt(Integer::intValue).toArray()).within(10).nearest();
		if (portalModel != null && CupidBot.getClientThread().runOnClientThreadOptional(portalModel::getClickbox).isPresent())
		{
	...
	public boolean handleHealthMonitoring()
	{
		...
		if (currentHealth < lastKnownHealth)
		{
			...
			WorldPoint playerLocation = CupidBot.getClient().getLocalPlayer().getWorldLocation();
			...
			CupidBot.log("Fell from agility course, walking back to start");
	...
	public boolean handleWalkToStart(WorldPoint playerWorldLocation)
	{
		...
		log.info("Player plane: {}", CupidBot.getClient().getTopLevelWorldView().getPlane());
