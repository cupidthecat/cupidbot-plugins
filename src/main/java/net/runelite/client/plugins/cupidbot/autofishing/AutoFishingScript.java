...
import net.runelite.client.plugins.cupidbot.autofishing.enums.AutoFishingState;
import net.runelite.client.plugins.cupidbot.autofishing.enums.Fish;
import net.runelite.client.plugins.cupidbot.autofishing.enums.HarpoonType;
...
public class AutoFishingScript extends Script {
    ...
    private void handleFishing() {
        if (Thread.currentThread().isInterrupted() || !CupidBot.isLoggedIn()) {
            return;
        }
    ...
    private Rs2TileObjectModel getNearbyFireOrRange() {
        ...
        try {
            return CupidBot.getRs2TileObjectCache().query().withIds(fireIds).within(15).nearestOnClientThread();
        } catch (RuntimeException e) {
            if (e.getMessage() != null && e.getMessage().toLowerCase().contains("interrupted waiting for client thread")) {
                Thread.currentThread().interrupt();
                return null;
