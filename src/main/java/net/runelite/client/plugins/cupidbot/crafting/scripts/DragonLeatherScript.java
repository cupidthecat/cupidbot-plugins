...
import net.runelite.client.plugins.cupidbot.crafting.enums.DragonLeatherArmour;
...
public class DragonLeatherScript extends Script implements ICraftingScript {
    ...
    private void handleBanking() {
        ...
        if (Rs2Bank.isOpen()) {
            if (config.useCostumeNeedle()) {
                if (!Rs2Inventory.hasItem(costumeNeedle)) {
                    if (!Rs2Bank.hasItem(costumeNeedle)) {
                        CupidBot.showMessage("You don't have a costume needle!");
                        state = CraftingState.STOPPED;
                        shutdown();
                        return;
                    }
                ...
                if (!Rs2Inventory.hasItem(needle) || !Rs2Inventory.hasItem(thread)) {
                    if (!Rs2Bank.hasAllItems(new int[]{needle, thread})) {
                        CupidBot.showMessage("You've run out of materials!");
                        state = CraftingState.STOPPED;
                        shutdown();
                        return;
                    }
            ...
            if (!Rs2Bank.hasItem(armour.getLeatherId())) {
                CupidBot.showMessage("You've run out of leather!");
                state = CraftingState.STOPPED;
                shutdown();
                return;
            }
    ...
    private void handleCrafting() {
        DragonLeatherArmour armour = config.dragonLeatherType();
        CupidBot.status = "Crafting " + armour.getName();
        ...
        if (lastCraftingXp <= 0) {
            lastCraftingXp = CupidBot.getClient().getSkillExperience(Skill.CRAFTING);
        }
        ...
        if (sleepUntil(() -> Rs2Widget.getWidget(17694733) != null, 5000)) {
            ...
            while (super.run() && hasMaterials() && CupidBot.isGainingExp) {
                long currentXp = CupidBot.getClient().getSkillExperience(Skill.CRAFTING);
