package net.runelite.client.plugins.cupidbot.hunterKabbits;
...
public class HunterKabbitsScript extends Script {
    ...
    public void run(HunterKebbitsConfig config, HunterKebbitsPlugin plugin) {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                if (!CupidBot.isLoggedIn() || !this.isRunning()) return;
                ...
                e.printStackTrace();
                CupidBot.status = "Error: " + e.getClass().getSimpleName() + " - " + e.getMessage();
            }
    ...
    private void handleRetrievingState(HunterKebbitsConfig config) {
        ...
        if (hintNpc != null && VALID_FALCON_NPC_IDS.contains(hintNpc.getId())) {
            Rs2NpcModel model = CupidBot.getRs2NpcCache().query()
            ...
            boolean anyFalconStillActive = CupidBot.getRs2NpcCache().query()
    ...
    private void handleCatchingState(HunterKebbitsConfig config) {
        ...
        Rs2NpcModel kebbit = CupidBot.getRs2NpcCache().query().withName(npcName).nearestOnClientThread();
        if (kebbit != null && kebbit.click("Catch")) {
            boolean falconActive = false;
            for (int i = 0; i < 10; i++) {
                if (!isRunning()) break;

                boolean found = CupidBot.getRs2NpcCache().query()
                        .where(npc -> VALID_FALCON_NPC_IDS.contains(npc.getId()))
                        .count() > 0;
    ...
    private KebbitHunting getKebbit(HunterKebbitsConfig config) {
        int level = CupidBot.getClient().getRealSkillLevel(Skill.HUNTER);
    ...
    private boolean isHintArrowNpcActive() {
        return CupidBot.getClient().getHintArrowNpc() != null;
    }
    ...
    private boolean isFalconWithPlayer() {
        boolean falconVisible = CupidBot.getRs2NpcCache().query()
                .where(npc -> VALID_FALCON_NPC_IDS.contains(npc.getId()))
                .count() > 0;
    ...
    public void shutdown() {
        ...
        CupidBot.status = "Script stopped.";
    }
}
