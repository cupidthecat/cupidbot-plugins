package net.runelite.client.plugins.cupidbot.baggedplants;

import net.runelite.api.widgets.Widget;
import net.runelite.client.plugins.cupidbot.CupidBot;
import net.runelite.client.plugins.cupidbot.Script;
import net.runelite.client.plugins.cupidbot.baggedplants.enums.BaggedPlantsState;
...
public class BaggedPlantsScript extends Script {
    ...
    private void calculateState(BaggedPlantsConfig config) {
        ...
        if (inHouse && hasUnnotedPlants) {
            ...
            var builtPlant = CupidBot.getRs2TileObjectCache().query().withId(BUILT_PLANT).nearest();
            if (builtPlant != null) {
                state = BaggedPlantsState.REMOVE_PLANT;
            } else {
                // No built plant, check for empty plant space to build on
                var plantSpace = CupidBot.getRs2TileObjectCache().query().withId(PLANT_SPACE).nearest();
                if (plantSpace != null) {
    ...
    private boolean checkRequiredItems() {
        if (!Rs2Inventory.hasItem(COINS)) {
            CupidBot.showMessage("Missing item Coins, go back and get it!");
            shutdown();
            return false;
        }
        ...
        if (totalWateringCans < 3) {
            CupidBot.showMessage("3 watering cans required - go back and get them!");
            shutdown();
            return false;
        }
        ...
        if (!Rs2Inventory.hasItem(UNNOTED_BAGGED_PLANT) && !Rs2Inventory.hasItem(NOTED_BAGGED_PLANT)) {
            CupidBot.showMessage("No more plants, shutting down");
            shutdown();
            return false;
        }
    ...
    private boolean isInHouse() {
        ...
        return CupidBot.getRs2NpcCache().query().withName("Phials").nearestOnClientThread() == null;
    }
    ...
    private void enterHouse(BaggedPlantsConfig config) {
        var housePortal = CupidBot.getRs2TileObjectCache().query().withId(HOUSE_PORTAL).nearest();
        if (housePortal != null) {
            ...
            CupidBot.showMessage("Can't find house portal!");
            shutdown();
        }
    }
    ...
    private void refillSupplies() {
        ...
        if (!Rs2Inventory.hasItem(UNNOTED_BAGGED_PLANT) && Rs2Inventory.hasItem(NOTED_BAGGED_PLANT)) {
            if (Rs2Widget.getWidget(14352385) == null) {
                if (!Rs2Inventory.isItemSelected()) {
                    ...
                    CupidBot.getClientThread().invoke(() -> CupidBot.getRs2NpcCache().query().withName("Phials").interact("Use"));
        ...
        if (fullWateringCans < 3) {
            ...
            if (wateringCanToRefill != -1) {
                var well = CupidBot.getRs2TileObjectCache().query().withId(SINK).nearest();
                if (well != null) {
                    ...
                    CupidBot.showMessage("Can't find well to refill watering cans!");
                    shutdown();
                    return;
                }
            } else {
                CupidBot.showMessage("Can't find watering can to refill!");
                shutdown();
                return;
            }
        }
