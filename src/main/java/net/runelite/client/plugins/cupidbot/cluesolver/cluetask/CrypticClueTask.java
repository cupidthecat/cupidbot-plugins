package net.runelite.client.plugins.cupidbot.cluesolver.cluetask;
...
import net.runelite.client.plugins.cupidbot.cluesolver.ClueSolverPlugin;
...
public class CrypticClueTask extends ClueTask {
    ...
    private void transitionToNextState() {
        ...
        } else if (clue.getNpc(clueScrollPlugin) != null && CupidBot.getRs2NpcCache().query().withName(clue.getNpc(clueScrollPlugin)).nearestOnClientThread() != null) {
    ...
    private boolean killEnemy() {
        Rs2NpcModel enemy = CupidBot.getRs2NpcCache().query().withName(clue.getEnemy().name()).nearestOnClientThread();
    ...
    private boolean lootGroundItem() {
        var groundItems = CupidBot.getRs2TileItemCache().query().within(10).toList();
    ...
    private boolean interactWithObject() {
        ...
        if (CupidBot.getRs2TileObjectCache().query().interact(targetObject, "Search")
                || CupidBot.getRs2TileObjectCache().query().interact(targetObject, "Investigate")
                || CupidBot.getRs2TileObjectCache().query().interact(targetObject, "Examine")
                || CupidBot.getRs2TileObjectCache().query().interact(targetObject, "Look-at")
                || CupidBot.getRs2TileObjectCache().query().interact(targetObject, "Open")) {
    ...
    private boolean interactWithNpc() {
        Rs2NpcModel targetNpc = CupidBot.getRs2NpcCache().query().withName(clue.getNpc(clueScrollPlugin)).nearestOnClientThread();
        if (targetNpc == null) {
