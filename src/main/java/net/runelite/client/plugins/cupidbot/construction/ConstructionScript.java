package net.runelite.client.plugins.cupidbot.construction;

import net.runelite.api.coords.WorldPoint;
import net.runelite.api.widgets.Widget;
import net.runelite.client.plugins.cupidbot.construction.ConstructionConfig;
import net.runelite.client.plugins.cupidbot.construction.enums.ConstructionState;
...
public class ConstructionScript extends Script {
    ...
    public Rs2TileObjectModel getClosestTile(List<Integer> objIDs) {
        ...
        return CupidBot.getRs2TileObjectCache().query().withIds(ids).nearest();
    }

    public Rs2NpcModel getButler() {
        return CupidBot.getRs2NpcCache().query().withName("Demon butler").nearestOnClientThread();
    }
    ...
    public boolean run(net.runelite.client.plugins.cupidbot.construction.ConstructionConfig config) {
    ...
    public void grabPlanksWhileWeBuild(net.runelite.client.plugins.cupidbot.construction.ConstructionConfig config, int actionDelay){
        if(getButler() != null) {
    ...
    private void calculateState(net.runelite.client.plugins.cupidbot.construction.ConstructionConfig config) {
        ...
        Rs2TileObjectModel objOnWorkingTile = CupidBot.getRs2TileObjectCache().query()
                .where(o -> o.getWorldLocation().equals(workingTile))
                .nearest();
        if (objOnWorkingTile == null || !objectIDs.contains(objOnWorkingTile.getId())) {
            workingTile = getClosestTile(objectIDs).getWorldLocation();
            objOnWorkingTile = CupidBot.getRs2TileObjectCache().query()
                    .where(o -> o.getWorldLocation().equals(workingTile))
                    .nearest();
        }
        ...
        if (objOnWorkingTile.getId() == objectIDs.get(0)) {
            ...
            CupidBot.getNotifier().notify("Looks like we are no longer in our house.");
            returnToTheHouse();
        }
    }

    private void returnToTheHouse(){
        Rs2TileObjectModel housePortal = CupidBot.getRs2TileObjectCache().query().withName("Portal").nearestOnClientThread();
        if(housePortal != null){
            ...
            CupidBot.getNotifier().notify("Can't find the house portal!");
            shutdown();
        }
    }

    private void buildSpace(net.runelite.client.plugins.cupidbot.construction.ConstructionConfig config, int actionDelay) {
        Rs2TileObjectModel space = CupidBot.getRs2TileObjectCache().query()
                .where(o -> o.getWorldLocation().equals(workingTile))
                .nearest();
    ...
    private void removeSpace(net.runelite.client.plugins.cupidbot.construction.ConstructionConfig config, int actionDelay) {
        Rs2TileObjectModel builtObject = CupidBot.getRs2TileObjectCache().query()
                .where(o -> o.getWorldLocation().equals(workingTile))
                .nearest();
    ...
    private void butler(net.runelite.client.plugins.cupidbot.construction.ConstructionConfig config, int actionDelay) {
