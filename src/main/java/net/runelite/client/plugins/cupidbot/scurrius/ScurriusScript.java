package net.runelite.client.plugins.cupidbot.scurrius;
...
import net.runelite.client.plugins.cupidbot.scurrius.enums.State;
...
public class ScurriusScript extends Script {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if (state != previousState) {
                    CupidBot.log("State changed to: " + getStateDescription(state));
                    previousState = state;
                }

                scurrius = CupidBot.getRs2NpcCache().query().withName("Scurrius").nearestOnClientThread();
                ...
                if (previousInFightRoom == null || isInFightRoom != previousInFightRoom) {
                    CupidBot.log(isInFightRoom ? "Player has entered the boss room." : "Player has exited the boss room.");
                ...
                if (!isScurriusPresent && !hasFood && !hasPrayerPotions) {
                    if (isInFightRoom) {
                        if (Rs2Inventory.hasItem(ItemID.POH_TABLET_VARROCKTELEPORT)) {
                            ...
                            CupidBot.log("No teleport available. Attempting to walk to bank (will likely fail).");
                        }
                    } else {
                        state = State.BANKING;
                    }
                ...
                if (state != State.WAITING_FOR_BOSS) {
                    ...
                    if (isScurriusPresent && !hasFood && CupidBot.getClient().getBoostedSkillLevel(Skill.HITPOINTS) < 25) {
                        state = State.TELEPORT_AWAY;
                    }

                    if (!isScurriusPresent && !isInFightRoom && hasFood && hasPrayerPotions) {
                        if (!hasRequiredSupplies()) {
                            CupidBot.log("Missing supplies, returning to BANKING.");
                            state = State.BANKING;
                        } else {
                ...
                switch (state) {
                    ...
                    case FIGHTING:
                            ...
                            for (WorldPoint worldPoint : dangerousWorldPoints) {
                                if (Rs2Player.getWorldLocation().equals(worldPoint)) {
                                    ...
                                    if (safeTile != null) {
                                        ...
                                        CupidBot.log("Dodging dangerous area, moving to safe tile at: " + safeTile);
                                    }
                                }
                            }
                        }
                        ...
                        if (currentTime - lastEatTime > EAT_COOLDOWN_MS) {
                            ...
                            if (CupidBot.getClient().getBoostedSkillLevel(Skill.HITPOINTS) < randomEatThreshold && !Rs2Player.isAnimating()) {
                                Rs2Player.eatAt(randomEatThreshold);
                                lastEatTime = currentTime;
                                CupidBot.log("Eating food at " + randomEatThreshold + "% health.");
                            }
                        }
                        ...
                        if (currentTime - lastPrayerTime > PRAYER_COOLDOWN_MS) {
                            ...
                            if (CupidBot.getClient().getBoostedSkillLevel(Skill.PRAYER) < randomPrayerThreshold && !Rs2Player.isAnimating()) {
                                Rs2Player.drinkPrayerPotionAt(randomPrayerThreshold);
                                lastPrayerTime = currentTime;
                                CupidBot.log("Drinking prayer potion at " + randomPrayerThreshold + "% prayer points.");
                            }
                        }

                        Rs2NpcModel giantRatModel = CupidBot.getRs2NpcCache().query()
                    ...
                    case WALK_TO_BOSS:
                        if (!hasRequiredSupplies()) {
                            CupidBot.log("Missing supplies, restarting pathfinding and returning to Bank.");
                        ...
                        CupidBot.getRs2TileObjectCache().query().withId(ObjectID.RAT_BOSS_ENTRANCE).interact(interactionType);
                    ...
                    case WAITING_FOR_BOSS:
                        ...
                        if (!hasLoggedRespawnWait) {
                            CupidBot.log("Waiting for Scurris to respawn...");
                            hasLoggedRespawnWait = true;
                            disableAllPrayers();
                        }
                        if (isScurriusPresent) {
                            state = State.FIGHTING;
                            CupidBot.log("Scurris has respawned, switching to FIGHTING.");
                        }
                        break;
                }
    ...
    private WorldPoint findSafeTile(WorldPoint playerLocation, List<WorldPoint> dangerousWorldPoints) {
        ...
        CupidBot.log("No safe tile found!");
        return null;
    }
    ...
    private boolean hasRequiredSupplies() {
        ...
        if (currentFoodCount < foodAmount) {
            CupidBot.log("Not enough food in inventory. Expected: " + foodAmount + ", Found: " + currentFoodCount);
            return false;
        }

        int currentPrayerPotionCount = Rs2Inventory.count(potionItemId);
        if (currentPrayerPotionCount < prayerPotionAmount) {
            CupidBot.log("Not enough prayer potions in inventory. Expected: " + prayerPotionAmount + ", Found: " + currentPrayerPotionCount);
            return false;
        }
    ...
    public void disableAllPrayers() {
        ...
        CupidBot.log("All prayers disabled to preserve prayer points.");
