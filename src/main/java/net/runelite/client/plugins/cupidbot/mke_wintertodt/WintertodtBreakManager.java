...
public class WintertodtBreakManager {
    ...
    private void initializeBreakTimer() {
        ...
        CupidBot.log(String.format("Next break scheduled in %d minutes", nextBreakIn / 60));
    }
    ...
    public boolean update() {
        try {
            ...
            if (currentTime >= nextBreakCheck) {
                ...
                if (breakTimeRemaining > 0 && breakActive) {
                    ...
                    if (logoutBreakActive && breakTimeRemaining % 30 == 0) {
                        CupidBot.log("Logout break countdown: " + (breakTimeRemaining / 60) + " minutes remaining");
                    }
                }
                ...
                if (nextBreakIn <= 0 && !breakActive && !lockState) {
                    if (isInSafeLocationForBreak()) {
                        ...
                        if (waitingForSafeSpotSince == 0) {
                            ...
                            CupidBot.log("Started waiting for safe location for break");
                        }
                        ...
                        if (waitingTime >= MAX_WAIT_FOR_SAFE_SPOT_MS || (waitingTime >= MAX_WAIT_FOR_SAFE_SPOT_MS - 180000 && MKE_WintertodtScript.state == State.WAITING)) {
                            if (!isWalkingToSafeSpot) {
                                CupidBot.log("Waiting for safe spot timed out, requesting walk to boss room safe area");
                                isWalkingToSafeSpot = true;
                                return true; // Signal to walk
                            } else if (isInSafeLocationForBreak()) {
                                // We've arrived at the safe spot
                                CupidBot.log("Arrived at safe spot, starting break");
                                waitingForSafeSpotSince = 0;
                                isWalkingToSafeSpot = false;
                                startRandomBreak();
                            }
                            ...
                            if (remainingWait % 30 == 0) { // Log every 30 seconds
                                CupidBot.log("Delaying break - not in safe location (will force walk in " + remainingWait / 60 + " minutes)");
                            }
                        }
                    }
                }
                ...
                if (breakTimeRemaining <= 0 && breakActive) {
                    if (logoutBreakActive) {
                        CupidBot.log("Logout break timer reached zero - ending break now");
                    }
                    endCurrentBreak();
                }
            }
            ...
            CupidBot.log("Error in WintertodtBreakManager: " + e.getMessage());
            e.printStackTrace();
            // Emergency cleanup
            emergencyCleanup();
        }
    ...
    private void startAfkBreak() {
        ...
        originalMousePosition = CupidBot.getMouse().getMousePosition();

        // Move mouse offscreen
        CupidBot.naturalMouse.moveOffScreen(90);

        // Update window title to show break status
        updateTitle();

        CupidBot.log(String.format("Started AFK break for %d minutes", duration));
    }
    ...
    private void startLogoutBreak() {
        ...
        if (!logoutSuccessful) {
            CupidBot.log("Failed to logout after 3 attempts - continuing with break anyway");
        }

        // Update window title to show break status
        updateTitle();

        CupidBot.log(String.format("Started logout break for %d minutes", duration));
    }
    ...
    private boolean attemptLogoutWithRetries(int maxAttempts) {
        for (int attempt = 1; attempt <= maxAttempts; attempt++) {
            try {
                CupidBot.log("Logout attempt " + attempt + "/" + maxAttempts);
                ...
                if (!CupidBot.isLoggedIn()) {
                    CupidBot.log("Logout successful on attempt " + attempt);
                    return true;
                }

                CupidBot.log("Logout attempt " + attempt + " failed - player still logged in");
                ...
                CupidBot.log("Error during logout attempt " + attempt + ": " + e.getMessage());
                if (attempt < maxAttempts) {
                    sleep(3000);
                }
            }
    ...
    private void endAfkBreak() {
        ...
        CupidBot.log("AFK break ended - action plan reset, resuming activities");
    }
    ...
    private void endLogoutBreak() {
        ...
        CupidBot.log("Logout break ended - action plan reset, attempting to log back in");
    ...
    private void attemptLogin() {
        try {
            ...
            if (CupidBot.isLoggedIn()) {
                CupidBot.log("Player already logged in");
                return;
            }

            CupidBot.log("Attempting to log back in after logout break...");
            ...
            CupidBot.log("All login attempts failed - player will need to log in manually to resume script");

        } catch (Exception e) {
            CupidBot.log("Error during login attempt: " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    private boolean attemptAutoLoginWithRetries(int maxAttempts) {
        if (!isAutoLoginPluginAvailable()) {
            CupidBot.log("AutoLoginPlugin not available - skipping auto login attempts");
            return false;
        }

        for (int attempt = 1; attempt <= maxAttempts; attempt++) {
            try {
                CupidBot.log("AutoLogin attempt " + attempt + "/" + maxAttempts);

                if (CupidBot.isLoggedIn()) {
                    CupidBot.log("Player already logged in during auto login attempt " + attempt);
                    sleep(20000);
                    return true;
                }
                ...
                while (!CupidBot.isLoggedIn() && System.currentTimeMillis() - loginStart < 60000) {
                    sleep(1000);
                }

                if (CupidBot.isLoggedIn()) {
                    CupidBot.log("Successfully logged in using AutoLoginPlugin on attempt " + attempt);
                    sleep(20000);
                    return true;
                }

                CupidBot.log("AutoLogin attempt " + attempt + " timed out");
                ...
                CupidBot.log("Error during AutoLogin attempt " + attempt + ": " + e.getMessage());
                if (attempt < maxAttempts) {
                    sleep(5000);
                }
            }
    ...
    private boolean attemptManualLoginWithRetries(int maxAttempts) {
        for (int attempt = 1; attempt <= maxAttempts; attempt++) {
            try {
                CupidBot.log("Manual login attempt " + attempt + "/" + maxAttempts);

                if (CupidBot.isLoggedIn()) {
                    CupidBot.log("Player already logged in during manual login attempt " + attempt);
                    sleep(20000);
                    return true;
                }
                ...
                if (LoginManager.getActiveProfile() == null) {
                    CupidBot.log("No active profile configured for automatic login");
                    return false;
                }
                ...
                CupidBot.log("Attempting manual login to world " + world + " (attempt " + attempt + ")");
                new Login(world);

                // Wait for login to complete (up to 30 seconds)
                long loginStart = System.currentTimeMillis();
                while (!CupidBot.isLoggedIn() && !Rs2Widget.isWidgetVisible(378,0) && System.currentTimeMillis() - loginStart < 30000) {
                    sleep(1000);
                }

                if (CupidBot.isLoggedIn()) {
                    CupidBot.log("Successfully logged in using manual login on attempt " + attempt);
                    sleep(20000);
                    return true;
                }

                CupidBot.log("Manual login attempt " + attempt + " timed out");
                ...
                CupidBot.log("Error during manual login attempt " + attempt + ": " + e.getMessage());
                if (attempt < maxAttempts) {
                    sleep(5000);
                }
            }
    ...
    private boolean isAutoLoginPluginAvailable() {
        try {
            return CupidBot.getPlugin(AutoLoginPlugin.class.getName()) != null;
        } catch (Exception e) {
            return false;
        }
    }
    ...
    private void enableAutoLoginPlugin() {
        try {
            AutoLoginPlugin autoLoginPlugin = (AutoLoginPlugin) CupidBot.getPlugin(AutoLoginPlugin.class.getName());
            if (autoLoginPlugin != null && !CupidBot.isPluginEnabled(autoLoginPlugin.getClass())) {
                CupidBot.getClientThread().runOnSeperateThread(() -> {
                    CupidBot.startPlugin(autoLoginPlugin);
                    return true;
                });
                CupidBot.log("AutoLoginPlugin enabled for break login");
            }
        } catch (Exception e) {
            CupidBot.log("Failed to enable AutoLoginPlugin: " + e.getMessage());
        }
    }
    ...
    private void disableAutoLoginPlugin() {
        try {
            if (isAutoLoginPluginAvailable()) {
                ...
                if (autoLoginPlugin != null && CupidBot.isPluginEnabled(autoLoginPlugin.getClass())) {
                    CupidBot.getClientThread().runOnSeperateThread(() -> {
                        CupidBot.stopPlugin(autoLoginPlugin);
                        return true;
                    });
                    CupidBot.log("AutoLoginPlugin disabled after use");
                }
            }
        } catch (Exception e) {
            CupidBot.log("Failed to disable AutoLoginPlugin: " + e.getMessage());
        }
    }
    ...
    private void onSuccessfulLogin() {
        try {
            CupidBot.log("Login detected");

            // Disable AutoLoginPlugin after successful login
            disableAutoLoginPlugin();

            CupidBot.log("Break system resuming normal operation");

            // Ensure we're on a Wintertodt world after login
            ensureWintertodtWorldAfterLogin();

        } catch (Exception e) {
            CupidBot.log("Error during post-login cleanup: " + e.getMessage());
        }
    }
        ...
        try {
            ...
            for (int world : wintertodtWorlds) {
                if (currentWorld == world) {
                    CupidBot.log("Already on Wintertodt world " + currentWorld + " after login");
                    return;
                }
            }

            // Not on a Wintertodt world, hop to one
            CupidBot.log("Not on Wintertodt world (current: " + currentWorld + ") - hopping after login...");

            int targetWorld = wintertodtWorlds[random.nextInt(wintertodtWorlds.length)];
            boolean hopSuccessful = CupidBot.hopToWorld(targetWorld);
            ...
            if (hopSuccessful) {
                ...
                if (Rs2Player.getWorld() == targetWorld) {
                    CupidBot.log("Successfully hopped to Wintertodt world " + targetWorld + " after login");
                } else {
                    CupidBot.log("World hop timeout after login - continuing on world " + Rs2Player.getWorld());
                }
            } else {
                CupidBot.log("Failed to hop to Wintertodt world after login - continuing on world " + currentWorld);
            }

        } catch (Exception e) {
            CupidBot.log("Error ensuring Wintertodt world after login: " + e.getMessage());
        }
    }
    ...
    public boolean isInSafeLocationForBreak() {
        try {
            ...
            CupidBot.log("Error checking safe location: " + e.getMessage());
            return false;
        }
    }
    ...
    public void forceBreak(BreakType type) {
        if (breakActive) {
            CupidBot.log("Break already active, ignoring force break request");
            return;
        }
    ...
    public void forceEndBreak() {
        ...
        CupidBot.log("Manually ending break - resetting action plan");
        breakTimeRemaining = 0;
        endCurrentBreak();
    }
    ...
    public void shutdown() {
        ...
        CupidBot.log("Break manager shutdown");
    }
