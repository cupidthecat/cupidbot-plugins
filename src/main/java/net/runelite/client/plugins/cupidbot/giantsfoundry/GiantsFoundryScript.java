package net.runelite.client.plugins.cupidbot.giantsfoundry;
...
import net.runelite.client.plugins.cupidbot.giantsfoundry.enums.CommissionType;
import net.runelite.client.plugins.cupidbot.giantsfoundry.enums.SmithableBars;
import net.runelite.client.plugins.cupidbot.giantsfoundry.enums.Stage;
import net.runelite.client.plugins.cupidbot.giantsfoundry.enums.State;
...
import static net.runelite.client.plugins.cupidbot.util.equipment.Rs2Equipment.get;
...
public class GiantsFoundryScript extends Script {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if ((weapon != null || shield != null) && !weapon.getName().equalsIgnoreCase("preform")) {
                    CupidBot.showMessage(("Please start the script without any weapon or shield in your equipment slot."));
                    sleep(5000);
                    return;
                }
                if (!Rs2Equipment.isWearing("ice gloves") && !Rs2Equipment.isWearing("smiths gloves")) {
                    CupidBot.showMessage(("Please start by wearing ice gloves or smiths gloves."));
                    sleep(5000);
                    return;
                }
    ...
    public boolean hasCommission() {
        CommissionType type1 = CommissionType.forVarbit(CupidBot.getVarbitValue(MouldHelper.SWORD_TYPE_1_VARBIT));
        CommissionType type2 = CommissionType.forVarbit(CupidBot.getVarbitValue(MouldHelper.SWORD_TYPE_2_VARBIT));
    ...
    public void getCommission() {
        if (!hasCommission()) {
            GiantsFoundryState.reset();
            var kovac = CupidBot.getRs2NpcCache().query().withName("kovac").nearestOnClientThread();
    ...
    private boolean hasSelectedMould() {
        return (CupidBot.getVarbitValue(GiantsFoundryState.VARBIT_BLADE_SELECTED) > 0
                && CupidBot.getVarbitValue(GiantsFoundryState.VARBIT_TIP_SELECTED) > 0
                && CupidBot.getVarbitValue(GiantsFoundryState.VARBIT_FORTE_SELECTED) > 0);
    }
    ...
    public void selectMould() {
        ...
        CupidBot.getRs2TileObjectCache().query().withId(MOULD_JIG).interact();

        sleepUntil(() -> Rs2Widget.findWidget("Forte", null) != null, 5000);

        Widget forte = Rs2Widget.findWidget("Forte", null);
        if (forte != null) {
            CupidBot.getMouse().click(forte.getBounds());
            sleep(600, 1200);
        ...
        if (blades != null) {
            CupidBot.getMouse().click(blades.getBounds());
            sleep(600, 1200);
            MouldHelper.selectBest();
            sleep(600, 1200);
        }
        ...
        if (tips != null) {
            CupidBot.getMouse().click(tips.getBounds());
            sleep(600, 1200);
            MouldHelper.selectBest();
            sleep(600, 1200);
            CupidBot.getMouse().click(forte.getBounds());
        ...
        if (setMould != null) {
            CupidBot.getMouse().click(setMould.getBounds());
        }
    }
    ...
    private boolean preconditionsMet() {
        ...
        if (CupidBot.getVarbitValue(MouldHelper.SWORD_TYPE_1_VARBIT) == 0) return false;
        return CupidBot.getVarbitValue(GiantsFoundryState.VARBIT_GAME_STAGE) == 1;
    }
    ...
    private boolean ensureBarsInInventory() {
        ...
        if (Rs2Bank.count(firstBarName) < config.firstBarAmount()
            ...
            CupidBot.log("Insufficient bars / items in bank to continue");
            this.shutdown();
            return false;
        }
    ...
    private void addBarsWithWidget(String barName, SmithableBars key) {
        ...
        CupidBot.getRs2TileObjectCache().query().interact(CRUCIBLE, "Fill");
    ...
    private void addBarsWithInventoryUse(String barName) {
        ...
        CupidBot.getRs2TileObjectCache().query().withId(CRUCIBLE).interact();
    ...
    private void pourCrucible() {
        CupidBot.getRs2TileObjectCache().query().interact(CRUCIBLE, "Pour");
        sleep(5000);
    ...
    public void pickupMould() {
        ...
        if (Rs2Inventory.isEmpty() && GiantsFoundryState.getCurrentStage() == null) {
            CupidBot.getRs2TileObjectCache().query().interact(MOULD_JIG, "Pick-up");
    ...
    public void handleGameLoop() {
        ...
        if(!Rs2Player.isAnimating(3000)) {
            CupidBot.log("Not animating, doAction -> true");
            doAction = true;
        }
        ...
        switch (state) {
            case HEATING:
                ...
                CupidBot.getRs2TileObjectCache().query().interact(LAVA_POOL, "Heat-preform");
            ...
            case COOLING_DOWN:
                ...
                CupidBot.getRs2TileObjectCache().query().interact(WATERFALL, "Cool-preform");
                GiantsFoundryState.heatingCoolingState.stop();
    ...
    private void handIn() {
        CupidBot.getClientThread().invoke(() -> CupidBot.getRs2NpcCache().query().withName("kovac").interact("Hand-in"));
    }
