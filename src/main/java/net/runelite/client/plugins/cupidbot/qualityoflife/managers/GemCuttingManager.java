...
import net.runelite.client.plugins.cupidbot.qualityoflife.enums.UncutGems;
...
public class GemCuttingManager {
    ...
    private void cutGemOnClick(MenuEntry event) {
        ...
        if (gemToCut.isEmpty()) {
            CupidBot.log("<col=5F1515>No gems available or level too low to cut gems</col>");
            return;
        }

        Rs2ItemModel gemItem = gemToCut.get(0);
        if (gemItem == null) {
            CupidBot.log("<col=5F1515>Could not find the selected gem in inventory</col>");
            return;
        }

        CupidBot.log("<col=245C2D>Cutting: " + gemItem.getName() + "</col>");
