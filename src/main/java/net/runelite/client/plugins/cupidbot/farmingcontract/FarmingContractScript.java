package net.runelite.client.plugins.cupidbot.farmingcontract;
...
public class FarmingContractScript extends Script {
            ...
            try {
                ...
                switch (phase) {
                    ...
                    case DONE:
                        ...
                        CupidBot.stopPlugin(CupidBot.getPluginManager()
                            .getPlugins().stream()
    ...
    private void saveContract() {
        if (contract != null) {
            ...
            CupidBot.getConfigManager().unsetRSProfileConfiguration(CONFIG_GROUP, CONFIG_KEY_CONTRACT);
            contractName = null;
        }
    }
    ...
    private String getCurrentTierForLevel() {
        int level = CupidBot.getClient().getRealSkillLevel(Skill.FARMING);
    ...
    private void harvestPatch(TileObject patch, PatchImplementation patchType) {
        ...
        if (patchType == PatchImplementation.FLOWER) {
            String cropName = CupidBot.getClientThread().runOnClientThreadOptional(
                () -> CupidBot.getItemManager().getItemComposition(cropItemId).getName()
            ).orElse("");
            if (!cropName.isEmpty()) {
    ...
    private boolean payGardener(PatchImplementation patchType) {
        ...
        if (gardener != null) {
            ...
            var guildGardener = CupidBot.getRs2NpcCache().query().withName(npcName).nearestOnClientThread();
    ...
    private int countWithNoted(int itemId) {
        ...
        Integer noted = CupidBot.getClientThread().runOnClientThreadOptional(() -> {
            var comp = CupidBot.getItemManager().getItemComposition(itemId);
