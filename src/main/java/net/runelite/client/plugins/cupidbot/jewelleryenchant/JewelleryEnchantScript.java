package net.runelite.client.plugins.cupidbot.jewelleryenchant;

import net.runelite.client.plugins.cupidbot.CupidBot;
import net.runelite.client.plugins.cupidbot.Script;
import net.runelite.client.plugins.cupidbot.jewelleryenchant.util.ElementalStaff;
import net.runelite.client.plugins.cupidbot.jewelleryenchant.util.Jewellery;
...
public class JewelleryEnchantScript extends Script {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if (!jewellery.hasRequiredLevel()) {
                    CupidBot.showMessage("Magic level too low.");
                    shutdown();
                    return;
                }
    ...
    private void ensureCorrectRunes(Jewellery jewellery) {
        if (!Rs2Inventory.hasItem(Runes.COSMIC.getItemId())) {
            if (Rs2Bank.hasItem(Runes.COSMIC.getItemId())) {
                ...
                CupidBot.showMessage("Out of Cosmic runes!");
                shutdown();
                return;
            }
        }
        for (Map.Entry<Runes, Integer> entry : jewellery.getRequiredRunes().entrySet()) {
            ...
            if (needsRuneInInventory && !Rs2Inventory.hasItemAmount(rune.getItemId(), amount)) {
                if (Rs2Bank.hasBankItem(rune.getItemId(), amount)) {
                    ...
                    CupidBot.showMessage("Out of " + rune.name() + " runes!");
                    shutdown();
                    return;
                }
            }
    ...
    private void handleJewelleryWithdrawal(Jewellery jewellery) {
        if (!Rs2Bank.hasItem(jewellery.getUnenchantedId())) {
            CupidBot.showMessage("You have no " + jewellery.getName() + " left in your bank.");
            shutdown();
            return;
        }
