package net.runelite.client.plugins.cupidbot.thieving;
...
import net.runelite.client.plugins.cupidbot.thieving.enums.ThievingNpc;
import net.runelite.client.plugins.cupidbot.thieving.enums.ThievingFood;
import net.runelite.client.plugins.cupidbot.thieving.npc.ThievingNpcStrategy;
import net.runelite.client.plugins.cupidbot.thieving.npc.WealthyCitizenStrategy;
...
ThievingNpcStrategy getActiveStrategy() {
    ...
    private String getNpcName(Rs2NpcModel npc) {
        ...
        return CupidBot.getClientThread().runOnClientThreadOptional(npc::getName).orElse(null);
    }
    ...
    private Rs2NpcModel getThievingNpcCache() {
        ...
        final Optional<Rs2NpcModel> npcOptional = CupidBot.getRs2NpcCache().query()
    ...
    private <T> T getAttackingNpcs(Function<Stream<Rs2NpcModel>, T> consumer, T defaultValue) {
        final Player me = CupidBot.getClient().getLocalPlayer();
        if (me == null) return defaultValue;

        final Rs2NpcModel[] npcs = CupidBot.getRs2NpcCache().query().toList().toArray(Rs2NpcModel[]::new);
    ...
    private int getMostExpensiveGroundItemId() {
        ...
        return CupidBot.getClientThread().runOnClientThreadOptional(() -> rs2TileItemCache.query()
                .toList()
                .stream()
                ...
                .distinct()
                .map(id -> {
                    final int price = CupidBot.getItemManager().getItemPrice(id);
    ...
    private State getCurrentState() {
        ...
        if (config.THIEVING_NPC() == ThievingNpc.VYRES) {
            ...
            if (CupidBot.getRs2NpcCache().query()
    ...
    private void loop() {
        ...
        switch(currentState) {
            case LOOT:
                ...
                final Rs2TileItemModel item = CupidBot.getRs2TileItemCache().query().withId(id).within(50).nearest();
                if (item == null) {
    ...
    public boolean run() {
        CupidBot.isCantReachTargetDetectionEnabled = true;
        lastAction = System.currentTimeMillis();
    ...
    private void hopWorld() {
        ...
        for (int attempt = 0; attempt < maxAttempts; attempt++) {
            final int currentWorld = CupidBot.getClient().getWorld();
            CupidBot.hopToWorld(Login.getRandomWorld(true, null));
            ...
            sleepUntilWithInterrupt(() -> CupidBot.getClient().getGameState() == GameState.HOPPING, attackedInterrupt, 15_000);
            sleepUntilWithInterrupt(() -> CupidBot.getClient().getGameState() == GameState.LOGGED_IN, attackedInterrupt, 15_000);
            boolean changed = sleepUntilWithInterrupt(() -> CupidBot.getClient().getWorld() != currentWorld, attackedInterrupt, 15_000);
    ...
    public void shutdown() {
        ...
        CupidBot.isCantReachTargetDetectionEnabled = false;
