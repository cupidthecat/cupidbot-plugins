package net.runelite.client.plugins.cupidbot.mmcaves;
...
import net.runelite.client.plugins.cupidbot.mmcaves.enums.CombatStyle;
import net.runelite.client.plugins.cupidbot.mmcaves.enums.LightSources;
import net.runelite.client.plugins.cupidbot.mmcaves.enums.Mode;
import net.runelite.client.plugins.cupidbot.mmcaves.enums.State;
...
public class MmCavesScript extends Script {
    ...
    public boolean run() {
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if (!hasLightSource) {
                    CupidBot.log("Player does NOT have a light source");
                    stopAndLog();
                }
                ...
                CupidBot.log("Error in MM Caves script: " + ex.getMessage());
            }
    ...
    private State getState() {
        if (mode == Mode.RANGE) {
            ...
            if (!wearingChin) {
                CupidBot.log("Player does not have any chinchompa's equipped");
                return State.STOP;
            }
        }
        ...
        if (mode == Mode.MAGIC) {
            if (
                ...
                CupidBot.log("Player does not have enough runes to cast selected spell: " + config.magicSpell().getSpell().getName());
                ...
                if (!missingRunes.isEmpty()) {
                    missingRunes.forEach((rune, amount) ->
                            CupidBot.log("Missing " + amount + " x " + rune.name()));
                }
        ...
        if (
                plugin.getMyWorldPoint().distanceTo(START_TILE) < 5 &&
                !plugin.isWorldChecked(CupidBot.getClient().getWorld()) &&
                !isDownstairs()
    ...
    private void handleWalkToStart() {
        CupidBot.log("Walking to start tile");
    ...
    private void handleCheckEmptyCave() {
        CupidBot.log("Checking if cave is empty");
        Rs2TileObjectModel hole = CupidBot.getRs2TileObjectCache().query().withId(28772).nearest();
        if (hole != null) {
            hole.click("Look-in");
    ...
    private void handleWorldHop() {
        CupidBot.log("Need to world hop");
        int nextWorld = -1;
        ...
        boolean isHopped = CupidBot.hopToWorld(nextWorld);
        if (!isHopped) return;
    ...
    private void handleFight() {
        ...
        if (!firstFightStarted) {
            CupidBot.log("Starting fight: initializing aggro reset timer");
            lastAggroResetTime = System.currentTimeMillis();
        ...
        if (Rs2Inventory.emptySlotCount() > 0) {
            ...
            for (Table.Cell<WorldPoint, Integer, GroundItem> cell : groundItems.cellSet()) {
                ...
                if (item != null) {
                    if (item.getId() == 143) {
                        CupidBot.log("Picking up potion from the ground");
        ...
        Rs2NpcModel target = CupidBot.getRs2NpcCache().query()
    ...
    private void handleBanking() {
        CupidBot.log("BANKING IS NOT SUPPORTED YET, WALKING TO EXIT TILE TO LOG OUT");
        state = State.STOP;
    }
    ...
    private void stopAndLog() {
        while (isDownstairs()) {
            ...
            if ( plugin.getMyWorldPoint().distanceTo(EXIT_TILE) < 3) {
                Rs2TileObjectModel rope = CupidBot.getRs2TileObjectCache().query().withId(28775).nearest();
                if (rope != null) {
                    rope.click("Climb-up");
        ...
        Rs2Player.logout();
        Plugin PlayerAssistPlugin = CupidBot.getPlugin(MmCavesPlugin.class.getName());
        CupidBot.stopPlugin(PlayerAssistPlugin);
    }
    ...
    private boolean attemptAttack(Rs2NpcModel target) {
        ...
        if (attacked) {
            ...
            CupidBot.log("Attack failed on: " + target.getName());
        }
