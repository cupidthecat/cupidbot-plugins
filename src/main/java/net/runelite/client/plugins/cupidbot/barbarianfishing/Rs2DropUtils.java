...
public final class Rs2DropUtils {
    ...
    private static SpeedManager speedUpMouse() {
        if (CupidBot.naturalMouse == null) return null;

        MouseMotionFactory factory = CupidBot.naturalMouse.getFactory();
    ...
    private static void restoreMouse(SpeedManager original) {
        if (CupidBot.naturalMouse == null || original == null) return;
        CupidBot.naturalMouse.getFactory().setSpeedManager(original);
    }
}
