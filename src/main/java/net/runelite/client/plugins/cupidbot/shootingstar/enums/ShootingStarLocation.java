package net.runelite.client.plugins.cupidbot.shootingstar.enums;
...
public enum ShootingStarLocation
{
	...
	public boolean hasRequirements()
	{
		...
		boolean hasLineOfSight = Boolean.TRUE.equals(CupidBot.getClientThread().runOnClientThreadOptional(() -> {
			if (CupidBot.getClient().getLocalPlayer() == null || CupidBot.getClient().getLocalPlayer().getWorldArea() == null)
			{
				return false;
			}
			return CupidBot.getClient().getLocalPlayer().getWorldArea().hasLineOfSightTo(CupidBot.getClient().getTopLevelWorldView(), this.getWorldPoint());
		}).orElse(false));
		switch (this)
		{
			case CRAFTING_GUILD:
			...
			case CUSTODIA_MOUNTAINS:
				...
				return CupidBot.getVarbitValue(VarbitID.VARLAMORE_VISITED) == 1;
			case CORSAIR_COVE:
