...
@PluginDescriptor(
        ...
        description = "CupidBot thieving plugin",
        authors = { "Mocrosoft", "Kryox", "Jesusfh" },
        version = ThievingPlugin.version,
        minClientVersion = "1.9.8.2",
        tags = {"thieving", "skilling"},
		iconUrl = "ThievingPlugin/assets/icon.png",
		cardUrl = "ThievingPlugin/assets/card.png",
...
public class ThievingPlugin extends Plugin {
    ...
    private void setStartXp() {
        ...
        if (startXp == 0 || (name != null && !name.equals(this.name))) {
            this.name = name;
            startXp = CupidBot.getClient().getSkillExperience(Skill.THIEVING);
        }
    }

    public int xpGained() {
        setStartXp();
        final int currentXp = CupidBot.getClient().getSkillExperience(Skill.THIEVING);
        return currentXp - startXp;
    }

	public int determineMaxCoinPouch() {
		if (CupidBot.getVarbitValue(VarbitID.ARDOUGNE_DIARY_ELITE_COMPLETE) == 1) {
			return 140;
		} else if (CupidBot.getVarbitValue(VarbitID.ARDOUGNE_DIARY_HARD_COMPLETE) == 1) {
			return 84;
		} else if (CupidBot.getVarbitValue(VarbitID.ARDOUGNE_DIARY_MEDIUM_COMPLETE) == 1) {
			return 56;
		} else {
			return 28;
		}
