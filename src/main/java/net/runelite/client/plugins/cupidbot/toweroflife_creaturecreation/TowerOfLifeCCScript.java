package net.runelite.client.plugins.cupidbot.toweroflife_creaturecreation;
...
public class TowerOfLifeCCScript extends Script {
        ...
        switch (currentState)
        {
            ...
            case MOVING_TO_ALTAR:
                if (!arrivedInTower)
                {
                    ...
                    {
                        CupidBot.log("Walking to basement ladder.");
                        Rs2Walker.walkTo(basementLadderLocation);
                    }
                    else
                    {
                        arrivedInTower = true;
                        CupidBot.log("Arrived in tower!");
                    }
                }
                else
                {
                    ...
                    {
                        Rs2TileObjectModel trapdoor = CupidBot.getRs2TileObjectCache().query().within(new WorldPoint(2648, 3212, 0), 1).nearest();
                        if (trapdoor != null)
                        {
                            CupidBot.log("Trapdoor id: " + trapdoor.getId());
                            trapdoor.click("Climb-down");
                        }
                        else
                        {
                            CupidBot.log("Trapdoor null");
                        }
                    }
                    else if (inBasement)
                    {
                        ...
                        {
                            CupidBot.log("Arrived in basement! Switching state");
    ...
    void HandleSummonIngredientAtBank(int disposableItemID, int secondaryItemID, ToLCreature _creature)
    {
        ...
        {
            CupidBot.showMessage("Missing items in bank! Shutting down.");
            CupidBot.stopPlugin(TowerOfLifeCCPlugin.class);
            return;
        }

        if (CupidBot.getVarbitValue(VarbitID.ARDOUGNE_DIARY_MEDIUM_COMPLETE) == 1)
        {
            int numItemsToWithdraw = (int)Math.floor(Rs2Inventory.emptySlotCount() * 0.5);

            //CupidBot.log("Number of secondary in bank: " + Rs2Bank.count(secondaryItemID));
    ...
    void HandleCreature(LootingParameters params, int disposableItem, int secondaryItem)
    {
        ...
        if (summonedCreature == null)
        {
            ...
            if (System.currentTimeMillis() - lastKillTime < 2000) {
                //CupidBot.log("Waiting a couple seconds since kill time before acting");
                return;
            }
            ...
            if (Rs2Inventory.hasItem(disposableItem))
            {
                ...
                //Rs2Inventory.waitForInventoryChanges(3000);
                CupidBot.getRs2TileObjectCache().query().interact(altarObjectId, "Activate");
                sleepUntil(() -> { summonedCreature = CupidBot.getRs2NpcCache().query()
                        .where(npc -> npc.getNpc() != null
                                && (npc.getNpc().getInteracting() == null || npc.getNpc().getInteracting() == CupidBot.getClient().getLocalPlayer()))
                        .nearest();
                    return summonedCreature != null;
                }, 5000);
                //CupidBot.log("Summoned creature");
            }
            else
            {
