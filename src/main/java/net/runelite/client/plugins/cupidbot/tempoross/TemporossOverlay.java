...
import static net.runelite.client.plugins.cupidbot.tempoross.TemporossScript.workArea;
...
public class TemporossOverlay extends Overlay {
    ...
    private void renderWorldPoint(Graphics2D graphics, WorldPoint point, Color color, String label) {
        ...
        //WorldPoint pl = WorldPoint.toLocalInstance(CupidBot.getClient().getTopLevelWorldView(), point).stream().findFirst().orElse(null);
        ...
        if (poly != null) {
            ...
            // Draw the label
            Point textLocation = Perspective.getCanvasTextLocation(CupidBot.getClient(), graphics, localPoint, label, 0);
