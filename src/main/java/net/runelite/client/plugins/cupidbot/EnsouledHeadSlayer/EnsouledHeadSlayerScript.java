package net.runelite.client.plugins.cupidbot.EnsouledHeadSlayer;
...
import net.runelite.client.plugins.cupidbot.util.item.Rs2EnsouledHead;
...
import static net.runelite.client.plugins.cupidbot.EnsouledHeadSlayer.EnsouledHeadSlayerScript.*;
import static net.runelite.client.plugins.cupidbot.util.antiban.enums.ActivityIntensity.MODERATE;
...
public class EnsouledHeadSlayerScript extends Script {
            ...
            try {
                ...
                if (config.useInventorySetup()) {
                    if (config.inventorySetup() == null) {
                        CupidBot.log("Inventory setup is null, please select an inventory setup. If one is already selected, please reselect it.");
                        return;
                    } else {
                    ...
                    case BANKING:
                        if (handleBanking(config)) {
                            if (Rs2Inventory.count("Ensouled") < 1) {
                                CupidBot.log("No ensouled heads found in inventory, stopping script.");
                                shutdown();
                                return;
                            }
    ...
    /*
		...
		4 = NONE
	*/
        if (CupidBot.getVarbitValue(Varbits.SPELLBOOK) != 3) {
            CupidBot.log("On wrong spellbook, switching to Arceuus...");
            CupidBot.getClientThread().invoke(() -> CupidBot.getRs2NpcCache().query().withName("Tyss").interact("Spellbook"));
        }
        ...
        if (ensouledHead == 0 && !Rs2Combat.inCombat() && Rs2Player.getInteracting() == null) {
            ...
            CupidBot.log("No ensouled heads found in inventory, banking...");
            BOT_STATE = EnsouledHeadSlayerStatus.BANKING;
            return;
        }
        if (Rs2Inventory.count(config.food().getName()) < 1 && CupidBot.getClient().getBoostedSkillLevel(Skill.HITPOINTS) < CupidBot.getClient().getRealSkillLevel(Skill.HITPOINTS) / 2) {
            ;
            CupidBot.log("Not enough food & below half health, banking...");
            BOT_STATE = EnsouledHeadSlayerStatus.BANKING;
            return;
        }
        ...
        var enemy = CupidBot.getRs2NpcCache().query()
        ...
        if (enemy.isEmpty() && Rs2Player.getInteracting() == null && CupidBot.getRs2TileObjectCache().query().withId(ENSOULED_GROUND_GRAPHICS).nearest() == null) {
            ...
            if (spell != null) {
                ...
                sleepUntil(() -> CupidBot.getRs2TileObjectCache().query().withId(ENSOULED_GROUND_GRAPHICS).nearest() != null, 4000);
                sleepUntil(() -> CupidBot.getRs2TileObjectCache().query().withId(ENSOULED_GROUND_GRAPHICS).nearest() == null, 4000);
                sleep(3500, 4500);
            }
        } else {
    ...
    private boolean handleBanking(EnsouledHeadSlayerConfig config) {
        ...
        if (Rs2Bank.count("Ensouled") < 1) {
            CupidBot.log("No ensouled heads found in bank, stopping script.");
            shutdown();
            return false;
        }
        ...
        if (!config.useInventorySetup()) {
            if (!getRunesAndEnsouledHeads(config)) {
                CupidBot.log("Failed to withdraw runes or ensouled heads");
            }
            var foodAmount = config.foodAmount() - Rs2Inventory.count(config.food().getId());
            if (foodAmount > 0) {
                CupidBot.log("Withdrawing " + foodAmount + " " + config.food().getName() + " from bank");
    ...
    private boolean getRunesAndEnsouledHeads(EnsouledHeadSlayerConfig config) {
        if (config.ensouledHeads().getName().equals("All")) {
            ...
            if (headCounts.isEmpty()) {
                CupidBot.log("No ensouled heads found in bank");
                return false;
            }
            ...
            if (primaryHead == null) {
                CupidBot.log("Could not identify ensouled head type");
                return false;
            }
            ...
            if (availableSlots <= 0) {
                CupidBot.log("Not enough inventory space");
                return false;
            }
            ...
            if (headCount == 0) {
                CupidBot.log("No " + selectedHead.getName() + " found in bank");
                return false;
            }
            var headsToWithdraw = Rs2Inventory.emptySlotCount() - config.foodAmount();
            CupidBot.log("Withdrawing " + headsToWithdraw + " " + selectedHead.getName() + " from bank");
    ...
    private boolean useInventorySetup(EnsouledHeadSlayerConfig config) {
        if (config.useInventorySetup()) {
            ...
            if (!successEq || !successInv) {
                CupidBot.log("Failed to load inventory setup, trying again...");
                attempts++;
                // Stupid failsafe to handle inventory setups failing the first time
                if (attempts > 2) {
                    CupidBot.log("Failed to load inventory setup after 2 attempts, stopping script.");
                    shutdown();
                    return false;
                }
                return false;
                ...
                attempts = 0;
                CupidBot.log("Loaded inventory setup successfully.");
            }
            Rs2Bank.closeBank();
        }
        return true;
