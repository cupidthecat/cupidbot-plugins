package net.runelite.client.plugins.cupidbot.sulphurnaguafigther;
...
public class SulphurNaguaScript extends Script {
    ...
    public boolean run(SulphurNaguaConfig config) {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if (!hasInitialized) {
                    startTotalExp = CupidBot.getClient().getOverallExperience();
    ...
    private void determineState(SulphurNaguaConfig config) {
        ...
        if (potionsToPickup > 0 && pickupReady) {
            if (Rs2Inventory.isFull()) {
                CupidBot.log("Inventory is full, cannot pick up remaining potions. Starting to fight.");
        ...
        if (totalOwnedPotions < targetPotions || hasIntermediateIngredients) {
            if (currentState == SulphurNaguaState.FIGHTING) {
                ...
                CupidBot.log("All potions used. Starting preparation for a new batch.");
            }

            if (config.changeInSulphurousEssence() && Rs2Inventory.hasItem(SULPHUROUS_ESSENCE_ID) && totalOwnedPotions == 0) {
                CupidBot.log("No potions left. Exchanging Sulphurous Essence for XP.");
    ...
    private void handlePreparation(SulphurNaguaConfig config) {
        ...
        if (vialsInInv > 0) {
            ...
            if (freeSlots >= grubsToGet) {
                ...
                if (freeSlots > 0) {
                    ...
                    if (potionsToDrop <= 0) {
                        CupidBot.log("Stuck: 0 free slots, 0 potions to drop, but need grubs.");
                        return;
                    }
                    CupidBot.log("Dropping " + potionsToDrop + " potions to make space for grubs.");
        ...
        if (freeSlots > 0) {
            ...
            if (potionsToDrop <= 0 && neededPotionsTotal > 0) {
                CupidBot.log("Stuck: 0 free slots, 0 vials, 0 potions to drop, but need more potions.");
                return;
            }

            CupidBot.log("Dropping " + potionsToDrop + " potions to make space for vials.");
            dropPotions(potionsToDrop);
        }
    }

    private void processAllIngredients() {
        if (Rs2Player.isAnimating() || CupidBot.isGainingExp) {
            return;
        }
        if (Rs2Inventory.hasItem(MOONLIGHT_GRUB_ID)) {
            CupidBot.log("Grinding all available grubs...");
        ...
        if (Rs2Inventory.hasItem(MOONLIGHT_GRUB_PASTE_ID) && Rs2Inventory.hasItem(VIAL_OF_WATER_ID)) {
            CupidBot.log("Mixing all available paste...");
        ...
        if (itemID == VIAL_OF_WATER_ID) {
            ...
            while (Rs2Inventory.count(itemID) < requiredAmount && System.currentTimeMillis() - startTime < 20000) {
                ...
                if (Rs2Dialogue.hasDialogueOption("Take herblore supplies.")) {
                    ...
                    CupidBot.getRs2TileObjectCache().query().interact(SUPPLY_CRATE_ID, "Take herblore supplies");
                }
                sleep(300, 500);
            }
        } else {
            ...
            if (CupidBot.getRs2TileObjectCache().query().interact(GRUB_SAPLING_ID, "Collect-from")) {
    ...
    private void dropPotions(int count) {
        ...
        CupidBot.log("Dropped " + dropped + " potions at " + dropLocation);
    }

    private void pickupDroppedPotions() {
        if (Rs2Inventory.isFull()) {
            CupidBot.log("Inventory is full, cannot pick up.");
            return;
        }
        if (potionsToPickup <= 0) {
            resetPreparationState();
        ...
        if (potionsToPickup <= 0) {
            CupidBot.log("Finished picking up all potions.");
            resetPreparationState();
        } else if (!foundPotion) {
            CupidBot.log("Could not find any more dropped potions. Resetting.");
            resetPreparationState();
        }
    }
    ...
    private void cleanupLeftoverIngredients() {
        CupidBot.log("Cleaning up leftover ingredients...");
    ...
    private void handleGettingRunecraftingXp(SulphurNaguaConfig config) {
        if (Rs2Dialogue.isInDialogue()) {
            CupidBot.log("Handling dialogue...");
        ...
        if (!Rs2Inventory.hasItem(SULPHUROUS_ESSENCE_ID)) {
            CupidBot.log("No essence to exchange. Returning to preparation.");
            currentState = isAtLocation(selectedLocation.getPrepArea()) ? SulphurNaguaState.PREPARATION : SulphurNaguaState.WALKING_TO_PREP;
            return;
        }
        ...
        var eytallali = CupidBot.getRs2NpcCache().query().withId(EYTALLALI_ID).nearest();
        if (eytallali == null) {
            CupidBot.log("Waiting for Eytallali to appear...");
            sleep(600, 1000);
            return;
        }

        if (Rs2Player.isAnimating() || CupidBot.isGainingExp) {
            CupidBot.log("Waiting for action to complete...");
            return;
        }

        if (Rs2Inventory.useItemOnNpc(SULPHUROUS_ESSENCE_ID, EYTALLALI_ID)) {
            CupidBot.log("Exchanging essence...");
    ...
    private void handleFighting(SulphurNaguaConfig config) {
        ...
        if (needsNewTarget) {
            if (getNaguaCombatArea() != null && getNaguaCombatArea().contains(Rs2Player.getWorldLocation())) {
                var nagua = CupidBot.getRs2NpcCache().query()
                ...
                CupidBot.log("Outside combat zone, walking back to center...");
                Rs2Walker.walkTo(selectedLocation.getFightAreaCenter());
                sleep(400, 800);
            }
        }
