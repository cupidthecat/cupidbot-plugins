package net.runelite.client.plugins.cupidbot.agility.courses;

import net.runelite.api.coords.WorldPoint;
import net.runelite.client.plugins.cupidbot.CupidBot;
import net.runelite.client.plugins.cupidbot.agility.models.AgilityObstacleModel;
...
public class BrimhavenSpikeCourse implements AgilityCourseHandler {
    ...
    public void reset() {
        ...
        CupidBot.log("BrimhavenSpike course flags reset");
    }
    ...
    public boolean handlePayment() {
        ...
        var captain = CupidBot.getRs2NpcCache().query().withId(CAPTAIN_IZZY_NPC_ID).nearest();
        if (captain == null) {
            CupidBot.log("Cap'n Izzy No-Beard not found!");
            return false;
        }

        if (captain.click("Pay")) {
            CupidBot.log("Attempting to pay Cap'n Izzy No-Beard...");
            ...
            if (paymentSuccess) {
                hasPaid = true;
                CupidBot.log("Successfully paid for Brimhaven Spike course!");
                return true;
            } else {
                CupidBot.log("Payment failed - coins not deducted");
                return false;
            }
        }
    ...
    public boolean handleLadderDescent() {
        ...
        CupidBot.log("Force clicking ladder...");
        CupidBot.getRs2TileObjectCache().query().interact(3617, "Climb-down");
        ...
        if (currentLoc.getPlane() == 3) {
            ...
            lastAgilityXpForStep = CupidBot.getClient().getSkillExperience(net.runelite.api.Skill.AGILITY);
            CupidBot.log("Successfully climbed down ladder to plane 3!");
            return true;
        }

        CupidBot.log("Ladder interaction failed - still on plane " + currentLoc.getPlane());
        return false;
    }
    ...
    public boolean handleWalkToStart(WorldPoint playerWorldLocation) {
        CupidBot.log("BrimhavenSpike handleWalkToStart called - Player at: " + playerWorldLocation + ", hasPaid: " + hasPaid + ", hasClimbedLadder: " + hasClimbedLadder);
        ...
        if (playerWorldLocation.getPlane() != 0 || playerWorldLocation.distanceTo(START_POINT) > 5) {
            ...
            if (playerWorldLocation.getPlane() == 3) {
                CupidBot.log("Already in the spike course area on plane 3 - checking obstacle 3");
                ...
                if (currentObstacleIndex >= 2) {
                    ...
                    CupidBot.log("Forcing to obstacle 3 (spikes) - using timed tile-walking");
                    return handleSpikeTileWalking();
                }

                CupidBot.log("In the spike course area on plane 3 - ready for obstacles (currentObstacleIndex: " + currentObstacleIndex + ")");
                return false; // Let the main script handle obstacles
            }
            CupidBot.log("Walking to Brimhaven Spike course start point");
        ...
        if (!hasPaid) {
            CupidBot.log("Attempting to pay Cap'n Izzy...");
            boolean paymentResult = handlePayment();
            CupidBot.log("Payment result: " + paymentResult);
            return paymentResult;
        }

        // If we haven't climbed the ladder yet, handle ladder descent
        if (!hasClimbedLadder) {
            CupidBot.log("Attempting to climb down ladder...");
            boolean ladderResult = handleLadderDescent();
            CupidBot.log("Ladder result: " + ladderResult);
            return ladderResult;
        }

        // If we're on plane 3, we're in the course area
        if (playerWorldLocation.getPlane() == 3) {
            CupidBot.log("On plane 3 - currentObstacleIndex: " + currentObstacleIndex);
        ...
        CupidBot.log("BrimhavenSpike handleWalkToStart returning false - no action needed");
        return false;
    }
    ...
    public boolean handleSpikeTileWalking() {
        ...
        CupidBot.log("Walking to spike tile: " + target);
        ...
        if (currentPos.distanceTo(target) > 2) {
            CupidBot.log("Walk failed - player didn't move to target tile, retrying");
        ...
        if (Rs2Player.getHealthPercentage() < initialHealth) {
            CupidBot.log("Player took damage - retrying obstacle immediately");
            return false; // Let the script retry
        }

        // Wait for XP drop with 4 second timeout
        int initialXp = CupidBot.getClient().getSkillExperience(net.runelite.api.Skill.AGILITY);
        boolean xpGained = Global.sleepUntil(() -> {
            int currentXp = CupidBot.getClient().getSkillExperience(net.runelite.api.Skill.AGILITY);
            return currentXp > initialXp;
        }, 4000); // 4 second timeout

        if (!xpGained) {
            CupidBot.log("No XP gained within 4 seconds - walk may have failed, retrying");
            return false; // Let the script retry
        }

        // Wait for animation to finish (animation ID changes to -1)
        CupidBot.log("XP gained! Waiting for animation to finish...");
        Global.sleepUntil(() -> {
        ...
        CupidBot.log("Animation finished, ready for next obstacle attempt");
        ...
        if (playerLocation.getPlane() == 3) {
            ...
            if (currentObstacleIndex < obstacles.size()) {
                ...
                if (currentObstacleIndex == obstacles.size() - 1) {
                    CupidBot.log("On obstacle 3 (spikes) - using timed tile-walking instead of game object");
                    return null; // This will trigger the timed tile-walking logic in handleWalkToStart
                }

                var gameObject = CupidBot.getRs2TileObjectCache().query().withId(currentObstacle.getObjectID()).within(playerLocation, 10).nearest();
                if (gameObject != null) {
                    CupidBot.log("Looking for obstacle " + (currentObstacleIndex + 1) + "/" + obstacles.size() + " (ID: " + currentObstacle.getObjectID() + ")");
                    return gameObject;
                }
            }
        }
    ...
    public boolean isObstacleComplete(int currentXp, int previousXp, long lastMovingTime, int waitDelay) {
        ...
        if (currentXp > previousXp) {
            ...
            List<AgilityObstacleModel> obstacles = getObstacles();
            CupidBot.log("XP gained! Current obstacle index: " + currentObstacleIndex + ", obstacles size: " + obstacles.size());
            ...
            if (currentObstacleIndex >= obstacles.size()) {
                ...
                CupidBot.log("Completed all obstacles, staying on obstacle 3 (spikes) for timed tile-walking");
            } else {
                CupidBot.log("Obstacle completed! Moving to obstacle " + (currentObstacleIndex + 1) + "/" + obstacles.size());
            }
