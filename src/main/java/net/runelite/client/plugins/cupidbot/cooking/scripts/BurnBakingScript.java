package net.runelite.client.plugins.cupidbot.cooking.scripts;
...
import net.runelite.client.plugins.cupidbot.cooking.AutoCookingConfig;
...
import static net.runelite.client.plugins.cupidbot.util.player.Rs2Player.toggleRunEnergy;
...
public class BurnBakingScript extends Script {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                long startTime = System.currentTimeMillis();
                    ...
                    int currentCookingLevel = CupidBot.getClient().getRealSkillLevel(Skill.COOKING);
                    ...
                    if (currentCookingLevel < 41 && currentCookingLevel >= 25 && shouldCookStew() && Rs2Inventory.hasItem("Uncooked stew")) {
                        ...
                        CupidBot.log("You've tried to prepare cake between level 25 and 40 so maybe you wanna make stew. Making stew now.");
                        manageRunEnergy();
                        cookItem("Uncooked stew");
                        CupidBot.naturalMouse.moveOffScreen();
    ...
    private void withdrawAndCheck(String itemName) {
        for (int attempt = 0; attempt < 5 && !Rs2Inventory.contains(itemName); attempt++) {
            if (attempt > 0) {
                CupidBot.log(itemName + " was not withdrawn, retrying... (Attempt " + (attempt + 1) + ")");
            }
        ...
        if (!Rs2Inventory.contains(itemName)) {
            CupidBot.log("Failed to withdraw " + itemName + " after multiple attempts!");
        }
    }
    ...
    public void walkToBanker() {
        ...
        if (bankerLocation == null) {
            CupidBot.log("Failed to get the banker's location.");
            return;
        }

        WorldPoint bankerWorldLocation = WorldPoint.fromLocal(CupidBot.getClient(), bankerLocation);
        WorldPoint playerWorldLocation = CupidBot.getClientThread().invoke(() -> CupidBot.getClient().getLocalPlayer().getWorldLocation());
        ...
        WorldPoint worldLocation = WorldPoint.fromLocal(CupidBot.getClient(), targetLocation);
    ...
    public static List<Rs2NpcModel> getBankerNPCs() {
        List<Rs2NpcModel> bankers = CupidBot.getRs2NpcCache().query()
                .where(npc -> npc.getNpc() != null && npc.getNpc().getComposition() != null
                        && npc.getNpc().getComposition().getActions() != null
                        && Arrays.asList(npc.getNpc().getComposition().getActions()).contains("Bank"))
                .toList();
    ...
    private void prepareBread() {
        ...
        if (currentCookingLevel < 25) {
            ...
            CupidBot.log("You've tried to prepare cake below level 25, so maybe you wanna make bread. Making bread now.");
                ...
                if (Rs2Bank.isOpen()) {
                    ...
                    // Step 4: If we have flour and water in the bank, withdraw them
                        ...
                        if (!Rs2Inventory.hasItem(POT_OF_FLOUR) || !Rs2Inventory.hasItem(BUCKET_OF_WATER)) {
                            CupidBot.log("trying to withdrawX didn't populate inventory, so withdrawing all");
                            Rs2Bank.openBank();
                        ...
                        CupidBot.log("Could not find flour or water in the bank for bread-making.");
                    }
    ...
    private void prepareStew() {
        CupidBot.log("Entering prepareStew method");
        ...
        CupidBot.log("Completed prepareStew method");
    }
    ...
    private boolean shouldCookStew() {
        CupidBot.log("Entering shouldCookStew method");
    ...
    private void cookItem(String uncookedItemName) {
        ...
        CupidBot.status = "Cooking " + uncookedItemName;
    ...
    public static void manageRunEnergy() {
        int energy = CupidBot.getClient().getEnergy();  // Get current run energy level
        boolean isRunEnabled = CupidBot.getVarbitPlayerValue(173) == 1;  // Check if run is enabled
    ...
    public void findAndInteractWithNearestRange() {
        ...
        net.runelite.client.plugins.cupidbot.api.tileobject.models.Rs2TileObjectModel rangeObj = CupidBot.getRs2TileObjectCache().query()
    ...
    private void prepareCake() {
        ...
        if (!Rs2Bank.isOpen() && Rs2Inventory.contains(1944) && Rs2Inventory.contains("Bucket of milk") && Rs2Inventory.contains("Pot of flour") && Rs2Inventory.contains("Cake tin")) {
            ...
            if (!Rs2Bank.hasItem(1944)) {
                CupidBot.log("No more egg found in the bank");
            }
        }
    }
    ...
    private void depositAndWithdrawItems() {
        ...
        if (Rs2Bank.isOpen() && Rs2Inventory.isEmpty()) {
            ...
            if (!Rs2Inventory.isFull()) {
                CupidBot.log("inventory not full after attempted withdrawing of 4x of 7 items (28 items)");
                CupidBot.log("Withdrawing ALL of X item(s) IF inventory doesn't have X item");
                CupidBot.log("If you are looping here make sure to have an EVEN NUMBER of EACH ingredient at the start");
