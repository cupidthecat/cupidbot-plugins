package net.runelite.client.plugins.cupidbot.gotr;
...
import net.runelite.client.plugins.cupidbot.gotr.data.CellType;
import net.runelite.client.plugins.cupidbot.gotr.data.GuardianPortalInfo;
import net.runelite.client.plugins.cupidbot.gotr.data.Mode;
import net.runelite.client.plugins.cupidbot.gotr.data.RuneType;
...
public class GotrScript extends Script {
    ...
    private void initializeGuardianPortalInfo() {
        ...
        guardianPortalInfo.put(ObjectID.GUARDIAN_OF_COSMIC, new GuardianPortalInfo("COSMIC", 27, ItemID.COSMIC_RUNE, 26896, 4359, RuneType.CATALYTIC, CellType.MEDIUM, CupidBot.getClientThread().runOnClientThreadOptional(() -> Quest.LOST_CITY.getState(CupidBot.getClient())).orElse(null)));
        ...
        guardianPortalInfo.put(ObjectID.GUARDIAN_OF_LAW, new GuardianPortalInfo("LAW", 54, ItemID.LAW_RUNE, 26898, 4362, RuneType.CATALYTIC, CellType.STRONG, CupidBot.getClientThread().runOnClientThreadOptional(() -> Quest.TROLL_STRONGHOLD.getState(CupidBot.getClient())).orElse(null)));
        guardianPortalInfo.put(ObjectID.GUARDIAN_OF_DEATH, new GuardianPortalInfo("DEATH", 65, ItemID.DEATH_RUNE, 26893, 4363, RuneType.CATALYTIC, CellType.OVERCHARGED, CupidBot.getClientThread().runOnClientThreadOptional(() -> Quest.MOURNINGS_END_PART_II.getState(CupidBot.getClient())).orElse(null)));
        guardianPortalInfo.put(ObjectID.GUARDIAN_OF_BLOOD, new GuardianPortalInfo("BLOOD", 77, ItemID.BLOOD_RUNE, 26894, 4364, RuneType.CATALYTIC, CellType.OVERCHARGED, CupidBot.getClientThread().runOnClientThreadOptional(() -> Quest.SINS_OF_THE_FATHER.getState(CupidBot.getClient())).orElse(null)));
    }
            ...
            try {
                ...
                if (!initCheck) {
                    ...
                    if (!Rs2Magic.isSpellbook(Rs2Spellbook.LUNAR)) {
                        CupidBot.log("Lunar spellbook not found...disabling npc contact");
                        useNpcContact = false;
                    }
                    initCheck = true;
                }
                ...
                CupidBot.log("Something went wrong in the GOTR Script: " + ex.getMessage() + ". If the script is stuck, please contact us on discord with this log.");
                ex.printStackTrace();
            }
    ...
    private boolean repairCells() {
        ...
        List<Rs2TileObjectModel> shieldCells = CupidBot.getRs2TileObjectCache().query()
        ...
        if (Rs2Inventory.hasItemAmount(GUARDIAN_ESSENCE, 10)) {
            for (Rs2TileObjectModel shieldCell : shieldCells) {
                if (CellType.GetShieldTier(shieldCell.getId()) < cellTier) {
                    CupidBot.log("Upgrading power cell at " + shieldCell.getWorldLocation());
    ...
    private boolean powerUpGreatGuardian() {
        if (Rs2Inventory.hasItem("guardian stone") && !shouldMineGuardianRemains && !isInLargeMine() && !isInHugeMine()) {
            ...
            CupidBot.getClientThread().invoke(() -> CupidBot.getRs2NpcCache().query().withName("The great guardian").interact("power-up"));
        ...
        if (!Rs2Inventory.hasItem("Uncharged cell")) {
            ...
            if (Rs2Inventory.isFull()) {
                if (Rs2Inventory.drop(ItemID.GUARDIAN_ESSENCE)) {
                    CupidBot.log("Dropped one Guardian essence to make space for Uncharged cell");
                }
            }
    ...
    private boolean usePortal() {
        if (!isInHugeMine() && CupidBot.getClient().hasHintArrow() && Rs2Inventory.count() < config.maxAmountEssence()) {
            if (leaveLargeMine()) return true;
            Rs2Walker.walkFastCanvas(CupidBot.getClient().getHintArrowPoint());
            sleepUntil(Rs2Player::isMoving);
            CupidBot.getRs2TileObjectCache().query().within(CupidBot.getClient().getHintArrowPoint(), 0).interact();
    ...
    private boolean depositRunesIntoPool() {
        ...
        // next round.
        Rs2TileObjectModel pool = CupidBot.getRs2TileObjectCache().query().withId(ObjectID.DEPOSIT_POOL).nearest();
    ...
    private void mineGuardianRemains() {
        if (CupidBot.getClient().hasHintArrow())
            return;
        if (Rs2Inventory.isFull()) {
            shouldMineGuardianRemains = false;
            return;
    ...
    private static void repairWithCordelia() {
        ...
        Rs2NpcModel pouchRepairNpc = CupidBot.getRs2NpcCache().query().withId(NpcID.APPRENTICE_CORDELIA_12180).nearest();
        ...
        CupidBot.log("Repairing pouches...");
    ...
    public  static boolean isInLargeMine() {
        ...
        return Rs2Player.getWorldLocation().getRegionID() == 14484
                && CupidBot.getClientThread().invoke(() -> CupidBot.getClient().getLocalPlayer().getWorldLocation().getX()) >= largeMineX;
    }

    public  boolean isInHugeMine() {
        int hugeMineX = 3594;
        return Rs2Player.getWorldLocation().getRegionID() == 14484
                && CupidBot.getClientThread().invoke(() -> CupidBot.getClient().getLocalPlayer().getWorldLocation().getX()) <= hugeMineX;
    }
    ...
    public ItemManager getItemManager() {
        return CupidBot.getItemManager();
    }

    public boolean isInMiniGame() {
        int parentWidgetId = 48889857;
        Widget elementalRuneWidget = CupidBot.getClient().getWidget(parentWidgetId);
    ...
    public static List<GameObject> getAvailableAltars() {
        ...
        List<GameObject> availableAltars = Rs2GameObject.getGameObjects().stream()
                .filter(x -> {
                    ...
                    if (portalInfo.getRequiredLevel()
                            > CupidBot.getClient().getBoostedSkillLevel(Skill.RUNECRAFT)) {
                        CupidBot.log("Filtered altar " + portalInfo.getName() + " – insufficient RC level");
                        return false;
                    }
                    if (portalInfo.getQuestState() != QuestState.FINISHED) {
                        CupidBot.log("Filtered altar " + portalInfo.getName() + " – quest not complete");
                        return false;
                    }
                    ...
                    CupidBot.log("Adding " + portalInfo.getName() + " to list of available altars");
                    return true;

                })
                .collect(Collectors.toList());

        CupidBot.log("Found " + availableAltars.size() + " active altars after filtering.");

        if (config.Mode() == Mode.POINTS) {
            // Sort by strongest → weakest CellType; if equal, fall back to balancing points
            CupidBot.log("Sorting by CellType (strongest→weakest) for POINTS mode...");
        ...
        if ((config.Mode() == Mode.BALANCED && elementalPoints < catalyticPoints) || config.Mode() == Mode.ELEMENTAL) {
            CupidBot.log(elementalPoints < catalyticPoints
            ...
            CupidBot.log("Sorting for BALANCED/ELEMENTAL mode (" +
        ...
        CupidBot.log("Returning unsorted altars (default mode).");
    ...
    public static void resetPlugin() {
        ...
        activeGuardianPortals.clear();
        greatGuardian = null;
        CupidBot.getClient().clearHintArrow();
    }
    ...
    private static boolean interactObject(int id, String action) {
        return interactObject(CupidBot.getRs2TileObjectCache().query().withId(id).nearest(), action);
    }
    ...
    public static Rs2TileObjectModel findRcAltar() {
        return CupidBot.getRs2TileObjectCache().query().withIds(
