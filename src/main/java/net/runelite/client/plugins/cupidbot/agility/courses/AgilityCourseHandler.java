...
public interface AgilityCourseHandler
{
	...
	default boolean waitForCompletion(final int agilityExp, final int plane)
	{
		...
		while (System.currentTimeMillis() - startTime < timeoutMs)
		{
			...
			if (Rs2Player.getHealthPercentage() < initialHealth ||
				CupidBot.getClient().getTopLevelWorldView().getPlane() != plane)
			{
				return true;
			}
	...
	default int getCurrentObstacleIndex()
	{
		...
		int playerPlane = CupidBot.getClient().getTopLevelWorldView().getPlane();
	...
	{
		if (CupidBot.getClient().getTopLevelWorldView().getPlane() != 0)
		{
			return false;
		}
		...
		{
			CupidBot.log("Going back to course's starting point");
