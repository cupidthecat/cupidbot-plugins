package net.runelite.client.plugins.cupidbot.varrockcleaner;
...
public class VarrockCleanerScript extends Script {
    ...
    private void takeUncleanedFinds() {
        ...
        if (CupidBot.getRs2TileObjectCache().query().interact(ObjectID.DIG_SITE_SPECIMEN_ROCKS, "Take")) {
    ...
    private void cleanFinds() {
        if (Rs2Inventory.contains("Uncleaned find") && CupidBot.getRs2TileObjectCache().query().interact(ObjectID.SPECIMEN_TABLE_24556, "Clean")) {
    ...
    private void storeFindsInCrate() {
        ...
        if (!Rs2Inventory.contains("Uncleaned find") && CupidBot.getRs2TileObjectCache().query().interact(ObjectID.STORAGE_CRATE, "Add finds")) {
    ...
    public void stop() {
        CupidBot.log("Varrack Cleaner plugin stopped.");
        currentState = State.TAKE_UNCLEANED;
        super.shutdown();
    }
}
