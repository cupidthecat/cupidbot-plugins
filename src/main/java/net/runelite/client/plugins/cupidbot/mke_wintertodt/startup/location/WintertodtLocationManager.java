...
public class WintertodtLocationManager {
    ...
    public static boolean attemptLeaveWintertodt()
    {
        ...
        if (Rs2Player.getWorldLocation().distanceTo(doorTile) > 4) {
            CupidBot.log("Walking closer to Wintertodt exit door...");
        ...
        if (Rs2Widget.hasWidget("Leave and lose")
            ...
            CupidBot.log("Confirming leave dialog...");
        ...
        Rs2TileObjectModel exitDoor = CupidBot.getRs2TileObjectCache().query().withId(ObjectID.DOORS_OF_DINH).within(6).nearest();
        if (exitDoor != null) {
            CupidBot.log("Clicking Wintertodt exit door...");
            ...
            CupidBot.log("Could not find Wintertodt exit door, trying to walk closer...");
        ...
        CupidBot.log("Still inside Wintertodt, retrying leave sequence...");
        return false;
    }
    ...
    public boolean navigateToWintertodt() {
        try {
            ...
            if (isInsideGameRoom()) {
                ...
                CupidBot.log("Player is inside Wintertodt game room, attempting custom door exit...");

                // Use custom door leaving logic - never webwalker
                if (attemptLeaveWintertodt()) {
                    CupidBot.log("Successfully left Wintertodt game room");
                    walkToWintertodtBank = true;
                } else {
                    CupidBot.log("Still attempting to leave Wintertodt game room...");
            ...
            if (tilesToWintertodBank < FAR_FROM_BANK_THRESHOLD && tilesToWintertodBank > 0) {
                CupidBot.log("Already close to Wintertodt - only walking to Wintertodt required (tilesToWintertodBank: " + tilesToWintertodBank + ")");
                walkToWintertodtBank = true;
            }
            ...
            if (walkToWintertodtBank) {
                ...
                if (sleepUntilTrue(this::isAtWintertodt, 1000, 180000)) { // 3 minute for walking
                    ...
                    CupidBot.log("Successfully walked to Wintertodt");
                    return true;
                } else {
                    CupidBot.log("Timeout while walking to Wintertodt - may still be travelling");
                }
            }
            ...
            if (hasGamesNecklace()) {
                ...
                CupidBot.log("Found Games necklace - using direct teleport to Wintertodt");
                ...
                if (sleepUntilTrue(this::isAtWintertodt, 1000, 20000)) { // 20 seconds for teleport
                    ...
                    CupidBot.log("Successfully teleported to Wintertodt");
                    return true;
                }
            }
            ...
            if (!handleBankingPhase(tilesToWintertodBank)) {
                CupidBot.log("Failed to complete banking phase");
            ...
            CupidBot.log("Using Rs2Walker to navigate to Wintertodt - will automatically use best route and teleports");
            ...
            if (sleepUntilTrue(this::isAtWintertodt, 1000, 240000)) { // 4 minute timeout
                ...
                CupidBot.log("Successfully navigated to Wintertodt using webwalker");
                return true;
            } else {
                CupidBot.log("Timeout while navigating to Wintertodt - may still be travelling");
                return false;
            }

        } catch (Exception e) {
            CupidBot.log("Error during navigation: " + e.getMessage());
            e.printStackTrace();
            return false;
        }
    }
        ...
        try {
            ...
            for (int gamesNecklaceId : GAMES_NECKLACE_ITEMS) {
                if (Rs2Inventory.hasItem(gamesNecklaceId)) {
                    CupidBot.log("Found Games necklace (ID: " + gamesNecklaceId + ") in inventory");
                    return true;
                }
            }
            ...
            for (int gamesNecklaceId : GAMES_NECKLACE_ITEMS) {
                if (Rs2Equipment.isWearing(gamesNecklaceId)) {
                    CupidBot.log("Found Games necklace (ID: " + gamesNecklaceId + ") equipped");
                    return true;
                }
            }
            ...
            CupidBot.log("Error checking for Games necklace: " + e.getMessage());
            return false;
        }
    }
    ...
    private boolean handleBankingPhase(int tilesToWintertodBank) {
        try {
            ...
            if (tilesToWintertodBank < tilesToClosestBank + 50) {
                CupidBot.log("Its faster to walk to Wintertodt than to walk other bank.");
                return true;
            }

            if (!Rs2Bank.walkToBank()) {
                CupidBot.log("Failed to reach bank, maybe missclicked");
            ...
            if (!bankUnnecessaryItems()) {
                CupidBot.log("Failed to bank items");
                return false;
            }
            ...
            if (!ensureTeleportItems()) {
                CupidBot.log("Failed to get teleport items");
                return false;
            }
            ...
            CupidBot.log("Error in banking phase: " + e.getMessage());
            return false;
        }
    }
    ...
    private boolean bankUnnecessaryItems() {
        try {
            ...
            CupidBot.log("Error banking unnecessary items: " + e.getMessage());
            return false;
        }
    }
    ...
    private boolean ensureTeleportItems() {
        try {
            ...
            if (!hasGamesNecklace) {
                CupidBot.log("No Games necklace available - getting emergency teleports");
            ...
            CupidBot.log("Error ensuring teleport items: " + e.getMessage());
            return false;
        }
    }
    ...
    public void reset() {
        ...
        CupidBot.log("Location manager state reset - ready for fresh navigation");
    }
