package net.runelite.client.plugins.cupidbot.blastoisefurnace;
...
import net.runelite.client.plugins.cupidbot.blastoisefurnace.enums.Bars;
import net.runelite.client.plugins.cupidbot.blastoisefurnace.enums.State;
...
public class BlastoiseFurnaceScript extends Script {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if (!Rs2GameObject.exists(BLAST_FURNACE_DISPENSER)) {
                    ...
                    CupidBot.getRs2TileObjectCache().query().interact(DWARF_KELDAGRIM_FACTORY_STAIRS);
                    return;
                }

                if (!init) {
                    int inCoffer = CupidBot.getVarbitValue(BLAST_FURNACE_COFFER);
                    ...
                    case BANKING:
                        ...
                        if (config.getBars().isRequiresCoalBag() && !Rs2Inventory.contains(coalBag)) {
                            if (!Rs2Bank.hasItem(coalBag)) {
                                CupidBot.showMessage("No coal bag found in inventory and bank.");
                        ...
                        if (config.getBars().isRequiresGoldsmithGloves()) {
                            ...
                            if (!hasGauntlets) {
                                if (!Rs2Bank.hasItem(GAUNTLETS_OF_GOLDSMITHING)) {
                                    CupidBot.showMessage("No goldsmith gauntlets found.");
                                    CupidBot.stopPlugin(plugin);
                                    return;
                                }
                        ...
                        if (CupidBot.getClient().getEnergy() < 8100) {
                            useStaminaPotions();
                        }
    ...
    private void handleTax() {
        ...
        var blastie = CupidBot.getRs2NpcCache().query().withName("Blast Furnace Foreman").nearestOnClientThread();
    ...
    private void handleDispenserLooting() {
        if (!Rs2Inventory.isFull()) {
            ...
            if (!Rs2Equipment.isWearing(ICE_GLOVES) && !Rs2Equipment.isWearing(SMITHING_UNIFORM_GLOVES_ICE)) {
                ...
                if (!equipped) {
                    CupidBot.showMessage("Ice gloves or smith gloves required to loot the hot bars.");
                    Rs2Player.logout();
                    CupidBot.stopPlugin(plugin);
                    return;
                }
            ...
            CupidBot.getRs2TileObjectCache().query().interact(BLAST_FURNACE_DISPENSER, "Take");
    ...
    private void retrieveItemsForCurrentFurnaceInteraction() {
        ...
        final int coal = CupidBot.getVarbitValue(
    ...
    private boolean putOreOnConveyorBelt() {
        ...
        if (!CupidBot.getRs2TileObjectCache().query().interact(BLAST_FURNACE_CONVEYER_BELT_CLICKABLE, "Put-ore-on")) {
    ...
    public void setTenMinuteTimer(){
        ...
        TimerTask task = new TimerTask() {
            @Override
            public void run() {
                CupidBot.log("Time's up! 10 minutes have passed.");
                timeIsUp = true;
                timerStarted = false;
                timer.cancel();
            }
        ...
        CupidBot.log("Timer started for 10 minutes...");
    ...
    private int fallbackBar(int primary) {
        int value = CupidBot.getVarbitValue(primary);
        return value > 0 ? value : CupidBot.getVarbitValue(net.runelite.api.gameval.VarbitID.BLAST_FURNACE_GOLD_BARS);
    }

    private int getBars(int varbitId) {
        return CupidBot.getVarbitValue(varbitId);
    }
    ...
    public boolean dispenserContainsBars() {
        return Arrays.stream(new int[]{
                ...
                BLAST_FURNACE_ADAMANTITE_BARS,
                BLAST_FURNACE_RUNITE_BARS
        }).anyMatch(id -> CupidBot.getVarbitValue(id) > 0);
    }
    ...
    public void checkAndTopOffCoffer() {
        int lvl = CupidBot.getClient().getRealSkillLevel(Skill.SMITHING);
        if (lvl < 60) return;
        ...
        CupidBot.getRs2TileObjectCache().query().interact(BLAST_FURNACE_AUTOMATA_COFFER, "use");
    ...
    public boolean fullCoffer() {
        int coffer = CupidBot.getVarbitValue(BLAST_FURNACE_COINSINCOFFER);
        return coffer == 1;
    }
}
