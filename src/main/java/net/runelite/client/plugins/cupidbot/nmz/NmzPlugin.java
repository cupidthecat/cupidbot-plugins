...
@PluginDescriptor(
        ...
        cardUrl = "NmzPlugin/assets/card.png",
        iconUrl = "NmzPlugin/assets/icon.png",
        tags = {"nmz", "cupidbot"},
...
public class NmzPlugin extends Plugin {
    ...
    public void onActorDeath(ActorDeath actorDeath) {
        if (config.stopAfterDeath() && actorDeath.getActor() == CupidBot.getClient().getLocalPlayer()) {
            CupidBot.getClientThread().runOnSeperateThread(() -> {
                Global.sleepUntil(nmzScript::isOutside, 10000);
                CupidBot.stopPlugin(this);
                return true;
            });
        }
    }
