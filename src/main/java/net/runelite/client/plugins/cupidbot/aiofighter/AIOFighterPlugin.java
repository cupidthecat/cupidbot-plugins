package net.runelite.client.plugins.cupidbot.aiofighter;
...
import net.runelite.client.plugins.cupidbot.aiofighter.bank.BankerScript;
import net.runelite.client.plugins.cupidbot.aiofighter.combat.*;
import net.runelite.client.plugins.cupidbot.aiofighter.enums.PrayerStyle;
import net.runelite.client.plugins.cupidbot.aiofighter.enums.State;
import net.runelite.client.plugins.cupidbot.aiofighter.loot.LootScript;
import net.runelite.client.plugins.cupidbot.aiofighter.safety.SafetyScript;
import net.runelite.client.plugins.cupidbot.aiofighter.shop.ShopScript;
import net.runelite.client.plugins.cupidbot.aiofighter.skill.AttackStyleScript;
...
@PluginDescriptor(
        ...
        description = "CupidBot AIO Fighter plugin",
        tags = {"fight", "cupidbot", "misc", "combat", "playerassistant"},
        cardUrl = "AIOFighterPlugin/assets/card.png",
        iconUrl = "AIOFighterPlugin/assets/icon.png",
...
public class AIOFighterPlugin extends Plugin {
    ...
    public static void clearWaitForLoot(String reason) {
        ...
        if (reason != null) {
            CupidBot.log("Clearing wait-for-loot state: " + reason);
        }
    }
    ...
    protected void startUp() throws AWTException {
        ...
        if (!config.toggleCenterTile() && CupidBot.isLoggedIn() && !config.slayerMode())
    ...
    private void applySpecialAttackConfig() {
        if (config.useSpecialAttack() && config.specWeapon() != null) {
            CupidBot.getSpecialAttackConfigs()
    ...
    private static <T> void setConfig(String key, T value) {
        CupidBot.getConfigManager().setConfiguration(AIOFighterConfig.GROUP, key, value);
    }

    private static <T> T getConfig(String key, Class<T> type) {
        return CupidBot.getConfigManager().getConfiguration(AIOFighterConfig.GROUP, key, type);
    }
    ...
    public static void setCurrentSlayerInventorySetup(InventorySetup currentInventorySetup) {
        CupidBot.log("Setting current inventory setup to: " + currentInventorySetup.getName());
    ...
    public static String getNpcAttackList() {
        return CupidBot.getConfigManager().getConfiguration(AIOFighterConfig.GROUP, "monster");
    }
    ...
    public void onHitsplatApplied(HitsplatApplied event){
        try {
            if (event.getActor() != CupidBot.getClient().getLocalPlayer()) return;
    ...
    public void onMenuOpened(MenuOpened event) {
        lastMenuOpenedPoint = CupidBot.getClient().getMouseCanvasPosition();
        trueTile = getSelectedWorldPoint();
    }
    @Subscribe
    private void onMenuEntryAdded(MenuEntryAdded event) {
        if (CupidBot.getClient().isKeyPressed(KeyCode.KC_SHIFT) && event.getOption().equals(WALK_HERE) && event.getTarget().isEmpty() && config.toggleCenterTile()) {
            addMenuEntry(event, SET, CENTER_TILE, 1);
        }
        if (CupidBot.getClient().isKeyPressed(KeyCode.KC_SHIFT) && event.getOption().equals(WALK_HERE) && event.getTarget().isEmpty()) {
    ...
    private WorldPoint getSelectedWorldPoint() {
        if (CupidBot.getClient().getWidget(InterfaceID.Worldmap.MAP_CONTAINER) == null) {
            if (CupidBot.getClient().getTopLevelWorldView().getSelectedSceneTile() != null) {
                return CupidBot.getClient().getTopLevelWorldView().isInstance() ?
                        WorldPoint.fromLocalInstance(CupidBot.getClient(), CupidBot.getClient().getTopLevelWorldView().getSelectedSceneTile().getLocalLocation()) :
                        CupidBot.getClient().getTopLevelWorldView().getSelectedSceneTile().getWorldLocation();
            }
        } else {
            return calculateMapPoint(CupidBot.getClient().isMenuOpen() ? lastMenuOpenedPoint : CupidBot.getClient().getMouseCanvasPosition());
        }
        return null;
    }
    public WorldPoint calculateMapPoint(Point point) {
        WorldMap worldMap = CupidBot.getClient().getWorldMap();
    ...
    public Point mapWorldPointToGraphicsPoint(WorldPoint worldPoint) {
        ...
        Widget map = CupidBot.getClient().getWidget(ComponentID.WORLD_MAP_MAPVIEW);
        if (map != null) {
    ...
    private void addMenuEntry(MenuEntryAdded event, String option, String target, int position) {
        List<MenuEntry> entries = new LinkedList<>(Arrays.asList(CupidBot.getClient().getMenu().getMenuEntries()));

        if (entries.stream().anyMatch(e -> e.getOption().equals(option) && e.getTarget().equals(target))) {
            return;
        }

        CupidBot.getClient().createMenuEntry(position)
                .setOption(option)
                .setTarget(target)
