...
public class GiantSeaweedFarmerScript extends Script {
    ...
    private void getToFossilIsland() {
        ...
        if (config.DigsiteInHouse()) {
            if (Rs2Inventory.hasItem("Farming cape", false) || Rs2Inventory.hasItem("Farming cape(t)", false)) {
                ...
                sleep(550, 750);
                CupidBot.getRs2TileObjectCache().query().interact(FARMGUILD_SPIRITTREE, "Travel");
                sleep(550, 750);
    ...
    private void handleBanking() {
        ...
        if (!readyToFarm) {
            ...
            CupidBot.log(missingItemsStr + " - shutting down");
            shutdownSequence();
        }
        BankSuccess = true;
    }
    ...
    private void handleDiving() {
        CupidBot.getRs2TileObjectCache().query().interact(BOAT, "Dive");
        sleepUntil(() -> Rs2Player.getWorldLocation().getPlane() == 1, 5000);
        if (Rs2Player.getWorldLocation().getPlane() != 1) {
            CupidBot.log("We failed to get underwater - Make sure to handle the warning dialog manually once");
            shutdownSequence();
            return;
        }
    ...
    private void handleFarmNull(){
        if (config.modeType().equals(GiantSeaweedFarmerConfig.modeType.RUN_ONCE)) {
            CupidBot.log("Finished farming, stopping...");
            shutdownSequence();
            return;
        }
        if (config.modeType().equals(GiantSeaweedFarmerConfig.modeType.RUN_ONCE_THEN_BANK)) {
            ...
            CupidBot.log("Finished farming, returning to wreck");
            return;
        }
    ...
    private boolean handlePatch(int patchId) {
        ...
        var objModel = CupidBot.getRs2TileObjectCache().query().withId(patchId).nearest();

        if (objModel == null) {
            objModel = CupidBot.getRs2TileObjectCache().query().where(o -> {
        ...
        switch (state) {
            ...
            case "Dead":
                ...
                return false;
            case "Diseased":
                CupidBot.showMessage("Diseased patch! Please turn off the script and then cure me manually as i cant do this automatically yet.");
                return false;
            case "Growing":
                return true;
            default:
    ...
    private void returnToBank() {
        ...
        if (Rs2Player.getWorldLocation().getPlane() == 1) {
            ...
            sleep(300, 500);
            CupidBot.getRs2TileObjectCache().query().interact(UNDERWATER_ANCHOR, "Climb");
            sleepUntil(() -> Rs2Player.getWorldLocation().getPlane() == 0, 7000);
        }


        if (Rs2Player.getWorldLocation().getPlane() != 0) {
            CupidBot.log("We failed to get back to the surface");
        }
        else {
            CupidBot.log("We're back at the bankt");
            if (config.modeType().equals(GiantSeaweedFarmerConfig.modeType.RUN_ONCE_THEN_BANK)) {
                shutdownSequence();
            }
        }
    ...
    private void shutdownSequence(){
        CupidBot.stopPlugin(giantSeaweedPlugin);
    }
