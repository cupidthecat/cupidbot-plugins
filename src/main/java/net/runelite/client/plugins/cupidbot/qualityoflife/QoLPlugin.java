package net.runelite.client.plugins.cupidbot.qualityoflife;
...
import net.runelite.client.plugins.cupidbot.CupidBotPlugin;
...
import net.runelite.client.plugins.cupidbot.qualityoflife.enums.WintertodtActions;
import net.runelite.client.plugins.cupidbot.qualityoflife.managers.CraftingManager;
import net.runelite.client.plugins.cupidbot.qualityoflife.managers.FiremakingManager;
import net.runelite.client.plugins.cupidbot.qualityoflife.managers.FletchingManager;
import net.runelite.client.plugins.cupidbot.qualityoflife.managers.GemCuttingManager;
import net.runelite.client.plugins.cupidbot.qualityoflife.scripts.*;
import net.runelite.client.plugins.cupidbot.qualityoflife.scripts.bank.BankpinScript;
import net.runelite.client.plugins.cupidbot.qualityoflife.scripts.pvp.PvpScript;
import net.runelite.client.plugins.cupidbot.qualityoflife.scripts.wintertodt.WintertodtOverlay;
import net.runelite.client.plugins.cupidbot.qualityoflife.scripts.wintertodt.WintertodtScript;
...
import static net.runelite.client.plugins.cupidbot.qualityoflife.scripts.wintertodt.WintertodtScript.isInWintertodtRegion;
import static net.runelite.client.plugins.cupidbot.util.Global.awaitExecutionUntil;
...
public class QoLPlugin extends Plugin implements KeyListener {
    ...
    private static final AtomicBoolean loggedCupidBotPluginCacheFailure = new AtomicBoolean(false);
    private static final AtomicBoolean loggedCupidBotPluginFallbackScan = new AtomicBoolean(false);

    private static final AtomicReference<QoLPlugin> selfRef = new AtomicReference<>();
    private static final AtomicReference<CupidBotPlugin> cupidbotPluginRef = new AtomicReference<>();
    ...
    private static CupidBotPlugin findCupidBotPluginOnce()
    {
        CupidBotPlugin cached = cupidbotPluginRef.get();
        if (cached != null)
        {
            return cached;
        }

        CupidBotPlugin found = (CupidBotPlugin) CupidBot.getPluginManager().getPlugins().stream()
                .filter(plugin -> plugin instanceof CupidBotPlugin)
                .findAny().orElse(null);
        if (found != null)
        {
            cupidbotPluginRef.compareAndSet(null, found);
        }
        return found;
    }
    ...
    protected void startUp() throws AWTException {
        selfRef.set(this);
        // Cache CupidBotPlugin once (avoid scanning plugin list in steady-state UI updates).
        try
        {
            findCupidBotPluginOnce();
        }
        catch (Exception ex)
        {
            // Best-effort cache; updateUiElements() can still fall back if needed.
            if (loggedCupidBotPluginCacheFailure.compareAndSet(false, true))
            {
                log.debug("QoL: failed to cache CupidBotPlugin during startup", ex);
            }
        }
        ...
        if (config.useSpecWeapon()) {
            ...
            CupidBot.getSpecialAttackConfigs().setSpecialAttackWeapon(config.specWeapon());
            CupidBot.getSpecialAttackConfigs().setMinimumSpecEnergy(config.specWeapon().getEnergyRequired());
        }
        if (config.autoRun()) {
            CupidBot.enableAutoRunOn = true;
        }
        if (config.autoStamina()) {
            CupidBot.useStaminaPotsIfNeeded = true;
            CupidBot.runEnergyThreshold = config.staminaThreshold() * 100;
        }
    ...
    public void onGameTick(GameTick event) {
        ...
        if (config.useSpecWeapon()) {
            if (CupidBot.getSpecialAttackConfigs().getSpecialAttackWeapon() != config.specWeapon()) {
                CupidBot.getSpecialAttackConfigs().setSpecialAttack(true);
                CupidBot.getSpecialAttackConfigs().setSpecialAttackWeapon(config.specWeapon());
                CupidBot.getSpecialAttackConfigs().setMinimumSpecEnergy(config.specWeapon().getEnergyRequired());
            }
    ...
    public void onHitsplatApplied(HitsplatApplied event) {
        if ((config.resumeFletchingKindling() || config.resumeFeedingBrazier())) {
            if (event.getActor() == CupidBot.getClient().getLocalPlayer()) {
    ...
    private void onMenuOptionClicked(MenuOptionClicked event) {
        ...
        if ((config.resumeFletchingKindling() || config.resumeFeedingBrazier()) && isInWintertodtRegion()) {
            if (event.getMenuOption().contains("Fletch") && event.getMenuTarget().isEmpty() && config.resumeFletchingKindling()) {
                ...
                CupidBot.log("Setting action to Fletch Kindle");
            }
        ...
        if (config.smartWorkbench() && event.getMenuOption().contains("Smart Work-at") && event.getMenuEntry().getIdentifier() == ObjectID.WORKBENCH_43754) {
            if (Rs2Inventory.anyPouchEmpty() && Rs2Inventory.hasItem(ItemID.GUARDIAN_ESSENCE)) {
                ...
                CupidBot.getClientThread().runOnSeperateThread(() -> {
                    ...
                    CupidBot.getRs2TileObjectCache().query().interact(ObjectID.WORKBENCH_43754);
                    return null;
                });
        ...
        if (config.smartGotrMine() && event.getMenuOption().contains("Smart Mine") && event.getMenuEntry().getIdentifier() == ObjectID.HUGE_GUARDIAN_REMAINS) {
            if (Rs2Inventory.anyPouchEmpty() && Rs2Inventory.hasItem(ItemID.GUARDIAN_ESSENCE)) {
                ...
                CupidBot.getClientThread().runOnSeperateThread(() -> {
                    Rs2Inventory.fillPouches();
                    CupidBot.getRs2TileObjectCache().query().interact(ObjectID.HUGE_GUARDIAN_REMAINS);
                    return null;
                });
            ...
            if (Rs2Inventory.anyPouchFull()) {
                CupidBot.getClientThread().runOnSeperateThread(() -> {
                    ...
                    Global.sleepUntil(() -> !Rs2Inventory.anyPouchFull(), () -> {
                                ...
                                CupidBot.getClientThread().invoke(() -> CupidBot.getRs2TileObjectCache().query().withName("Altar").interact());
                                Rs2Inventory.waitForInventoryChanges(3000);
                            }
                            , 10000, 200);
                    return null;
    ...
    private void handleMenuOptionClicked(MenuEntry menuEntry, List<NewMenuEntry> menuEntriesList, String... stopOptions) {
        ...
        if (Arrays.stream(stopOptions).anyMatch(menuEntry.getOption()::contains)) {
            recordActions = false;
            CupidBot.log("<col=5F1515>Stopped recording actions</col>");
        }
    }
    ...
    private void onMenuEntryAdded(MenuEntryAdded event) {
        ...
        if (config.useDoLastBank()) {
            if ("Talk-to".equals(option)) {
                for (MenuEntry e : CupidBot.getClient().getMenuEntries()) {
    ...
    private void customHaProfitOnClicked(MenuEntry entry) {
        ...
        CupidBot.getMouse().click(CupidBot.getClient().getMouseCanvasPosition(), highAlch);
        Global.sleep(Rs2Random.randomGaussian(150, 50));
        CupidBot.getMouse().click(CupidBot.getClient().getMouseCanvasPosition(), highAlchItem);
    ...
    public void onConfigChanged(ConfigChanged ev) {
        if ("smoothRotation".equals(ev.getKey()) && config.smoothCameraTracking() && CupidBot.isLoggedIn()) {
            previousCamera[YAW_INDEX] = CupidBot.getClient().getCameraYawTarget();
        }
        ...
        if (ev.getKey().equals("autoRun")) {
            CupidBot.enableAutoRunOn = config.autoRun();
        }

        if (ev.getKey().equals("autoStamina")) {
            CupidBot.useStaminaPotsIfNeeded = config.autoStamina();
        }
    ...
    public void onBeforeRender(BeforeRender render) {
        if (!CupidBot.isLoggedIn() || !config.smoothCameraTracking()) {
            return;
        }
    ...
    private void customBankingOnClicked(MenuEntry event) {
        ...
        if (bankMenuEntries.isEmpty()) {
            CupidBot.log("<col=5F1515>No actions recorded</col>");
            return;
        }
        CupidBot.log("<col=245C2D>Banking</col>");
    ...
    private void customTrackOnClicked(MenuEntry event) {
        if (Rs2Camera.isTrackingNpc()) {
            ...
            CupidBot.log("<col=5F1515>Stopped tracking old NPC, try again to track new NPC</col>");
            return;
        }
    ...
    private void customFurnaceOnClicked(MenuEntry event) {
        ...
        CupidBot.log("<col=245C2D>Furnace</col>");
    ...
    private void recordNewActions(MenuEntry event) {
        ...
        CupidBot.log("<col=245C2D>Recording actions for: </col>" + option);
    }
    ...
    private void customCookingOnClicked(MenuEntry event) {
        CupidBot.getClientThread().runOnSeperateThread(() -> {
            ...
            CupidBot.log("<col=245C2D>Cooking</col>");
    ...
    private void quickTeleportToHouse(MenuEntry entry) {
        ...
        if (!Rs2Magic.hasRequiredRunes(Rs2Spells.TELEPORT_TO_HOUSE)) {
            CupidBot.log("<col=5F1515>Missing Required Runes</col>");
            return;
        }

        CupidBot.log("<col=245C2D>Casting: </col>Teleport to House");
        CupidBot.getClientThread().runOnSeperateThread(() -> {
    ...
    private void applySmoothingToAngle(int index) {
        int currentAngle = index == YAW_INDEX ? CupidBot.getClient().getCameraYawTarget() : 0;
        ...
        if (index == YAW_INDEX) {
            CupidBot.getClient().setCameraYawTarget(changed);
        }
    ...
    private void addMenuEntry(MenuEntryAdded event, String option, String target, Consumer<MenuEntry> callback) {
        int index = CupidBot.getClient().getMenuEntries().length;
        CupidBot.getClient().createMenuEntry(index)
                .setOption(option)
                .setTarget(target)
    ...
    private void addLoadoutMenuEntry(MenuEntryAdded event, String option, String target, Consumer<MenuEntry> callback) {
        int index = CupidBot.getClient().getMenuEntries().length - 1;
        CupidBot.getClient().createMenuEntry(index)
                .setOption(option)
                .setTarget(target)
                .setParam0(event.getActionParam0())
    ...
    private boolean updateUiElements() {
        ...
        try {
            ...
            CupidBotPlugin cupidbotPlugin = findCupidBotPluginOnce();
            if (cupidbotPlugin == null && loggedCupidBotPluginFallbackScan.compareAndSet(false, true))
            {
                log.debug("QoL: CupidBotPlugin ref was empty; fell back to plugin manager scan.");
            }

            // If ConfigPlugin is not found, log an error and return false
            if (cupidbotPlugin == null) {
                CupidBot.log("Config Plugin not found");
                return false;
            }
            cupidbotPluginRef.set(cupidbotPlugin);

            // Get the plugin list panel from the ConfigPlugin instance
            JPanel pluginListPanel = getPluginListPanel(cupidbotPlugin);
            ...
            if (currentPluginList == null) {
                CupidBot.log("Plugin list is null, waiting for it to be initialized");
                return false;
            }
    ...
    private static void restoreUiElements()
    {
        ...
        {
            QoLPlugin self = selfRef.get();
            CupidBotPlugin cupidbotPlugin = cupidbotPluginRef.get();
            if (self != null && cupidbotPlugin != null)
            {
                JPanel pluginListPanel = self.getPluginListPanel(cupidbotPlugin);
    ...
    private JPanel getPluginListPanel(CupidBotPlugin cupidbotPlugin) throws ClassNotFoundException {

        Class<?> pluginListPanelClass = Class.forName("net.runelite.client.plugins.cupidbot.ui.CupidBotPluginListPanel");
        if (cupidbotPlugin == null)
        {
            throw new IllegalStateException("CupidBotPlugin instance is null");
        }
        return (JPanel) cupidbotPlugin.getInjector().getProvider(pluginListPanelClass).get();
    }
            ...
            try {
                ...
                if (clipboardText != null && !clipboardText.isEmpty()) {
                    CupidBot.getClientThread().invoke(() -> {
                        CupidBot.getClient().setVarcStrValue(VarClientID.MESLAYERINPUT, clipboardText);
                        CupidBot.getClient().runScript(ScriptID.GE_ITEM_SEARCH);
                    });
                }
            } catch (Exception ex) {
                CupidBot.log("Failed to paste from clipboard: " + ex.getMessage());
            }
        }
    }
