package net.runelite.client.plugins.cupidbot.frostyrc;
...
import net.runelite.client.plugins.cupidbot.frostyrc.enums.RuneType;
import net.runelite.client.plugins.cupidbot.frostyrc.enums.State;
import net.runelite.client.plugins.cupidbot.frostyrc.enums.Teleports;
...
public class RcScript extends Script {
    ...
    public boolean run() {
        ...
        state = State.BANKING;
        CupidBot.log("Script has started");
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if (lumbyElite == -1) {
                    clientThread.invoke(() -> {
                        lumbyElite = CupidBot.getClient().getVarbitValue(Varbits.DIARY_LUMBRIDGE_ELITE);
                    });
                    return;
                }
                ...
                CupidBot.log("Error in script" + ex.getMessage());
            }
    ...
    public void shutdown() {
        Rs2Antiban.resetAntibanSettings();
        super.shutdown();
        CupidBot.log("Script has been stopped");
        //Rs2Player.logout();
    }
    ...
    private void handleBanking() {
        ...
        if (!Rs2Inventory.allPouchesFull() && !Rs2Inventory.contains(pureEss)) {
            if (!Teleports.CRAFTING_CAPE.matchesRegion(currentRegion)
                ...
                CupidBot.log("Not in banking region, teleporting");
                handleBankTeleport();
            }
        }
        ...
        if (Rs2Inventory.isFull() && Rs2Inventory.allPouchesFull() && Rs2Inventory.contains(pureEss)) {
            CupidBot.log("We are full, skipping bank");
        ...
        if (config.runeType() == RuneType.BLOOD) {
            if (!config.usePoh() && lumbyElite != 1) {
                if (!Rs2Equipment.isWearing(lunarStaff)) {
                    CupidBot.log("Looking for and withdrawing lunar staff");
                    ...
                    CupidBot.log("No lunar staff found, withdrawing dramen staff");
            ...
            if (!Rs2Inventory.contains(activeBloodEssence) && !Rs2Inventory.contains(inactiveBloodEssence)) {
                if (!Rs2Bank.hasItem(activeBloodEssence)) {
                    ...
                    CupidBot.log("Withdrawing blood essence");
                    sleepGaussian(900, 200);
                } else {
            ...
            for (Teleports bankTeleport : bankTeleports) {
                for (Integer bankTeleportID : bankTeleport.getItemIds()) {
                    if (Rs2Equipment.isWearing(bankTeleportID) || Rs2Inventory.contains(Teleports.HOUSE_TAB.getItemIds())) {
                        ...
                        CupidBot.log("Withdrawing bank teleport " + bankTeleport.getName());
                        ...
                        if (!Rs2Bank.hasItem(bankTeleportID)) {
                            CupidBot.log("Withdrawing all house tabs");
                ...
                if (hasBankTeleport) {
                    CupidBot.log("We have a bank teleport: " + bankTeleport.getName());
                    break;
                }
            }
        }
        ...
        if (!Rs2Equipment.isWearing("Ring of dueling") && Rs2Bank.hasItem("Ring of dueling")) {
            CupidBot.log("Withdrawing ring of dueling");
    ...
    private void handleFillPouch() {
        while (!Rs2Inventory.allPouchesFull() || !Rs2Inventory.isFull() && isRunning()) {
            CupidBot.log("Pouches are not full yet");
            if (Rs2Bank.isOpen()) {
    ...
    private void handleFeroxRunEnergy() {
		if (forceDrinkAtFerox || Rs2Player.getRunEnergy() <= 15 || Rs2Player.getHealthPercentage() <= 20) {
			CupidBot.log("We are thirsty...let us Drink");
            forceDrinkAtFerox = true;
            if (plugin.getMyWorldPoint().distanceTo(feroxPoolWp) > 5) {
                CupidBot.log("Walking to Ferox pool");
                Rs2Walker.walkTo(feroxPoolWp);
                sleepUntil(() -> plugin.getMyWorldPoint().distanceTo(feroxPoolWp) < 5);
            }

            if (plugin.getMyWorldPoint().distanceTo(feroxPoolWp) < 5) {
                CupidBot.log("Interacting with the Ferox pool");
                CupidBot.getRs2TileObjectCache().query().interact(feroxPool, "Drink");
            }
    ...
    private void handleArdyCloak() {
        ...
        for (Integer itemId : ardyCloakTeleport.getItemIds()) {
            if (Rs2Equipment.isWearing(itemId)) {
                CupidBot.log("Using Ardy cloak");
                Rs2Equipment.interact(itemId, ardyCloakTeleport.getInteraction());
                CupidBot.log("Waiting for region " + monasteryRegion);
        ...
        if (plugin.getMyWorldPoint().distanceTo(monasteryFairyRing) > 7) {
            CupidBot.log("Walking to monastery fairy ring");
            Rs2Walker.walkTo(monasteryFairyRing);
            sleepUntil(() -> plugin.getMyWorldPoint().distanceTo(monasteryFairyRing) < 7);
            sleepGaussian(900, 200);
        }

        Rs2TileObjectModel fairyRing = CupidBot.getRs2TileObjectCache().query()
                .where(obj -> {
        ...
        if (plugin.getMyWorldPoint().distanceTo(monasteryFairyRing) < 7) {
            if (fairyRing == null) {
                CupidBot.log("Unable to find fairies, resetting from bank to retry");
                state = State.BANKING;
                return;
            } else {
                CupidBot.log("Interacting with fairies");
        ...
        if (config.usePoh()) {
            if (Rs2Equipment.isWearing(Arrays.toString(farmingCapeTeleport.getItemIds()))) {
                ...
                if (plugin.getMyWorldPoint().distanceTo(guildSpiritTreeLoc) > 10) {
                    ...
                    CupidBot.getRs2TileObjectCache().query().interact(guildSpiritTree, "Travel");
                    ...
                    if (parent != null && parent.getChildren() != null) {
                        for (Widget child : parent.getChildren()) {
                            if (child != null && child.getText() != null && child.getText().toLowerCase().contains("house")) {
                                CupidBot.log("Found house widgetId");
                                Rs2Widget.clickWidget(child);
                                sleepUntil(() -> !Rs2Player.isAnimating(), 5000);
                                sleepUntil(() -> CupidBot.getClient().getTopLevelWorldView() != null, 5000);
                    ...
                    if (Rs2Player.getRunEnergy() < 45) {
                        sleepGaussian(700, 200);
                        CupidBot.log("We are thirsty..let us Drink");
                        Rs2TileObjectModel poolObj = CupidBot.getRs2TileObjectCache().query().withIds(29241, 29240, 29239, 29238, 29237).nearest();
                        if (poolObj != null) {
                            poolObj.click("Drink");
                    ...
                    if (Rs2Player.getRunEnergy() > 45) {
                        if (config.runeType() == RuneType.BLOOD) {
                            sleepGaussian(700, 200);
                            CupidBot.log("Looking for fairies");
                            handlePohFairyRing();
                        }
                    }
                }
    ...
    private void handleWrathWalking() {
        ...
        if (Rs2Inventory.contains(mythCape)) {
            CupidBot.log("Interacting with myth cape");
			...
			Rs2TileObjectModel statue = CupidBot.getRs2TileObjectCache().query().withName("Mythic Statue").nearestOnClientThread();
			if (statue != null && !Rs2Player.isAnimating()) {
				statue.click("Teleport");
			}

            if (plugin.getMyWorldPoint().getRegionID() == mythicStatueRegion) {
                CupidBot.log("Walking to Wrath ruins");
                Rs2Walker.walkTo(outsideWrathRuins);
                sleepUntil(() -> plugin.getMyWorldPoint().getRegionID() == mythicStatueRegion);
                CupidBot.log("Current position " + plugin.getMyWorldPoint());

                if (plugin.getMyWorldPoint() == outsideWrathRuins) {
                    CupidBot.getRs2TileObjectCache().query().interact(wrathRuins, "Enter");
        ...
        if (config.runeType() == RuneType.WRATH && Rs2Player.getRunEnergy() > 90) {
            ...
            for (Integer itemId : homeTeleports.getItemIds()) {
                if (Rs2Inventory.contains(itemId)) {
                    ...
                    if (pohPortal != null) {
                        CupidBot.log("Poh portal found, we are home");
                    }
                    CupidBot.log("We should be in poh fully loaded");
                }
            }
    ...
    private void handlePohFairyRing() {
        if (CupidBot.getRs2TileObjectCache().query().withId(ObjectID.POH_FAIRY_RING).nearest() != null) {
            CupidBot.getRs2TileObjectCache().query().interact(ObjectID.POH_FAIRY_RING, "Last-destination (DLS)");
            CupidBot.log("Using fairy ring");
            Rs2Player.waitForAnimation(1200);
            sleepUntil(() -> plugin.getMyWorldPoint().equals(caveFairyRing), 1200);
            state = State.WALKING_TO;
        } else {
            Rs2TileObjectModel pohTreeRing = CupidBot.getRs2TileObjectCache().query()
                    .where(obj -> {
                        String name = obj.getName();
            ...
            if (pohTreeRing != null) {
                ...
                CupidBot.log("Using fairy tree");
                Rs2Player.waitForAnimation();
                sleepUntil(() -> plugin.getMyWorldPoint().equals(caveFairyRing));
            } else {
                CupidBot.log("Unable to find fairy ring, resetting to banking for a retry");
                state = State.BANKING;
            }
        }
    ...
    private void handleWalking() {
        ...
        if (config.runeType() == RuneType.BLOOD) {
            CupidBot.log("Current location after waiting: " + plugin.getMyWorldPoint());
            if (plugin.getMyWorldPoint().equals(caveFairyRing)) {
                sleepGaussian(900, 200);
                CupidBot.getRs2TileObjectCache().query().interact(16308, "Enter");
            ...
            if (plugin.getMyWorldPoint().equals(firstCaveExit) &&
                ...
                CupidBot.log("Walking to blood ruins " +
                        outsideBloodRuins93);
    ...
    private void handleCrafting() {
        ...
        if (config.runeType() == RuneType.BLOOD) {
            CupidBot.getRs2TileObjectCache().query().interact(bloodRuins, "Enter");
            sleepUntil(() -> !Rs2Player.isAnimating() && plugin.getMyWorldPoint().getRegionID() == bloodAltarRegion);
            sleepGaussian(700, 200);
            CupidBot.getRs2TileObjectCache().query().interact(bloodAltar, "Craft-rune");
            ...
            while (plugin.getMyWorldPoint().getRegionID() == bloodAltarRegion && isRunning()) {
                if (Rs2Inventory.allPouchesEmpty() && !Rs2Inventory.contains("Pure essence")) {
                    CupidBot.log("We are in altar region and out of p ess, banking...");
                    handleBankTeleport();
                    sleepGaussian(500, 200);
                }
            }
        ...
        if (config.runeType() == RuneType.WRATH) {
            CupidBot.log("Entering wrath ruins");
            CupidBot.getRs2TileObjectCache().query().interact(wrathRuins, "Enter");
            sleepUntil(() -> plugin.getMyWorldPoint().getRegionID() == wrathAltarRegion);
            sleepGaussian(1100, 200);
            CupidBot.log("Crafting runes");
            handleEmptyPouch();
        }
    ...
    private void handleEmptyPouch() {
        while (!Rs2Inventory.allPouchesEmpty() && isRunning()) {
            CupidBot.log("Pouches are not empty. Crafting more");
            ...
            if (config.runeType() == RuneType.WRATH) {
                CupidBot.getRs2TileObjectCache().query().interact(wrathAltar, "Craft-rune");
            }
            Rs2Player.waitForXpDrop(Skill.RUNECRAFT);
            plugin.updateXpGained();
        }
        ...
        for (Teleports teleport : bankTeleport) {
            for (Integer bankTeleportsId : teleport.getItemIds()) {
                if (Rs2Equipment.isWearing(bankTeleportsId)) {
                    CupidBot.log("Using: " + teleport.getName());
