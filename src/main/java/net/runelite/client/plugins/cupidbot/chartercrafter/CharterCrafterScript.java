package net.runelite.client.plugins.cupidbot.chartercrafter;
...
public class CharterCrafterScript extends Script {
    ...
    private void bootstrap() {
        ...
        Rs2NpcModel trader = CupidBot.getRs2NpcCache().query().withName(TRADER_NAME).nearestOnClientThread();
        if (trader == null) {
    ...
    private void buyMaterials() {
        ...
        if (remainingSeaweed > 0 || remainingSand > 0) {
            ...
            worldHopAttempts = 0;
            beforeHopWorld = CupidBot.getClient().getWorld();
    ...
    private void worldHop() {
        if (worldHopPending) {
            GameState gs = CupidBot.getClient().getGameState();
        ...
        boolean hopCall = CupidBot.hopToWorld(world);
        if (!hopCall) {
            sleep(800);
        }
    }
