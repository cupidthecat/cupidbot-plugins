package net.runelite.client.plugins.cupidbot.mahoganyhomez;
...
public class MahoganyHomesScript extends Script {
    ...
    private void log(String format, Object... args) {
        if (plugin.getConfig().logMessages()) {
            CupidBot.log(String.format(format, args));
        }
    }
    ...
    private void fix() {
        ...
        if (Rs2Widget.isWidgetVisible(InterfaceID.PohFurnitureCreation.FRAME)){
            CupidBot.log("Out of plank and furniture creation widget pop up");
            Rs2Bank.walkToBank();
            bank();
            return;
        }
    ...
    private void interactWithObject(GameObject object) {
        ...
        if (CupidBot.getRs2TileObjectCache().query().withId(object.getId()).interact(action)) {
            sleepUntil(() -> {
    ...
    private boolean openDoorToObject(GameObject object, Rs2WorldPoint objectLocation) {
        ...
        for (WorldPoint wp : walkerPath) {
            ...
            var doorModel = CupidBot.getRs2TileObjectCache().query().withId(door.getId()).nearest();
        ...
        List<String> doorNames = doors.stream()
                .map(d -> {
                    var m = CupidBot.getRs2TileObjectCache().query().withId(d.getId()).nearest();
...
//        log("Doors found: %s", doors.size());
        ...
        for (TileObject door : doors) {
            var doorObj = CupidBot.getRs2TileObjectCache().query().withId(door.getId()).nearest();
            ...
            if (actions != null && actions.contains("Open")) {
                ...
                if (CupidBot.getRs2TileObjectCache().query().withId(door.getId()).interact("Open")) {
    ...
    private void tryToUseLadder() {
        ...
        var closestLadder = CupidBot.getRs2TileObjectCache().query().withIds(Arrays.stream(plugin.getCurrentHome().getLadders()).mapToInt(Integer::intValue).toArray()).nearest();
    ...
    private void finish() {
        if (plugin.getCurrentHome() != null
            ...
            var npc = CupidBot.getRs2NpcCache().query().withId(plugin.getCurrentHome().getNpcId()).nearest();
            if (npc == null && Rs2Player.getWorldLocation().getPlane() > 0) {
                ...
                var ladders = CupidBot.getRs2TileObjectCache().query()
    ...
    private void getNewContract() {
        if (plugin.getCurrentHome() == null) {
            ...
            if (contractLocation.distanceTo2D(Rs2Player.getWorldLocation()) > 10) {
                ...
                var npc = CupidBot.getRs2NpcCache().query().withNames("Amy", "Marlo", "Ellie", "Angelo").nearestOnClientThread();
                    ...
                    if (plugin.getConfig().usePlankSack()) {
                        ...
                        Global.sleepUntil(() -> planksInPlankSack() == 28, () -> {
                            ...
                            if (plankSack != null) {
                                ...
                                CupidBot.doInvoke(plankSackEntry,Rs2Inventory.itemBounds(plankSack));
                                Rs2Inventory.waitForInventoryChanges(1000);
                            }
                        }, 20000, 1000);
                        ...
                        if (additionalPlanksNeeded <= 0) {
                            ...
                            CupidBot.showMessage("Please free up inventory space! Need " + additionalPlanksNeeded + " more planks but only " + freeSlots + " slots available. Stopping script.");
                            shutdown();
                            return;
                        }
                    }
