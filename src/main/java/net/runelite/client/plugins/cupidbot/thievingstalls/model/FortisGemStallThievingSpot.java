...
public class FortisGemStallThievingSpot implements IStallThievingSpot {
    ...
    public void thieve() {
        ...
        final Rs2TileObjectModel stall = CupidBot.getRs2TileObjectCache().query().withId(STALL_ID).within(SAFESPOT, 2).nearest();
        if (stall == null) {
            boolean started = CupidBot.hopToWorld(Login.getRandomWorld(Rs2Player.isMember()));
            if (started) {
                Global.sleepUntil(CupidBot::isLoggedIn, 15000);
            }
            return;
        }
