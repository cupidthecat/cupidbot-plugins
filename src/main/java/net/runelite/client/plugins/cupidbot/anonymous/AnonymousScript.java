package net.runelite.client.plugins.cupidbot.anonymous;
...
public class AnonymousScript extends Script {
    ...
    public void anonymize(Client client, AnonymousConfig config) {
        if (config.maskTitleName()) CupidBot.getConfigManager().setConfiguration("runelite", "usernameInTitle", false);
    ...
    public void onBeforeRender(Client client, AnonymousConfig config) {
        if (!CupidBot.isLoggedIn() || !super.run() || !isRunning() || client == null) return;
    ...
    public boolean run(AnonymousConfig config, Client client) {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                if (!CupidBot.isLoggedIn() || !super.run() || !isRunning() || client == null || config == null) return;
