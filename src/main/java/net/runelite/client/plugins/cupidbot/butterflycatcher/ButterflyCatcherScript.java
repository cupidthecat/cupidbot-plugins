package net.runelite.client.plugins.cupidbot.butterflycatcher;
...
public class ButterflyCatcherScript extends Script {
    ...
    public boolean run(ButterflyCatcherConfig config) {
        ...
        CupidBot.log(TAG + " Starting — target: " + targetButterfly.getDisplayName()
                + " | mode: " + catchMode
    ...
    private void tick() {
        try {
            ...
            if (hunterLevel < effectiveLevel()) {
                CupidBot.log(TAG + " Hunter level too low — stopping.");
                shutdown();
                return;
            }
    ...
    private void tickCatching() {
        ...
        var target = CupidBot.getRs2NpcCache()
                .query()
                .withIds(targetButterfly.getNpcIds())
                .nearest();

        if (target == null) {
            CupidBot.log(TAG + " No " + targetButterfly.getDisplayName()
