...
public class WintertodtGearManager {
    ...
    public boolean setupOptimalGear() {
        try {
            CupidBot.log("Setting up optimal Wintertodt gear using database analysis...");
            ...
            CupidBot.log("Error setting up optimal gear: " + e.getMessage());
            e.printStackTrace();
            return false;
        }
    }
    ...
    private boolean analyzeOptimalGearFromDatabase() {
        try {
            CupidBot.log("Analyzing gear using comprehensive database...");
            ...
            CupidBot.log("Error analyzing gear from database: " + e.getMessage());
            return false;
        }
    }
        ...
        try {
            ...
            for (Quest quest : Quest.values()) {
                try {
                    ...
                    CupidBot.log("Failed to check quest: " + quest.name());
                }
            }
            ...
            CupidBot.log("Error checking quest states: " + e.getMessage());
    ...
    private boolean equipOptimalGear() {
        try {
            CupidBot.log("Equipping optimal gear setup...");
            ...
            CupidBot.log("Optimal gear equipped successfully!");
            return true;

        } catch (Exception e) {
            CupidBot.log("Error equipping gear: " + e.getMessage());
            return false;
        }
    }
    ...
    private void equipGearItem(WintertodtGearItem gearItem) {
        ...
        if (Rs2Equipment.isWearing(itemId)) {
            CupidBot.log("Already wearing: " + gearItem.getItemName());
            return;
        }
        ...
        if (!Rs2Inventory.hasItem(itemId)) {
            if (Rs2Bank.hasItem(itemId)) {
                ...
                CupidBot.log("Warning: Cannot find " + gearItem.getItemName() + " in bank");
                return;
            }
        }

        // Equip the item
        CupidBot.log("Equipping: " + gearItem.getItemName());
    ...
    private void bankCurrentGear() {
        try {
            CupidBot.log("Banking current gear (preserving optimal tool placement)...");
            ...
            CupidBot.log("Error banking gear: " + e.getMessage());
        }
    }
    ...
    private boolean ensureBankAccess() {
        if (!Rs2Bank.isOpen()) {
            ...
            if (!Rs2Bank.openBank()) {
                CupidBot.log("Failed to open bank for gear setup");
                return false;
            }
            Rs2Player.waitForWalking();
            if (!Rs2Bank.isOpen()) {
                CupidBot.log("Bank not open for gear setup");
                return false;
            }
        }
        return true;
    ...
    private void logDetailedGearAnalysis() {
        CupidBot.log("=== DETAILED GEAR ANALYSIS RESULTS ===");
        ...
        CupidBot.log("=== GEAR SUMMARY ===");
        CupidBot.log("Total gear pieces: " + optimalGear.size());
        CupidBot.log("Warmth-providing items: " + warmthItems + "/4 (max cold protection)");
        CupidBot.log("Pyromancer pieces: " + pyromancerPieces + "/4 (for XP bonus)");
        CupidBot.log("Total weight reduction: " + Math.abs(totalWeight) + " kg");

        if (warmthItems >= 4) {
            CupidBot.log("MAXIMUM COLD PROTECTION ACHIEVED");
        } else {
            CupidBot.log("Only " + warmthItems + "/4 warmth items - consider getting more warm gear");
        }

        if (pyromancerPieces >= 4) {
            CupidBot.log("Pyromancer XP bonus active ( 2.5% bonus)");
        }
    }
    ...
    public void reset() {
        optimalGear.clear();
        gearAnalysisLog.clear();
        CupidBot.log("Gear manager reset - ready for fresh analysis");
    }
