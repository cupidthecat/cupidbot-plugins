...
import net.runelite.client.plugins.cupidbot.qualityoflife.enums.FiremakingLogs;
...
public class FiremakingManager {
    ...
    private void lightLogsOnClick(MenuEntry event) {
        ...
        if (logToLight.isEmpty()) {
            CupidBot.log("<col=5F1515>No logs available or level too low to light logs</col>");
            return;
        }

        Rs2ItemModel logItem = logToLight.get(0);
        if (logItem == null) {
            CupidBot.log("<col=5F1515>Could not find the selected log in inventory</col>");
            return;
        }

        CupidBot.log("<col=245C2D>Lighting: " + logItem.getName() + "</col>");
