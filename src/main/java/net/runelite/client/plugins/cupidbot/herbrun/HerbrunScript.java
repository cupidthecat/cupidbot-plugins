package net.runelite.client.plugins.cupidbot.herbrun;
...
public class HerbrunScript extends Script {
    ...
    private void populatePatches() {
        this.farmingHandler = new FarmingHandler(CupidBot.getClient(), configManager);
        herbPatches.clear();
    ...
    private boolean handleHerbPatch() {
        ...
        final var obj = CupidBot.getRs2TileObjectCache().query().withIds(HERB_PATCH_IDS).nearest();
    ...
    private static String getHerbPatchState(TileObject rs2TileObject) {
        ...
        var varbitValue = CupidBot.getVarbitValue(game_obj.getVarbitId());
    ...
    private boolean handleFlowerPatch(String region) {
        ...
        final var obj = CupidBot.getRs2TileObjectCache().query().withIds(FLOWER_PATCH_IDS).nearest();
        if (obj == null) {
    ...
    private boolean handleAllotmentPatches(String region) {
        ...
        var allObjects = CupidBot.getRs2TileObjectCache().query().withIds(ALLOTMENT_PATCH_IDS).toList();
    ...
    private static String getPatchState(TileObject tileObj) {
        ...
        int varbit = CupidBot.getVarbitValue(comp.getVarbitId());
    ...
    private boolean applyCompost(Rs2TileObjectModel obj) {
        ...
        int xpBefore = CupidBot.getClient().getSkillExperience(Skill.FARMING);
        Rs2Inventory.use(compost.getItemId());
        obj.click("Compost");
        Rs2Player.waitForWalking();
        boolean applied = sleepUntil(
                () -> CupidBot.getClient().getSkillExperience(Skill.FARMING) > xpBefore, 5000);
    ...
    private boolean noteProduceViaLeprechaun() {
        Rs2NpcModel leprechaun = CupidBot.getRs2NpcCache().query().withName("Tool leprechaun").nearestOnClientThread();
    ...
    private void recoverOwnLimpwurtDrops() {
        for (int i = 0; i < 10; i++) {
            Rs2TileItemModel root = CupidBot.getRs2TileItemCache().query()
    ...
    private AllotmentSeedType getFirstAllotmentSeedInInventory() {
        ...
        for (AllotmentSeedType seed : AllotmentSeedType.getPlantableSeeds(
                CupidBot.getClient().getRealSkillLevel(Skill.FARMING))) {
    ...
    private boolean withdrawSpecificSeeds(int itemId, String seedName, int count, int levelRequired, boolean allowPartial) {
        int farmingLevel = CupidBot.getClient().getRealSkillLevel(Skill.FARMING);
