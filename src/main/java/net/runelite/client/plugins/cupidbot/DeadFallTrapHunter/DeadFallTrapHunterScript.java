package net.runelite.client.plugins.cupidbot.DeadFallTrapHunter;
...
public class DeadFallTrapHunterScript extends Script {
    ...
    public boolean run(DeadFallTrapHunterConfig config, DeadFallTrapHunterPlugin plugin) {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if (forceBank) {
                    CupidBot.log("Inventory too full - Banking");
                    handleBanking(config);
                }
                if (hasDied) {
                    CupidBot.log("We died - Shutting down");
                    shutdown();
                }
                if ((Rs2Inventory.count("axe") < 1 && !Rs2Equipment.isWearing("axe")) && plugin.getTraps().isEmpty()) {
                    CupidBot.log("No axe found - Banking");
                    handleBanking(config);
                    return;
                }
                if (!Rs2Inventory.hasItem("Knife")) {
                    CupidBot.log("No knife found - Banking");
                    handleBanking(config);
                    return;
                }
                ...
                if (!isNearArea(deadFallTrapHunting)) {
                    CupidBot.log("Walking to " + deadFallTrapHunting.getName() + " hunting area");
    ...
    private void handleWoodCutting(DeadFallTrapHunterConfig config, DeadFallTrapHunterPlugin plugin) {
        ...
        if (tree != null) {
            ...
            CupidBot.log("No nearby tree found - Walking back to origin");
            WalkBackToHunterArea(config, plugin);
        }
    }
    ...
    private DeadFallTrapHunting getBestDeadFallHuntingCreature() {
        var skillLevel = CupidBot.getClient().getRealSkillLevel(Skill.HUNTER);
        if (skillLevel > 56) {
        ...
        CupidBot.log("Not high enough hunter level for any deadfall creature");
        shutdown();
        return null;
    }
    ...
    private void getBestAxe(DeadFallTrapHunterConfig config) {
        ...
        if (bestAxeOverall != null) {
            ...
            CupidBot.log("No suitable axe found in bank, inventory, or equipment");
            shutdown();
        }
    }
    ...
    private boolean handleExistingTraps(DeadFallTrapHunterPlugin plugin, DeadFallTrapHunterConfig config) {
        ...
        if (!Rs2Player.isAnimating() && !Rs2Player.isMoving()) {
            Rs2TileObjectModel gameObject = CupidBot.getRs2TileObjectCache().query().within(location, 0).first();
            if (gameObject != null) {
    ...
    private void setNewTrap(DeadFallTrapHunting deadFallTrapHunting, DeadFallTrapHunterConfig config) {
        if (CupidBot.getRs2TileObjectCache().query().withId(deadFallTrapHunting.getTrapId()).count() > 0) {
            CupidBot.getRs2TileObjectCache().query().interact(deadFallTrapHunting.getTrapId(), "Set-trap");
