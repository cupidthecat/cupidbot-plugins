...
public class GauntletHelperScript extends Script {
    ...
    public void checkNPC() {
        TIME_NPC = now;
        Tornado = CupidBot.getRs2NpcCache().query().withId(CG_TORNADO).nearest();
        hunllef = CupidBot.getRs2NpcCache().query().where(npc -> HUNLLEF_IDS.contains(npc.getId())).toList().stream()
                .findFirst()
                .orElse(null);
    ...
    private void checkVitalsStart(){
        int HP = CupidBot.getClient().getBoostedSkillLevel(Skill.HITPOINTS);
        if (HP > STAT_HP) {HP_WENT_UP = true;}
        int PRAY = CupidBot.getClient().getBoostedSkillLevel(Skill.PRAYER);
    ...
    private void checkFood() {
        ...
        int maxHp = CupidBot.getClient().getRealSkillLevel(Skill.HITPOINTS);
