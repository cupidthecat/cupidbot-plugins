package net.runelite.client.plugins.cupidbot.pyrefox.managers;
...
import net.runelite.client.plugins.cupidbot.pyrefox.PyreFoxConfig;
import net.runelite.client.plugins.cupidbot.pyrefox.PyreFoxConstants;
import net.runelite.client.plugins.cupidbot.pyrefox.PyreFoxPlugin;
import net.runelite.client.plugins.cupidbot.pyrefox.enums.PyreFoxState;
import net.runelite.client.plugins.cupidbot.pyrefox.helpers.BankHelper;
import net.runelite.client.plugins.cupidbot.pyrefox.helpers.PlayerHelper;
...
//                System.out.println("Total time for loop " + totalTime);
    ...
    private void _handleCatching()
    {
        ...
        {
            CupidBot.log("No Trap GameObject found.");
            return;
        }
    ...
    private Rs2TileObjectModel _getTrapObjectAtTrapLocation()
    {
        ...
        return CupidBot.getRs2TileObjectCache().query().within(point, 0).first();
    }
    ...
    private WorldPoint _getTrapObjectWorldPoint()
    {
        ...
        {
            var rock = CupidBot.getRs2TileObjectCache().query().withId(PyreFoxConstants.GAMEOBJECT_ROCK_NO_TRAP).within(10).nearest();
            if (rock == null)
            {
                _log("No rock found");
                return null;
        ...
        if (_config.UseMeatPouch())
            if (Rs2Inventory.hasItem(_config.MeatPouch().getClosedItemID()))
                if(Rs2Inventory.interact(_config.MeatPouch().getClosedItemID(), "Open"))
                    CupidBot.log("Opened meat bag.");

        if (!Rs2Inventory.hasItem(ItemID.KNIFE) || !PlayerHelper.playerHasAxeOnHim())
        {
            CupidBot.log("You lack required items - shutting down!");
            shutdown();
        }
