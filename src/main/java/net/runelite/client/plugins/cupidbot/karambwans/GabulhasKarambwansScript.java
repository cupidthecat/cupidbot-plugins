package net.runelite.client.plugins.cupidbot.karambwans;
...
import net.runelite.client.plugins.cupidbot.karambwans.enums.KarambwanBankLocation;
import net.runelite.client.plugins.cupidbot.karambwans.enums.FairyRingAccessMethod;
...
import static net.runelite.client.plugins.cupidbot.karambwans.GabulhasKarambwansInfo.botStatus;
import static net.runelite.client.plugins.cupidbot.karambwans.GabulhasKarambwansInfo.states;
...
public class GabulhasKarambwansScript extends Script {
        ...
        while (!Rs2Inventory.isFull() && super.isRunning()) {
            if (!Rs2Player.isAnimating()) {
                if (Rs2Inventory.contains(ItemID.TBWT_RAW_KARAMBWANJI)) {
                    ...
                    CupidBot.showMessage("Raw karambwanji not detected. Shutting down");
                    shutdown();
                    return;
                }
            }
    ...
    private void walkToRingToBank() {
        ...
        var fairyRing = CupidBot.getRs2TileObjectCache().query().nearest(ringLocation, 3);
        if (fairyRing != null) {
    ...
    private void doBank() {
        if (config.bankLocation() == KarambwanBankLocation.SEERS &&
                CupidBot.getVarbitValue(VarbitID.KANDARIN_DIARY_HARD_COMPLETE) == 1
                && Rs2Magic.hasRequiredRunes(Rs2Spells.CAMELOT_TELEPORT))
        {
            Rs2Magic.cast(Rs2Spells.CAMELOT_TELEPORT, "Seers'", 2);
            sleepUntil(() -> Rs2Bank.isNearBank(net.runelite.client.plugins.cupidbot.util.bank.enums.BankLocation.CAMELOT, 15), 5000);
        } else {
    ...
    private void interactWithFishingSpot() {
        CupidBot.getRs2NpcCache().query().withId(NpcID._0_45_48_KARAMBWAN).interact("Fish");
    }
    ...
    private void walkToFish() {
        if (config.fairyRingAccessMethod() == FairyRingAccessMethod.POH) {
            ...
            sleepUntil(() -> CupidBot.getRs2TileObjectCache().query().withId(FAIRY_RING_ID).nearest() != null || CupidBot.getRs2TileObjectCache().query().withId(SPIRITUAL_FAIRY_TREE_ID).nearest() != null, 5000);

            boolean interacted = CupidBot.getRs2TileObjectCache().query().interact(FAIRY_RING_ID, "Last-destination (DKP)") ||
                                 CupidBot.getRs2TileObjectCache().query().interact(SPIRITUAL_FAIRY_TREE_ID, "Last-destination (DKP)");
            ...
            sleepUntil(() -> CupidBot.getRs2TileObjectCache().query().withId(FAIRY_RING_ID).nearestOnClientThread() != null, 5000);

            // Action is "Last-destination", NOT "Last-destination (DKP)" — the code suffix is not part of the action text.
            boolean interacted = CupidBot.getRs2TileObjectCache().query()
