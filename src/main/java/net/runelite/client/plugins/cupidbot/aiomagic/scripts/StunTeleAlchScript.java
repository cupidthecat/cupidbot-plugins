...
public class StunTeleAlchScript extends Script {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if (configuredTargetNpc == null || configuredTargetNpc.isBlank()) {
                    CupidBot.showMessage("Set a stun NPC name in config");
                    shutdown();
                    return;
                }
                if (plugin.getStunSpell() == null) {
                    CupidBot.showMessage("Set a stun spell in config");
                    shutdown();
                    return;
                }
                if (plugin.getTeleportSpell() == null) {
                    CupidBot.showMessage("Set a teleport spell in config");
                    shutdown();
                    return;
                }
                ...
                if (plugin.getAlchItemNames().isEmpty()
                    ...
                    CupidBot.log("Missing alchemy items...");
                    return;
                }
                ...
                if (!Rs2Magic.hasRequiredRunes(plugin.getStunSpell().getRs2Spell())) {
                    CupidBot.showMessage("Out of runes for " + plugin.getStunSpell().name());
                    shutdown();
                    return;
                }
                ...
                var target = CupidBot.getRs2NpcCache().query()
                ...
                if (target.getWorldLocation() == null) {
                    CupidBot.log("NPC has no valid world location: " + configuredTargetNpc);
                    return;
                }
                if (!Rs2Magic.cast(plugin.getStunSpell().getRs2Spell().getMagicAction())) {
                    CupidBot.log("Failed to select stun spell: " + plugin.getStunSpell().name());
                    return;
                }
                if (!sleepUntil(() -> CupidBot.getClient().isWidgetSelected(), 800)) {
                    CupidBot.log("Stun spell was not selected in time");
                    return;
                }
                boolean stunned = target.click();
                if (!stunned) {
                    CupidBot.log("Failed to cast stun on NPC: " + configuredTargetNpc);
                    return;
                }
                // 2) ALCHEMY
                if (Rs2AntibanSettings.naturalMouse) {
                ...
                CupidBot.log("Total time for loop " + totalTime);
            } catch (Exception ex) {
                CupidBot.log("Stun TeleAlchemy loop failed: " + ex.getMessage());
            }
    ...
    private void attemptNpcRecoveryTeleport(String configuredTargetNpc) {
        ...
        if (!recovered) {
            CupidBot.log("Recovery teleport failed for missing NPC: " + configuredTargetNpc);
        }
    }
    ...
    private boolean castConfiguredTeleportAndWait(String context) {
        ...
        if (!Rs2Magic.cast(plugin.getTeleportSpell().getRs2Spell().getMagicAction())) {
            CupidBot.log("Failed to cast teleport (" + context + "): " + plugin.getTeleportSpell().name());
            return false;
        }
        ...
        boolean teleportStarted = sleepUntil(() ->
                ...
                2400);
        if (!teleportStarted) {
            CupidBot.log("Teleport did not start in time (" + context + "): " + plugin.getTeleportSpell().name());
            return false;
        }
        ...
        if (!teleportFinished) {
            CupidBot.log("Teleport did not finish in time (" + context + "): " + plugin.getTeleportSpell().name());
            return false;
        }
