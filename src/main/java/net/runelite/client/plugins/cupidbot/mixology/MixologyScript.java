package net.runelite.client.plugins.cupidbot.mixology;
...
import net.runelite.client.plugins.cupidbot.mixology.enums.AlchemyObject;
import net.runelite.client.plugins.cupidbot.mixology.enums.MixologyState;
import net.runelite.client.plugins.cupidbot.mixology.enums.PotionComponent;
import net.runelite.client.plugins.cupidbot.mixology.enums.PotionModifier;
...
import static net.runelite.client.plugins.cupidbot.mixology.enums.AlchemyObject.MIXING_VESSEL;
...
public class MixologyScript extends Script {
    ...
    public boolean run(MixologyConfig config) {
        ...
        if (!Rs2AntibanSettings.naturalMouse) {
            CupidBot.log("Hey! Did you know this script works really well with natural mouse? Feel free to enable it in the antiban settings.");
        }
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if (leverRetries >= 20) {
                    CupidBot.log("Failed to create a potion. Please do this step manually and restart the script.");
                    return;
                }
                ...
                if (isInMinigame) {
                    ...
                    if (digweed != null && !Rs2Player.isAnimating() && !Rs2Inventory.hasItem(DIGWEED)
                            && config.pickDigWeed()) {
                        CupidBot.getRs2TileObjectCache().query().withId(digweed.objectId()).interact();
                ...
                switch (mixologyState) {
                    ...
                    case REFINER:
                        ...
                        if (Rs2Inventory.hasItem(config.agaHerb().toString()) || Rs2Inventory.hasItem(config.lyeHerb().toString()) || Rs2Inventory.hasItem(config.moxHerb().toString())) {
                            CupidBot.getRs2TileObjectCache().query().withId(ObjectID.MM_LAB_MILL).interact();
                            Rs2Player.waitForAnimation();
                            sleepGaussian(450, 150);
                            if (!config.useQuickActionRefiner()) {
                                sleepUntil(() -> !CupidBot.isGainingExp, 30000);
                            }
                            return;
                        }
                        if (Rs2Bank.openBank()) {
                            ...
                            if (!Rs2Bank.hasItem(herb, true)) {
                                CupidBot.showMessage("Failed to find " + herb + " in your bank. Shutting down script...");
                                shutdown();
                                return;
                            }
                        ...
                        break;
                    case DEPOSIT_HOPPER:
                        if (CupidBot.getRs2TileObjectCache().query().withId(ObjectID.MM_LAB_HOPPER).interact()) {
                            Rs2Player.waitForWalking();
                    ...
                    case TAKE_FROM_MIXIN_VESSEL:
                        CupidBot.getRs2TileObjectCache().query().withId(MIXING_VESSEL.objectId()).interact();
                    ...
                    case MIX_POTION_STAGE_2:
                        ...
                        if (Rs2Player.isAnimating()) {
                            ...
                            if (nonFulfilledPotion.potionModifier().alchemyObject() == AlchemyObject.RETORT && config.useQuickActionOnRetort()&&CupidBot.getVarbitValue(11327)<15&&CupidBot.getVarbitValue(11327)!=0) {
                                quickActionProcessPotion(nonFulfilledPotion);
                                sleep(350, 400);
                            }
                            return;
                    ...
                    case CONVEYER_BELT:
                        ...
                        if (CupidBot.getRs2TileObjectCache().query().withId(AlchemyObject.CONVEYOR_BELT.objectId()).interact()) {
        ...
        switch (nonFulfilledPotion.potionModifier()) {
            case HOMOGENOUS:
                if (isAlchemyObjectAnimating(AlchemyObject.AGITATOR, 11633, 11632)) {
                    CupidBot.getRs2TileObjectCache().query().withId(AlchemyObject.AGITATOR.objectId()).interact();
                } else {
            ...
            case CONCENTRATED:
                if (isAlchemyObjectAnimating(AlchemyObject.RETORT, 11643, 11642)) {
                    CupidBot.getRs2TileObjectCache().query().withId(AlchemyObject.RETORT.objectId()).interact();
                } else {
            ...
            case CRYSTALISED:
                if (isAlchemyObjectAnimating(AlchemyObject.ALEMBIC, 11638, 11637)) {
                    CupidBot.getRs2TileObjectCache().query().withId(AlchemyObject.ALEMBIC.objectId()).interact();
                } else {
    ...
    private static boolean isAlchemyObjectAnimating(AlchemyObject alchemyObject, int... animationIds) {
        Rs2TileObjectModel model = CupidBot.getRs2TileObjectCache().query().withId(alchemyObject.objectId()).nearest();
        if (model == null) return false;
        try {
            net.runelite.api.Scene scene = CupidBot.getClient().getTopLevelWorldView().getScene();
    ...
    private void createPotion(PotionOrder potionOrder, MixologyConfig config) {
        for (PotionComponent component : potionOrder.potionType().components()) {
            ...
            if (component.character() == 'A') {
                CupidBot.getRs2TileObjectCache().query().withId(AlchemyObject.AGA_LEVER.objectId()).interact();
            } else if (component.character() == 'L') {
                CupidBot.getRs2TileObjectCache().query().withId(AlchemyObject.LYE_LEVER.objectId()).interact();
            } else if (component.character() == 'M') {
                CupidBot.getRs2TileObjectCache().query().withId(AlchemyObject.MOX_LEVER.objectId()).interact();
            }
    ...
    private boolean canCreatePotion(PotionOrder potionOrder) {
        Rs2TileObjectModel[] mixerModels = {
                CupidBot.getRs2TileObjectCache().query().withId(ObjectID.MM_LAB_MIXER_03).nearest(),
                CupidBot.getRs2TileObjectCache().query().withId(ObjectID.MM_LAB_MIXER_02).nearest(),
                CupidBot.getRs2TileObjectCache().query().withId(ObjectID.MM_LAB_MIXER_01).nearest()
        };
