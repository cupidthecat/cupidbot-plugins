...
public class TheMessScript extends Script {
    ...
    private void tick() {
        try {
            if (!CupidBot.isLoggedIn() || !super.run() || !isRunning()) return;
            ...
            if (shouldSuppressTickFailure(ex)) {
                Thread.currentThread().interrupt();
                log.info("[mess] tick interrupted; exiting current cycle");
                return;
            }
    ...
    static boolean shouldSuppressTickFailure(Throwable throwable) {
        return Script.isInterruption(throwable);
    }
    ...
    private static void setChatboxAmount(int amount) {
        CupidBot.getClientThread().runOnClientThreadOptional(() -> {
            ...
            CupidBot.getClient().setVarcStrValue(VarClientStr.INPUT_TEXT, String.valueOf(amount));
            return null;
        });
    }
    ...
    private void hopWorld() {
        ...
        CupidBot.hopToWorld(Login.getRandomWorld(true));
        sleepUntil(() -> CupidBot.getClient().getGameState() == GameState.HOPPING, 5000);
        sleepUntil(() -> CupidBot.getClient().getGameState() == GameState.LOGGED_IN, 15000);
        if (CupidBot.getClient().getWorld() == currentWorld) {
