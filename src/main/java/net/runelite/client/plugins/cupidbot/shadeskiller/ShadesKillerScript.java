package net.runelite.client.plugins.cupidbot.shadeskiller;

import net.runelite.api.Skill;
import net.runelite.client.plugins.cupidbot.CupidBot;
import net.runelite.client.plugins.cupidbot.Script;
import net.runelite.client.plugins.cupidbot.shadeskiller.enums.State;
...
import static net.runelite.client.plugins.cupidbot.shadeskiller.enums.State.USE_TELEPORT_TO_BANK;
...
public class ShadesKillerScript extends Script {
    ...
    private boolean withdrawRequiredItems() {
        ...
        if (key.isEmpty()) {
            CupidBot.showMessage("You are missing a silver key.");
            sleep(5000);
            return false;
        }
    ...
    public boolean run(ShadesKillerConfig config) {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if (CupidBot.getClient().getBoostedSkillLevel(Skill.HITPOINTS) <= config.hpTreshhold() && Rs2Inventory.hasItem(config.teleportItemToBank())) {
                ...
                switch (state) {
                    ...
                    case FIGHT_SHADES:
                        ...
                        if (npc != null && !CupidBot.getClient().getLocalPlayer().isInteracting()) {
                            npc.click("Attack");
                            return;
                        }
                        if (!Rs2Combat.inCombat() && !isLooting) {
                            CupidBot.getClientThread().invoke(() -> CupidBot.getRs2NpcCache().query()
