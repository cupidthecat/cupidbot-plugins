package net.runelite.client.plugins.cupidbot.banksshopper;
...
public class BanksShopperScript extends Script {
    ...
    public boolean run(BanksShopperConfig config) {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                if (!super.run() || !CupidBot.isLoggedIn() || Rs2AntibanSettings.actionCooldownActive) return;
                ...
                switch (state) {
                    case SHOPPING:
                        ...
                        if (missingAllRequiredItems && plugin.getSelectedAction() == Actions.SELL) {
                            CupidBot.status = "[Shutting down] - Reason: Not enough supplies.";
                        ...
                        if (Rs2Shop.isOpen()) {
                            ...
                            for (String itemName : plugin.getItemNames()) {
                                if (!isRunning() || CupidBot.pauseAllScripts.get()) break;
    ...
    private boolean tryDirectBankDeposit() {
        ...
        if (!alreadyOpen) {
            ...
            if (bankObject != null) {
                CupidBot.log("[BanksShopper] direct-click bank id=" + bankObject.getId()
                ...
                if (!sleepUntil(() -> Rs2Bank.isOpen() || Rs2DepositBox.isOpen(), 15_000)) {
                    CupidBot.log("[BanksShopper] click sent but no bank/deposit UI opened");
                    return false;
                }
            } else {
    ...
    private void hopWorld() {
        ...
        sleepUntil(() -> CupidBot.hopToWorld(world), 15000);
