...
@PluginDescriptor(
        ...
        tags = {"pumster", "cupidbot", "Vyrewatch"},
        authors = {"Pumsters"},
        version = PVirewatchKillerPlugin.version,
        minClientVersion = "2.0.7",
        iconUrl = "PVirewatchKillerPlugin/assets/icon.png",
        cardUrl = "PVirewatchKillerPlugin/assets/card.png",
...
public class PVirewatchKillerPlugin extends Plugin {
    ...
    private void onGameTick(GameTick event) {
        ...
        if(fightArea != null) {
            if(fightArea.contains(CupidBot.getClient().getLocalPlayer().getWorldLocation())) {
                ticksOutOfArea = 0;
            } else if (!plugin.rechargingPrayer) {
                ticksOutOfArea++;
            }
        ...
        if(CupidBot.getClient().getBoostedSkillLevel(Skill.PRAYER) > 0) {
    ...
    {
        if (event.getContainerId() == CupidBot.getClient().getItemContainer(InventoryID.INVENTORY).getId())
        {
