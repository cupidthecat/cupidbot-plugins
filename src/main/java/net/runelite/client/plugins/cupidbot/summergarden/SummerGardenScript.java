package net.runelite.client.plugins.cupidbot.summergarden;
...
import  net.runelite.client.plugins.cupidbot.summergarden.ElementalCollisionDetector;
import  net.runelite.client.plugins.cupidbot.summergarden.SummerGardenConfig;
...
public class SummerGardenScript extends Script {
    ...
    public boolean run( net.runelite.client.plugins.cupidbot.summergarden.SummerGardenConfig config) {
                ...
                if (config.autoHandInAndReset()) {
                    ...
                    if (botState == BotState.RUN) {
                        ...
                        if (config.sendInvFullNotification() && !sendRs2InventoryFullNotification) {
                            ...
                            //CupidBot.getClient().playSoundEffect(OUT_OF_SUPPLY_SOUND, SoundEffectVolume.HIGH);
                            CupidBot.log("You need a beer glass and a pestle and mortar to make sq'irk juice.");
    ...
    private boolean isInAlKharid() {
        return CupidBot.getClientThread().invoke(() -> CupidBot.getClient().getLocalPlayer().getWorldLocation().getRegionID()) == REGION_ALKHARID;
    }

    private boolean isInGarden() {
        return CupidBot.getClientThread().invoke(() -> CupidBot.getClient().getLocalPlayer().getWorldLocation().getRegionID()) == REGION_GARDEN;
    }

    private boolean isInHouseArea() {
        WorldPoint loc = CupidBot.getClientThread().invoke(() -> CupidBot.getClient().getLocalPlayer().getWorldLocation());
    ...
    private WallObject getHouseDoor() {
        ...
        LocalPoint doorLp = LocalPoint.fromWorld(CupidBot.getClient(), 3321, 3142);
        if (doorLp == null) {
            return null;
        }
    ...
    private boolean goInsideHouse() {
        ...
        if (!isInAlKharid() || CupidBot.getClient().getTopLevelWorldView().getPlane() != 0) {
            return false;
        }
        ...
        if (!isInHouseArea()) {
            ...
            if (getHouseDoor() != null) {
                CupidBot.getRs2TileObjectCache().query().interact(OBJECT_HOUSE_DOOR_CLOSED);
    ...
    private void exitGarden() {
        ...
        if (CupidBot.getRs2TileObjectCache().query().withId(12941).nearest() == null) {
            return;
        }

        // If the player is still in the garden then click the fountain to exit and wait until the player is teleported out.
        if (isInGarden()) {
            CupidBot.getRs2TileObjectCache().query().interact(12941);
    ...
    private void doMaze(SummerGardenConfig config) {
        ...
        // Click tree
        if (WORLD_POINT_MAZE_STARTING_LOCATION.equals(CupidBot.getClientThread().invoke(() -> CupidBot.getClient().getLocalPlayer().getWorldLocation()))) {
            if (config.waitForOneClick() || ElementalCollisionDetector.getTicksUntilStart() == 0) {
                CupidBot.getRs2TileObjectCache().query().interact(OBJECT_SUMMER_TREE);
                sleepUntil(() -> Rs2Player.isMoving());
                sleepUntil(() -> !Rs2Player.isMoving(), 30000);
                sleepUntilOnClientThread(() -> CupidBot.getClient().getLocalPlayer().getWorldLocation().getY() < 5481);
        ...
        if (CupidBot.getClientThread().invoke(() -> CupidBot.getClient().getLocalPlayer().getWorldLocation().getY()) >= 5481) {
            return;
        }

        // Click Gate
        Rs2TileObjectModel gate = CupidBot.getRs2TileObjectCache().query().withId(ObjectID.GATE_11987).nearest();
        if (gate != null) {
            gate.click();
            sleepUntil(Rs2Player::isMoving);
            sleepUntil(() -> !Rs2Player.isMoving());
            sleepUntilOnClientThread(() -> CupidBot.getClient().getLocalPlayer().getWorldLocation().equals(WORLD_POINT_MAZE_STARTING_LOCATION));
        }
    }
    ...
    private void completeAndReset() {
        ...
        var npcOsman = CupidBot.getRs2NpcCache().query().withName(NPC_NAME_OSMAN).nearestOnClientThread();
        if (npcOsman == null) {
            var osmanLocalLocation = LocalPoint.fromWorld(CupidBot.getClient(), osmanLocation);
        ...
        if (lastInteractedActor == null || !Objects.equals(lastInteractedActor.getName(), NPC_NAME_OSMAN)) {
            CupidBot.getClientThread().invoke(() -> CupidBot.getRs2NpcCache().query().withName(NPC_NAME_OSMAN).interact("Talk-to"));
        ...
        if (Objects.equals(lastInteractedActor.getName(), NPC_NAME_OSMAN)) {
            var npcDialogueText = CupidBot.getClient().getWidget(WidgetInfo.DIALOG_NPC_TEXT);
            ...
            //sleepUntil(() -> CupidBot.getClient().getWidget(WidgetInfo.DIALOG_OPTION_OPTIONS) != null, 2000);
            var playerDialogueOptionsWidget = CupidBot.getClient().getWidget(WidgetInfo.DIALOG_OPTION_OPTIONS);
            ...
            //sleepUntil(() -> CupidBot.getClient().getWidget(WidgetInfo.DIALOG_PLAYER_TEXT) != null, 2000);
            var playerDialogueText = CupidBot.getClient().getWidget(WidgetInfo.DIALOG_PLAYER_TEXT);
            ...
            //sleepUntil(() -> CupidBot.getClient().getWidget(WidgetInfo.DIALOG_SPRITE_TEXT) != null, 2000);
            var npcDialogueSpriteText = CupidBot.getClient().getWidget(WidgetInfo.DIALOG_SPRITE_TEXT);
    ...
    private void handleReturnToHouse() {
        ...
        if (!isInHouseArea()) {
            var outsideHouseDoorLocalLocation = LocalPoint.fromWorld(CupidBot.getClient(), WORLD_POINT_OUTSIDE_HOUSE_DOOR);
            if (outsideHouseDoorLocalLocation != null && CupidBot.getClient().getLocalPlayer().getLocalLocation().distanceTo(outsideHouseDoorLocalLocation) >= MAX_DISTANCE) {
        ...
        while (Rs2Inventory.count("Beer glass") < 25) {
            CupidBot.getRs2TileObjectCache().query().interact(OBJECT_BEER_GLASS_SHELF);
            sleep(3000);
        }
        ...
        var npcApprentice = CupidBot.getRs2NpcCache().query().withName(NPC_NAME_APPRENTICE).nearestOnClientThread();
        ...
        if (lastInteractedActor == null || !Objects.equals(lastInteractedActor.getName(), NPC_NAME_APPRENTICE)) {
            CupidBot.getClientThread().invoke(() -> CupidBot.getRs2NpcCache().query().withName(NPC_NAME_APPRENTICE).interact("Teleport"));
    ...
    private void makeLastJuice() {
        ...
        while (Rs2Inventory.count("Summer sq'irkjuice") != 26) {
            CupidBot.getRs2TileItemCache().query().withName("Summer sq'irkjuice").interact("Take");
